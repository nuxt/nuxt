...
export function parseData (html: string, appId = 'nuxt-app', multiApp = false) {
  if (!isRenderingJson) {
  ...
  const regexp = !multiApp ? /<script type="application\/json" id="__NUXT_DATA__"(?<attrs>[^>]+)>(?<script>.*?)<\/script>/ : new RegExp(`<script type="application\\/json" data-nuxt-data="${appId}"(?<attrs>[^>]+)>(?<script>.*?)<\\/script>`)
