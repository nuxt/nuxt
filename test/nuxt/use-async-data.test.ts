...
describe('useAsyncData', () => {
  ...
  it('should throw TypeError when key is empty', () => {
    expect(() => useAsyncData('', () => Promise.resolve('test'))).toThrowErrorMatchingInlineSnapshot('[Error: [NUXT_E7011] [useAsyncData] key must be a non-empty string.]')
  })
  ...
  it('should capture errors', async () => {
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringMatching(
      /\[NUXT_E3004\] \[useAsyncData\] Incompatible options detected for "[^"]+":\n- different handler\n- different `default` value[\s\S]*fix: You can use a different key or move the call to a composable to ensure/,
    ))
    warn.mockRestore()
    vi.unstubAllGlobals()
  })
  ...
  it('should warn if incompatible options are used', async () => {
    ...
    const incompatibleOptionsCalls = () => warn.mock.calls.filter(args => String(args[0]).includes('Incompatible options'))
    ...
    expect(warn).toHaveBeenCalledWith(expect.stringMatching(
      /\[NUXT_E3004\] \[useAsyncData\] Incompatible options detected for "dedupedKey3":\n- mismatching `deep` option[\s\S]*fix: You can use a different key or move the call to a composable to ensure/,
    ))
