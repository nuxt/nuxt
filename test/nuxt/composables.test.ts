...
describe('routing utilities: `navigateTo`', () => {
  ...
  it('navigateTo should disallow navigation to external URLs by default', () => {
    expect(() => navigateTo('https://test.com')).toThrow(/\[NUXT_E2001\].*Navigating to external URL `https:\/\/test\.com` is not allowed by default/)
  ...
  it('navigateTo should disallow navigation to data/script URLs', () => {
    ...
    for (const [url, protocol] of urls) {
      expect(() => navigateTo(url, { external: true })).toThrow(new RegExp(`\\[NUXT_E2002\\].*Cannot navigate to URL.*with \\\`${protocol}:\\\` protocol`))
    }
  })
