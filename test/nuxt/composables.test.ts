...
describe('routing utilities: `navigateTo`', () => {
  ...
  it('navigateTo should disallow navigation to external URLs by default', () => {
    expect(() => navigateTo('https://test.com')).toThrowErrorMatchingInlineSnapshot('[Error: [nuxt] Navigating to external URL `https://test.com` is not allowed by default. Use `navigateTo(\'https://test.com\', { external: true })`.]')
  ...
  it('navigateTo should disallow navigation to data/script URLs', () => {
    ...
    for (const [url, protocol] of urls) {
      expect(() => navigateTo(url, { external: true })).toThrowError(`[nuxt] Cannot navigate to URL \`${url}\` with \`${protocol}:\` protocol.`)
    }
  })
