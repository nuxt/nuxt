...
test('overlays a script syntax error introduced while the page is open', async ({ page, goto }) => {
  writeFileSync(join(fixtureDir, 'app/app.vue'), appVue)
  await goto('/')
  await expect(page.locator('nuxt-error-overlay')).toHaveCount(0)

  writeFileSync(join(fixtureDir, 'app/app.vue'), appVue.replace('function throwInTheBrowser', 'const broken: = 1\nfunction throwInTheBrowser'))

  const overlay = page.locator('nuxt-error-overlay')
  await expect(overlay).toHaveCount(1, { timeout: 15_000 })
  await expect(overlay.locator('.mb-message')).toContainText('Unexpected token')
