...
describe('experimental.lazyRouteDiscovery', () => {
  ...
  it('resolves route stubs via the discovery endpoint', async () => {
    // per-path resolution returns the matching top-level stub (with its group marker)
    const about = await $fetch<{ records: Array<{ path: string, meta?: Record<string, unknown> }>, notFound?: string[] }>('/__nuxt_routes', { query: { path: '/about' } })
    expect(about.records.some(r => r.path === '/about')).toBe(true)
    expect(about.records[0]!.meta?.__nuxtRouteGroup).toBeDefined()
    expect(about.notFound).toEqual([])

    // dynamic path resolves to its pattern stub, not the literal
    const dynamic = await $fetch<{ records: Array<{ path: string }> }>('/__nuxt_routes', { query: { path: '/dynamic/123' } })
    expect(dynamic.records.some(r => r.path.includes(':'))).toBe(true)

    // a genuinely-unknown root path is reported as notFound (no catch-all at the root)
    const missing = await $fetch<{ records: unknown[], notFound?: string[] }>('/__nuxt_routes', { query: { path: '/definitely-not-a-route' } })
    expect(missing.notFound).toContain('/definitely-not-a-route')

    // the static-hosting fallback manifest returns the entire stub table
    const all = await $fetch<{ records: unknown[] }>('/__nuxt_routes.json')
    expect(all.records.length).toBeGreaterThan(1)
  })

  it('boots without the full route table: inlines linked routes, hides unlinked ones', async () => {
    const page = await createPage('/')
    await page.waitForFunction(() => window.useNuxtApp?.()._route.fullPath === '/')

    // fog of war: the payload seeds only the current route + routes linked on the page. `/about` is
    // linked (inlined); `/opt` is not linked anywhere and must be absent from the client's boot state.
    const payload = await page.evaluate(() => (window.useNuxtApp?.().payload as { routes?: Array<{ path?: string }> }).routes?.map(r => r.path) ?? [])
    expect(payload).toContain('/about')
    expect(payload.some(p => p?.includes('opt'))).toBe(false)

    const knownBefore = await page.evaluate(() => (window.useNuxtApp?.().$router as { getRoutes: () => Array<{ path: string }> }).getRoutes().some(r => r.path.startsWith('/opt')))
    expect(knownBefore).toBe(false)
    await page.close()
  })

  it('discovers an unlinked route via the endpoint on client-side navigation', async () => {
    const page = await createPage('/')
    await page.waitForFunction(() => window.useNuxtApp?.()._route.fullPath === '/')

    // navigating to an unlinked route triggers a discovery request, then renders
    const discovery = page.waitForRequest(req => req.url().includes('/__nuxt_routes'))
    await page.evaluate(() => (window.useNuxtApp?.().$router as { push: (to: string) => Promise<unknown> }).push('/opt'))
    await discovery
    await page.waitForFunction(() => window.useNuxtApp?.()._route.path === '/opt')
    expect(await page.locator('body').textContent()).toContain('opt-none')
    await page.close()
  })

  it('batches concurrent link discovery into few endpoint requests', async () => {
    const page = await createPage()
    const requests: string[] = []
    page.on('request', (r) => {
      if (r.url().includes('/__nuxt_routes')) { requests.push(r.url()) }
    })
    // `/spa` inlines nothing (ssr: false), so its ~20 nav links all discover on the client — these
    // must coalesce into a couple of requests, not one per link
  ...
  it('a `discover="none"` link is not inlined or auto-discovered, but still navigates on click', async () => {
    const page = await createPage('/')
    await page.waitForFunction(() => window.useNuxtApp?.()._route.fullPath === '/')

    // not inlined into the payload, and not registered by on-mount discovery
    const inlined = await page.evaluate(() => (window.useNuxtApp?.().payload as { routes?: Array<{ path?: string }> }).routes?.some(r => r.path === '/static-hidden'))
    expect(inlined).toBeFalsy()
    const known = await page.evaluate(() => (window.useNuxtApp?.().$router as { getRoutes: () => Array<{ path: string }> }).getRoutes().some(r => r.path === '/static-hidden'))
    expect(known).toBe(false)

    // clicking discovers it and navigates
    await page.locator('#link-nodiscover').click()
    await page.waitForFunction(() => window.useNuxtApp?.()._route.path === '/static-hidden')
    expect(await page.locator('#page').textContent()).toContain('static-hidden-page')
    await page.close()
  })

  it('follows a redirect to an unlinked target, discovering both', async () => {
    const page = await createPage('/')
    await page.waitForFunction(() => window.useNuxtApp?.()._route.fullPath === '/')

    // `/redirect-hidden` (unlinked) redirects to `/static-hidden` (also unlinked): navigating to it
    // discovers the redirect stub, follows it, then discovers the target
    await page.evaluate(() => (window.useNuxtApp?.().$router as { push: (to: string) => Promise<unknown> }).push('/redirect-hidden'))
    await page.waitForFunction(() => window.useNuxtApp?.()._route.path === '/static-hidden')
    expect(await page.locator('#page').textContent()).toContain('static-hidden-page')
    await page.close()
  })

  it('shows a 404 for a genuinely unknown route on client-side navigation', async () => {
    const page = await createPage('/')
    await page.waitForFunction(() => window.useNuxtApp?.()._route.fullPath === '/')
