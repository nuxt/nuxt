...
export default defineNuxtModule({
  ...
  setup (_options, nuxt) {
    ...
    // the renderer config re-exports the head module's templates through `#build`, and the
    // SSR bundle imports the asset URL helpers a server runtime would otherwise provide
    const writtenTemplates = new Set(['unhead-options.mjs', 'unhead.config.mjs', 'paths.mjs'])
    nuxt.hook('app:templates', (app) => {
      for (const template of app.templates) {
        if (writtenTemplates.has(template.filename)) {
          template.write = true
        }
      }
    })
    ...
    nuxt.hook('build:done', async () => {
      ...
      const outputs = {
        ...
        'entry.mjs': `export { default } from ${JSON.stringify('./' + relative(dir, serverEntry))}`,
