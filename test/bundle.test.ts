...
describe.skipIf(isStubbed || process.env.SKIP_BUNDLE_SIZE === 'true' || process.env.ECOSYSTEM_CI)('minimal nuxt application', () => {
  ...
  it('default client bundle size (pages)', async () => {
    ...
    expect.soft(roundToKilobytes(clientStats!.totalBytes)).toMatchInlineSnapshot(`"187k"`)
  ...
  it('default server bundle size', async () => {
    ...
    expect.soft(roundToKilobytes(serverStats.totalBytes)).toMatchInlineSnapshot(`"80.8k"`)
  ...
  it('default server bundle size (pages)', async () => {
    ...
    expect.soft(roundToKilobytes(serverStats.totalBytes)).toMatchInlineSnapshot(`"186k"`)
