...
describe.skipIf(isStubbed || process.env.SKIP_BUNDLE_SIZE === 'true' || process.env.ECOSYSTEM_CI)('minimal nuxt application', () => {
  ...
  it('default server bundle size', async () => {
    ...
    expect.soft(roundToKilobytes(serverStats.totalBytes)).toMatchInlineSnapshot(`"70.4k"`)
  ...
  it('default server bundle size (pages)', async () => {
    ...
    expect.soft(roundToKilobytes(modules.totalBytes)).toMatchInlineSnapshot(`"484k"`)
