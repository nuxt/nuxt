...
describe.skipIf(isWindows || process.env.ECOSYSTEM_CI)('minimal nuxt application', () => {
  ...
  it('default client bundle size', async () => {
    ...
    expect(roundToKilobytes(stats.client.totalBytes)).toMatchInlineSnapshot('104')
  ...
  it('default server bundle size', async () => {
    ...
    expect(roundToKilobytes(stats.server.totalBytes)).toMatchInlineSnapshot('91')

    const modules = await analyzeSizes('node_modules/**/*', serverDir)
    expect(roundToKilobytes(modules.totalBytes)).toMatchInlineSnapshot('2632')
