        ...
        <v-card-text>
          <v-list>
            ...
            <p v-else-if="$fetchState.error">
              Error while fetching users: {{ $fetchState.error.message }}
            </p>

            <template v-for="(user, index) in users" v-else>
...
export default {
  async fetch() {
    if (process.server) {
      this.users = this.$root.context.ssrContext.$db.getCollection("users").data
    } else {
