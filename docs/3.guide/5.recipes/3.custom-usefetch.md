...
:read-more{to="/docs/api/composables/create-use-fetch"}

## Recipe: Custom `$fetch` Instance

If you need lower-level control, you can create a custom `$fetch` instance with a [Nuxt plugin](/docs/directory-structure/app/plugins) and either use it with `useAsyncData` directly or pass it to `createUseFetch`.
...
::callout
Wrapping with [`useAsyncData`](/docs/api/composables/use-async-data) **avoids double data fetching when doing server-side rendering** (server & client on hydration).
::
