...
Our data is safe with the server-side route in place, but without doing anything else, unauthenticated users would probably get some odd data when trying to access the `/users` page. We should create a [client-side middleware](/docs/directory-structure/middleware) to protect the route on the client side and redirect users to the login page.
