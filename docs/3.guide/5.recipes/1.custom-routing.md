...
In Nuxt, your routing is defined by the structure of your files inside the [app/pages directory](/docs/directory-structure/app/pages). However, since it uses [vue-router](https://router.vuejs.org) under the hood, Nuxt offers you several ways to add custom routes in your project.
...
On top of customizing options for [`vue-router`](https://router.vuejs.org/api/interfaces/routeroptions), Nuxt offers [additional options](/docs/api/nuxt-config#router) to customize the router.
...
::note
Routing is case-sensitive by default to match Nitro. Set `router.options.sensitive` to `false` to opt out.
::
