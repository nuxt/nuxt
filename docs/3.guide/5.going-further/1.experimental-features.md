...
Enable or disable the seasonal snow effect on the development server loading screen.

When enabled, snowflakes will be displayed during winter months (November-January in Northern Hemisphere, May-July in Southern Hemisphere). The effect automatically respects `prefers-reduced-motion` accessibility settings.
