...
These hooks are available for [Nuxt modules](/docs/guide/modules) and build context.
...
App hooks can be mainly used by [Nuxt Plugins](/docs/directory-structure/app/plugins) to hook into rendering lifecycle but could also be used in Vue composables.
...
These hooks are available for [server plugins](/docs/directory-structure/server#server-plugins) to hook into Nitro's runtime behavior.
