...
It's possible to use different [server builders](/docs/guide/going-further/builders) with Nuxt, either Nitro 3 (`nitro`, with Nuxt 5+), Nitro 2 (`nitropack`, with Nuxt 3/4), or Vite directly.
...
These names, except `useAppConfig`, are [auto-imported](/docs/directory-structure/server) in server code, so the import statement above is optional. Writing it keeps the file working in a project with server auto-imports off, and is what a module should do.
...
To read, append or remove a response header, use `event.res.headers` directly: it is a standard `Headers` object.
...
export default defineEventHandler(event => proxyRequest(toNuxtRequestEvent(event), 'https://example.com'))
```

Helpers that need you to call `toNuxtRequestEvent` are `proxy`, `proxyRequest`, `fetchWithEvent`, `writeEarlyHints`, and reading `event.node`, `event.waitUntil()` or `event.runtime`. The helper works on every builder, and a cast to `NuxtRequestEvent` is safe even if the runtime's event is web-shaped, as it is under `@nuxt/nitro-server`.
...
- **Caching.** `defineCachedHandler()` and `defineCachedFunction()` come from `nitro/cache`. For caching that runs anywhere, depend on a cache library directly.
- **Server plugins and hooks.** `definePlugin()` comes from `nitro`, and the runtime hooks (including `render:html`) from `useNitroHooks()` in `nitro/app`.
- **App-relative fetch.** `serverFetch()` comes from `nitro`. It calls a route of the app without an origin, which needs the runtime's router.
- **Lazy handlers.** `lazyEventHandler()` comes from `nitro/h3`.
