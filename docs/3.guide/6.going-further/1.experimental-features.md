...
When using the Vite builder (default), decorators are lowered via [Babel](https://babeljs.io/) using [`@rollup/plugin-babel`](https://github.com/rollup/plugins/tree/master/packages/babel) and [`@babel/plugin-proposal-decorators`](https://babeljs.io/docs/babel-plugin-proposal-decorators). When using the webpack or rspack builders, decorators are lowered via [esbuild](https://github.com/evanw/esbuild/releases/tag/v0.21.3).
...
::note
When using the Vite builder, Nuxt will prompt you to install the required Babel packages (`@rollup/plugin-babel` and `@babel/plugin-proposal-decorators`) if they are not already present.
::
