...
When enabled, the client bundle ships a lightweight route table containing only the fields needed for route matching (path, name, alias and redirect). The full route records — page meta and component chunk references — are batched into lazily-loaded group chunks that are fetched when a matching link becomes visible or a navigation targets them. The group chunk needed to hydrate the current route is preloaded via `modulepreload` hints, and the loading indicator covers group fetches during navigation.
...
The client route table is stubbed in both development and production for behavior parity; the server always uses the full route table, so server-side rendering, prerendering and route rules are unaffected.
...
- Pages with dynamic `path`, `name`, `alias` or `redirect` in `definePageMeta` cannot be stubbed and remain in the initial bundle, as do pages without a name, children or redirect (their stubs would not be registrable by vue-router).
...
- Setting `prefetch: false` on a `<NuxtLink>` also opts it out of eager route discovery; its route group is then only fetched when a navigation targets it.
- `router.getRoutes()` returns lightweight stub records (no components or page meta) for routes that have not been discovered yet. URL matching and route names are always complete.
...
- With `experimental.ssrStreaming`, hydrating a streamed page performs one extra sequential request (the route group chunk), as streamed responses cannot carry per-route preload hints.
