...
Lazily discover routes on navigation ("fog of war"), reducing the initial bundle size for applications with many routes.

When enabled, the client no longer ships the full route table. On first load it knows only the route being rendered plus the routes linked on that page — their matching stubs (path, name, alias, redirect) are inlined into the SSR payload. Every other route is discovered on demand: when a `<NuxtLink>` to it renders or a navigation targets it, the client asks the server for that route's stub, patches it into the router, loads its component chunk and navigates. As a result the initial payload stays flat as the number of routes grows.

```ts twoslash [nuxt.config.ts]
export default defineNuxtConfig({
  experimental: {
    lazyRouteDiscovery: true,
    // or tune how many route records are batched per group chunk (default 10)
    // lazyRouteDiscovery: { groupSize: 20 },
  },
})
```

Undiscovered routes are resolved by a `/__nuxt_routes` server endpoint (holding the full stub table server-side); discovery requests are batched into one call per burst. For static hosting the endpoint is prerendered to `/__nuxt_routes.json`, which the client fetches once as a fallback. The server always uses the full route table, so server-side rendering, prerendering and route rules are unaffected.

There are some things to be aware of:

- This feature requires `experimental.scanPageMeta` (enabled by default) so route matching fields can be statically extracted; without it the flag resolves to `false`.
- Pages with dynamic `path`, `name`, `alias` or `redirect` in `definePageMeta` cannot be stubbed and stay eager (always shipped to the client), as do pages without a name, children or redirect (their stubs would not be registrable by vue-router).
- A custom `routes` function in `router.options` is ignored while this feature is enabled (with a development-mode warning), as the transform would otherwise only apply to the stubbed route records.
- `<NuxtLink discover="none">` opts a link out of render-time discovery — useful on link-heavy pages to keep them out of the payload; the route is discovered on click instead.
- `router.getRoutes()` returns only the routes discovered so far. Routes are added as they are linked to or navigated to.
- Unnamed page routes are assigned a generated route name (for example `_lazy-group-0-1`) so their records can be swapped in atomically.
- Modules that duplicate pages via `pages:extend` should assign unique route names to the copies (as `@nuxtjs/i18n` does), since pages whose `definePageMeta` sets a custom `name` would otherwise collide.
- `__nuxtRouteGroup` is a reserved route meta key.
- With `experimental.ssrStreaming`, links rendered inside a streamed boundary are not inlined and are discovered on the client after hydration.

::read-more{icon="i-simple-icons-github" color="gray" to="https://github.com/nuxt/nuxt/issues/32196" target="_blank"}
See the tracking issue for this feature.
::
