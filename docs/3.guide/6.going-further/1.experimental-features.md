...
::warning
Known bug (fix in progress): the per-route `streaming: false` opt-out is currently not honored — matched routes still stream. As a temporary workaround, disable `experimental.ssrStreaming` globally for the affected deployment.
::

::note
SSR streaming requires `@unhead/vue` v3 for streaming head support. Head tags added during rendering (e.g. from Suspense boundaries) are injected into the stream as they resolve.
::

::warning
SSR streaming is not currently compatible with `componentIslands`. When component islands are enabled, all requests fall back to fully-buffered rendering.
::

::note
If an error occurs during streaming (after the HTTP status is already committed), the error is injected into the payload for client-side error page rendering.
::
