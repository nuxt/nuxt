...
export default defineNitroPlugin((nitro) => {
  nitro.hooks.hook('render:html', (ctx, { event }) => {
    ...
    for (const section of sections) {
      ...
      for (let i = 0; i < ctx[section].length; i++) {
        ctx[section][i] = ctx[section][i].replace(SCRIPT_RE, (_, rest) => `<script nonce="${nonce}"` + rest)
      }
    }
  })
})
