...
Browsers and indexing crawlers (which get buffered HTML via `botRegex`) are unaffected, but non-JavaScript clients outside the bot regex will not see those tags at all. Keep SEO-critical tags (canonical links, JSON-LD, social meta) synchronous, or opt the route out of streaming with `routeRules: { '/path': { streaming: false } }`. In development the streaming handler warns once per route when head tags are demoted to client-side patches, so this never fails silently.
::
