...
When a `<NuxtLink>` is prefetched and the destination route has a `_payload.json`, forward any `<link rel="preload">` hints that the destination set via [`useHead`](/docs/4.x/api/composables/use-head) (or via modules like [`@nuxt/image`](https://image.nuxt.com)'s `<NuxtImg preload>`) into the current document.
...
Requires `experimental.payloadExtraction` to be enabled (the default for prerendered/cached routes).
