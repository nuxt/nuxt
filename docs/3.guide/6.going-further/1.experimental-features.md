...
Setting it to `'vue-onigiri'` opts into an alternative implementation where islands are serialized as a component tree (powered by [vue-onigiri](https://github.com/huang-julien/vue-onigiri)) instead of pre-rendered HTML. This mode requires `@nuxt/vite-builder` and is not available with other bundlers. It also turns on `experimental.viteEnvironmentApi`.

::note
Remote islands (`'local+remote'`, the `remoteIsland` option and the `source` prop of `<NuxtIsland>`) are not available with `'vue-onigiri'`.
::
