...
Enables the experimental [`<NuxtClientFallback>`](/docs/4.x/api/components/v-load-client-fallback) component for rendering content on the client if there's an error in SSR.
