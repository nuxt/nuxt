...
export default defineNuxtConfig({
  experimental: {
    componentIslands: true, // false, 'local+remote' or 'vue-onigiri'
  },
})
```

Setting it to `'vue-onigiri'` opts into an alternative implementation where islands are serialized as a component tree (powered by [vue-onigiri](https://github.com/huang-julien/vue-onigiri)) instead of pre-rendered HTML. This mode requires `@nuxt/vite-builder` and is not available with other bundlers.
