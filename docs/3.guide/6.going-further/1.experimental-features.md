...
::warning
This option is deprecated and will be removed. It is forced on and setting it is ignored.
::
...
The error is now JSON-encoded as a whole, so `error.data` arrives with its original shape and is never stringified.
...
The default is `'client'`. It is forced to `false` when `ssr: false` is set.
...
Loads a client-side navigation that matches no page route as a full document, so the server can respond with a file in `public/`, a server route, or its 404 page. With this enabled, `<NuxtLink to="/brochure.pdf">` and `<NuxtLink to="/rss.xml">` work on click as they do on a hard load.

A path is never reloaded if the current document was already served for it, so hosts that respond to unknown paths with an SPA fallback render the client-side 404 page.
...
Reject a request to a path no server route answers.
...
Nuxt types `$fetch` and `useFetch` from the routes your server will serve, whatever this option is
set to, so a response is typed by the handler that answers it. What the option decides is whether a
path Nuxt does not recognise is an error or is simply typed as `unknown`.
...
- `false` (default) - a path Nuxt does not recognise is accepted and resolves to `unknown`, and no
  ...
  [`routeRules`](/docs/guide/concepts/rendering#hybrid-rendering) proxy or a catch-all handler can answer
  a path no route describes.
...
made through one is checked against the route it actually asks for. Naming the response type also opts a call out, since doing so turns off
...
Read more about typed routes, and about declaring a route Nuxt cannot see.
::
...
number of file watchers active in dev mode and is the default. If the active builder does not implement
...
The `useState.resetOnClear` option controls whether [`clearNuxtState`](/docs/api/utils/clear-nuxt-state) resets state to its initial value (provided by the `init` function of [`useState`](/docs/api/composables/use-state)) instead of setting it to `undefined`. This defaults to `true`.
