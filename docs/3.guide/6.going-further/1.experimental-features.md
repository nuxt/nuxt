...
This option is the master switch. Configuring [`app.viewTransition`](/docs/4.x/api/nuxt-config#viewtransition) or `definePageMeta({ viewTransition })` alone does not register the plugin. See [View Transitions API](/docs/4.x/getting-started/transitions#view-transitions-api-experimental) for defaults, overrides, and known issues.
