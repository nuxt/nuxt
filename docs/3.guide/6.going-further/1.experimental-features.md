...
Internally, Nuxt uses `@nuxt/schema` to define these experimental features. You can refer to the [API documentation](/docs/guide/going-further/experimental-features) or the [source code](https://github.com/nuxt/nuxt/blob/main/packages/schema/src/config/experimental.ts) for more information.
...
- `'client'` - Payload is inlined in HTML for the initial server render, and extracted to `_payload.json` files for client-side navigation.
...
::read-more{to="/docs/getting-started/prerendering#payload-extraction"}
Read more about payload extraction and the consequences of each mode.
::
...
Page routes (including aliases) are compiled into a static route matcher at build time, and incoming requests are checked against it before server-side rendering begins. Requests that cannot match any page skip loading the app entirely and go straight to the error page.

Requests without an `Accept: text/html` header (API clients, most scanners) receive a JSON error without loading the app at all. HTML requests still server-render the error page. A [`cache` route rule](/docs/guide/concepts/rendering#hybrid-rendering) covering the path advertises its `maxAge` as `Cache-Control` on GET/HEAD misses too, letting a CDN absorb repeat requests for unknown paths.
...
::warning
This is opt-in because it can break apps that rely on runtime routing:

- pages added dynamically with `router.addRoute()` in a plugin. To surface this early, `addRoute` is removed from the type of the router returned by [`useRouter`](/docs/api/composables/use-router) when the option is enabled.
- route middleware that redirects unknown paths to existing pages

The option is disabled automatically in development, when `hashMode` is enabled, when a root-level catch-all page (such as `pages/[...slug].vue`) matches every path, and when a custom `app/router.options` file may modify `routes` (a file whose default export is a plain object without a `routes` key remains compatible).
::
...
You can also pass an object to configure [view transition types](/docs/getting-started/transitions#view-transition-types), which allow different CSS animations based on the type of navigation:
...
:read-more{to="/docs/guide/concepts/server-components"}

::note
Skip `nuxt-client` on non-SFC components such as [`<NuxtLink>`](/docs/api/components/nuxt-link). See [selective hydration with `nuxt-client`](/docs/guide/concepts/server-components#selective-hydration-with-nuxt-client).
::
...
This allows modules to access additional metadata from the page metadata in the build context. If you are using this within a module, it's recommended also to [augment the `NuxtPage` types with your keys](/docs/directory-structure/app/pages#typing-custom-metadata).
...
::read-more{icon="i-simple-icons-github" color="gray" to="/docs/directory-structure/app/components#delayed-or-lazy-hydration"}
...
The `useState.resetOnClear` option controls whether [`clearNuxtState`](/docs/api/utils/clear-nuxt-state) resets state to its initial value (provided by the `init` function of [`useState`](/docs/api/composables/use-state)) instead of setting it to `undefined`. This defaults to `true` with `compatibilityVersion: 5`.
...
When a `<NuxtLink>` is prefetched and the destination route has [payload extraction](#payloadextraction) enabled (the default for prerendered and cached routes), forward any `<link rel="preload">` hints that the destination set via [`useHead`](/docs/api/composables/use-head) (or via modules like [`@nuxt/image`](https://image.nuxt.com)'s `<NuxtImg preload>`) into the current document.

Forwarded links are downgraded from `rel="preload"` to `rel="prefetch"`, apart from `as="image"` hints, which keep `rel="preload"` with any `fetchpriority` stripped.

Only user-defined head tags are forwarded; build-time JS/CSS chunk preloads are already handled separately by the prefetch pipeline.
...
Chromium and Safari log a `was preloaded using link preload but not used within a few seconds` warning for each forwarded image hint on a route the user never visits. Forwarded image requests also carry no `Purpose: prefetch` header, so they count as ordinary traffic against image CDN quotas, rate limits and analytics.
...
Type `$fetch` and `useFetch` from the routes your server builder reports, rather than from the
...
The generated route set carries the `body`, `query` and `headers` a handler validates as well as its
response, typed as it arrives over the wire, so a resolving call is typed more tightly than before:

- a `Date` a handler returns is a `string` at the call site
- a `body` the handler validates cannot be omitted or given the wrong shape
- a method the route does not answer is rejected
- `params` is no longer accepted, since it was only ever an alias for `query`
- a route you registered by augmenting `ServerRoutes` by hand needs rewriting, as the two
  descriptions of a route differ (see [the upgrade guide](https://nuxt.com/docs/5.x/getting-started/upgrade#typed-fetch-rebuilt-on-generated-route-types)).
  A route declared with `Endpoint` (imported from `nuxt/app`) in the rewritten shape is typed
  with `routeTypedFetch` both on and off, so you can rewrite it ahead of the switch

Because of those, it is opt-in, and `future.compatibilityVersion: 5` enables it. Setting it
explicitly selects either typing whatever the compatibility version, so you can adopt it before
moving to Nuxt 5, or move to Nuxt 5 with `routeTypedFetch: false` and adopt it separately.

::note
This option exists only while both typings are supported. From Nuxt 5 the generated route set is the
only source and the option is gone.
::

::note
While Nuxt 4 runs on nitro v2, nitro declares a global `$fetch` of its own, typed from
`InternalApi`. Import `$fetch` from `#build/fetch` where you want the generated route types to apply
to a direct call; `useFetch`, `useLazyFetch` and `useRequestFetch()` are unaffected.
::

## strictRouteTypes

Reject a request to a path no server route answers. Requires
[`routeTypedFetch`](#routetypedfetch), which types `$fetch` and `useFetch` from the routes your
server will serve.
...
With `routeTypedFetch` enabled a response is typed by the handler that answers it whatever this
option is set to. What the option decides is whether a path Nuxt does not recognise is an error or
is simply typed as `unknown`.

- `false` (default) - a path Nuxt does not recognise is accepted and resolves to `unknown`, and no
  method is rejected. Nuxt cannot see every way a request may be answered: nitro middleware, a
  [`routeRules`](/docs/guide/concepts/rendering#hybrid-rendering) proxy or a catch-all handler
  can answer a path no route describes.
- `true` - only the routes your server builder reports are accepted, for the methods they answer.
- `'isomorphic'` - as `true`, and your pages are included as `GET` routes returning `string`, so
  `$fetch('/about')` is typed by the route the Vue router serves rather than rejected.

With the option on, a typo and a method the route does not answer are both errors:
...
Two things stay accepted, because they are not Nuxt's to describe: an absolute URL
(`$fetch('https://example.com')`), and a path built at runtime, which resolves to `unknown` as it
does with the option off. A `baseURL` is resolved into the path before it is matched, so a request
made through one is checked against the route it actually asks for. Naming the response type also
opts a call out, since doing so turns off inference for the request:
...
::note
Files in your `public/` directory are not routes your server builder reports, so requesting one
with `strictRouteTypes` enabled is an error. Fetch those with an explicit response type, or leave
the option off.
::

Set this only where your routing is enumerable. An app with a catch-all page under `'isomorphic'`
matches every path, which is correct but means the option constrains nothing.
...
::important
Enabling this feature changes how Vite plugins are registered and configured. See the [Vite Environment API migration guide](https://nuxt.com/docs/5.x/getting-started/upgrade#migration-to-vite-environment-api) for details on updating your plugins.
::
...
Streaming is automatically disabled for bot and crawler user agents (such as Googlebot, Bingbot, etc.) to ensure search engines receive fully-rendered HTML for SEO safety. The default pattern matches indexing crawlers only; Lighthouse and other audit tools deliberately fall outside it so synthetic measurements reflect the same streamed response real users get.

You can customise which user agents are opted out with `botRegex`. A request whose `user-agent` header matches the pattern is **not** streamed: it receives the fully-rendered, buffered HTML. Requests that do not match are streamed. Setting `botRegex` replaces the default pattern rather than adding to it, so list every crawler you want to keep out of streaming:
...
- Bot/crawler user agents, that is, requests whose `user-agent` matches `botRegex`
- Prerendered routes (`nuxt generate`)
...
::warning
**Head tags registered after the shell flushes become client-side patches.** The renderer waits for the first render pass before flushing the shell, so `useHead`/`useSeoMeta` called synchronously in `<script setup>` (before any `await`), in plugins, or in middleware render into the shell `<head>` as usual. Tags registered later (after an `await` in setup, or inside a nested `<Suspense>` boundary) cannot reach the already-sent `<head>`; they are delivered as inline scripts that patch the DOM on the client.

Clients that run JavaScript apply the patch, and clients matching `botRegex` never stream in the first place, so they receive the full buffered `<head>`. Anything else (a crawler your `botRegex` does not list, a link unfurler, `curl`) will not see those tags at all. JSON-LD scripts, `noscript` tags and body-positioned tags are the exception: they are rendered as real markup before `</body>`, where every client can read them. Keep other SEO-critical tags (canonical links, social meta) synchronous, or opt the route out of streaming with `routeRules: { '/path': { streaming: false } }`.
::
