...
The renderer needs three things from the build.

### The Artifacts to Render With

The renderer imports the build outputs above (`nuxt/entry`, `nuxt/manifest`, and the rest), so a
builder that already fulfils the [build output contract](#the-build-output-contract) has them.

A server builder that bundles the app build's output, rather than inlining those module bodies, also
...
### Its Build-Time Configuration

The renderer reads the flags it was built with from `nuxt/renderer-config`, a module the server
builder provides. Everything in it is an inlined constant, so a build folds away the branches it
does not use. `addRendererConfig()` generates it from `nuxt.options` and points the specifier at it:

```ts
import { addRendererConfig } from '@nuxt/kit/internal'

addRendererConfig()
```

Constants a builder resolves for itself (anything derived from route rules, from a page matcher, or
from an asset it emits) default to the value of a build without the feature, and are replaced with
`overrides`, whose values are JS expressions:
...
`getRendererConfig()` returns the same module body without registering anything, for a builder that
serves its own virtual modules (as `@nuxt/nitro-server` does).

### The Defines It Is Compiled With

The renderer and the app runtime branch on `import.meta.*` flags, which a builder must replace at
build time. `getRendererDefines()` returns the set for a phase, so a builder does not re-derive it:

```ts
import { getRendererDefines } from '@nuxt/kit/internal'

const define = getRendererDefines('server') // or 'prerender'
```

Leaving them in place is not merely a size cost: `import.meta.prerender` reaching the runtime as
`undefined` silently disables payload extraction and the prerender caches.

::note
Import `createNuxtRenderer` from `nuxt/renderer` only in the module that is your server entry.
Anything the entry can also reach (an island handler, a route-rule helper) should import the pieces
it needs from the granular `nuxt/renderer/*` subpaths instead: pulling the barrel into a module the
entry shares produces a chunk cycle, which some bundlers resolve into a broken entry namespace.
::

::warning
`nuxt/renderer`, `nuxt/renderer-config`, `serverBuild.input`, and the renderer helpers in
