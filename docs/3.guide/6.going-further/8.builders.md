...
A **builder** is the part of Nuxt responsible for bundling your application. Nuxt ships with three official builders, [Vite](https://vite.dev) (the default), [webpack](https://webpack.js.org) and [Rspack](https://rspack.dev), and you can select one with the [`builder`](/docs/api/nuxt-config#builder) option or supply your own.

This guide explains how builders fit into the Nuxt build, what contract a builder must fulfil, and how to author one.

::note
Authoring a builder is an advanced topic. Most apps never need a custom builder; if you only want to influence the bundle, a [module](/docs/guide/modules) that registers bundler plugins is usually the right tool.
::
...
The Nuxt core (the `nuxt` context described in [How Nuxt Works](/docs/guide/going-further/internals)) and your modules generate the virtual application: the entrypoints, route table, plugins, component registry, and the rest of the [virtual file system](/docs/guide/going-further/internals#the-nuxt-interface) under `#build`. The builder takes that virtual application and turns it into real JavaScript and CSS bundles, and in development runs a dev server that serves and hot-reloads them.
...
Nuxt resolves the active builder from the [`builder`](/docs/api/nuxt-config#builder) option. It accepts either a module specifier that default-exports a `NuxtBuilder`, or an inline object:
...
A builder communicates with the rest of Nuxt almost entirely through [hooks](/docs/api/advanced/hooks). It reads build configuration off `nuxt.options` and, when appropriate, lets modules extend its bundler configuration.

### Letting Modules Extend the Bundler

Modules influence the bundle through [Nuxt Kit](/docs/guide/going-further/kit) helpers. A builder should honour the relevant ones:

- [`addVitePlugin`](/docs/api/kit/builder#addviteplugin) / [`addWebpackPlugin`](/docs/api/kit/builder#addwebpackplugin) register bundler-specific plugins.
...
- [`extendViteConfig`](/docs/api/kit/builder#extendviteconfig) / [`extendWebpackConfig`](/docs/api/kit/builder#extendwebpackconfig) mutate the resolved bundler config.
...
`setBuildOutput` writes to `nuxt.buildOutputs[key]`. From inside a bundler plugin that already holds the `nuxt` instance you can assign `nuxt.buildOutputs[key]` directly; `setBuildOutput` is the convenience for code that resolves `nuxt` via [`useNuxt()`](/docs/api/kit/context#usenuxt).
