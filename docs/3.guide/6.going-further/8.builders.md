...
and a context), matching the `RequestEvent` shape in `@nuxt/schema`. Set `~app` on it to name the
event the application should see, and `useRequestEvent()` and the render hooks receive that instead.
`runtimeConfig` is passed the event too, for a runtime that resolves it per request. Everything the
renderer cannot
...
surface](/docs/5.x/guide/going-further/server-imports) that application and module server code
...
ships. Those are written over the `RequestEvent` shape, and are what `@nuxt/vite-server`
runs on.

The event you hand a handler has to satisfy `RequestEvent`. Contribute its type through the
`ServerTypes` registry in `@nuxt/schema`: that is what `NuxtRequestEvent` and `useRequestEvent()`
resolve to, and what your own implementations of the surface take.

If your own event is not web-shaped, hand handlers a web-shaped view of it and name your event in
`~app`. `toNuxtRequestEvent()` returns it.
