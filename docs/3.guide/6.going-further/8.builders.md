...
- An **app builder** bundles your application: the browser bundle and, when SSR is enabled, the SSR app entry. Nuxt ships three official app builders, [Vite](https://vite.dev) (the default), [webpack](https://webpack.js.org) and [Rspack](https://rspack.dev). You select one with the [`builder`](/docs/api/nuxt-config#builder) option, or supply your own. Throughout the rest of this guide, "builder" on its own means an app builder.
...
This guide explains how each role fits into the Nuxt build, what contract each fulfills, and how to author one. It assumes you're familiar with the [Nuxt interface](/docs/guide/going-further/internals) and with writing [modules](/docs/guide/modules).
...
Nuxt core and your modules generate the virtual application: the entrypoints, route table, plugins, component registry, and the rest of the [virtual file system](/docs/guide/going-further/internals#the-nuxt-interface) under `#build`. The builder turns that virtual application into real JavaScript and CSS bundles, and in development runs a dev server that serves and hot-reloads them.
...
The [`builder`](/docs/api/nuxt-config#builder) option accepts either a module specifier that default-exports a `NuxtBuilder`, or an inline object:
...
When you run [`nuxt build`](/docs/api/commands/build) or [`nuxt dev`](/docs/api/commands/dev), Nuxt:
...
An app builder communicates with the rest of Nuxt almost entirely through [hooks](/docs/api/advanced/hooks). It reads build configuration from `nuxt.options` and, where appropriate, lets modules extend its bundler configuration.

### Letting Modules Extend the Bundler

Modules influence the bundle through [Nuxt Kit](/docs/guide/going-further/kit) helpers, and an app builder honors the relevant ones:

- [`addVitePlugin`](/docs/api/kit/builder#addviteplugin) and [`addWebpackPlugin`](/docs/api/kit/builder#addwebpackplugin) register bundler-specific plugins.
- [`addBuildPlugin`](/docs/api/kit/builder#addbuildplugin) registers an [unplugin](https://github.com/unjs/unplugin) factory, so a single plugin works across every builder.
- [`extendViteConfig`](/docs/api/kit/builder#extendviteconfig) and [`extendWebpackConfig`](/docs/api/kit/builder#extendwebpackconfig) mutate the resolved bundler config.
...
Use the [`setBuildOutput`](/docs/api/kit/builder#setbuildoutput) helper from `@nuxt/kit`:
...
`setBuildOutput` writes to `nuxt.buildOutputs[key]`, resolving the `nuxt` instance through [`useNuxt()`](/docs/api/kit/context#usenuxt). Inside a bundler plugin that already holds the instance, you can assign `nuxt.buildOutputs[key]` directly.
...
The remaining sections cover the other role: taking the app build and standing up a server that renders with it. A server builder is not selected with the [`builder`](/docs/api/nuxt-config#builder) option; it's a module that registers the renderer's modules with its own bundler and describes the build it produced. If you're writing an app builder, you're done at this point.
...
One of the modules in the list is `nuxt/server`, the set of [server imports](/docs/guide/going-further/server-imports) that app and module code uses on the server. The types always come from the `nuxt` package, but the implementation may come from whichever server builder is running.
...
::read-more{to="/docs/guide/going-further/internals"}
Learn more about the Nuxt interface and the build vs. runtime split.
::

::read-more{to="/docs/api/kit/builder"}
