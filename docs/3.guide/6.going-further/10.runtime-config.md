...
Nuxt provides a private `runtimeConfig.app.secret` as the application's root secret. Modules and server features can use it to derive their own secrets. The value defaults to an empty string, is only available on the server, and you can set it with `NUXT_APP_SECRET` without declaring the key in `nuxt.config`.
...
- On client-side, only keys in `runtimeConfig.public` and `runtimeConfig.app` (which is used by Nuxt internally, and excludes `runtimeConfig.app.secret`) are available, and the object is both writable and reactive.
