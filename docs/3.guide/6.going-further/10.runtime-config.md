...
Nuxt provides a private `runtimeConfig.secretKey` as the application's root secret. Modules and server features can use it to derive their own secrets. The value defaults to an empty string, and you can set it with `NUXT_SECRET_KEY` without declaring the key in `nuxt.config`.
...
Nuxt does not require `NUXT_SECRET_KEY` unless a module or server feature uses it.
