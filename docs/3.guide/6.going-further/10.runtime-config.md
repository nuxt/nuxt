...
Nuxt includes a private `runtimeConfig.secretKey` that modules and server features can use as a project-level root for purpose-specific secrets. Set it with `NUXT_SECRET_KEY`; unlike custom runtime config, you do not need to declare this key in `nuxt.config`.

Generate a cryptographically random value of at least 32 bytes with `openssl rand -base64 32`. The value must remain stable across builds and server instances; changing it invalidates dependent sessions and signatures. Nuxt does not require it unless a module or server feature uses it.
