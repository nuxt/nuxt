...
Set `compatibility.nuxt` in your [nuxt/modules](https://github.com/nuxt/modules) registry entry to a [semver](https://semver.org/) range of supported Nuxt majors (for example `^3.0.0`, `^4.0.0`, or `^3 || ^4`). The [modules catalog](/modules) reads that range for Nuxt 3 / Nuxt 4 badges and for the version filter (`?version=3`, `?version=4`, or `?version=all`).
