...
- "local" modules exist within a Nuxt project, either [inlined in Nuxt config](/docs/api/nuxt-config#modules) or within [the `modules` directory](/docs/directory-structure/modules).
...
You can get type hinting for this function using the higher-level `defineNuxtModule` helper provided by [Nuxt Kit](/docs/guide/going-further/kit).
...
- Vue components
- Composables
- [Nuxt plugins](/docs/directory-structure/app/plugins)
...
::tip
Learn more about asset injection in [the recipes section](/docs/guide/modules/recipes-basics).
::
