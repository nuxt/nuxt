...
When Nitro starts, it initializes and executes the plugins under the [`/server/plugins`](/docs/directory-structure/server#server-plugins) directory. These plugins can:
...
:read-more{to="/docs/directory-structure/server#server-plugins"}
...
:read-more{to="/docs/directory-structure/server#server-middleware"}
...
Plugins execute in a specific order and may have dependencies on one another. For more details, including execution order and parallelism, refer to the [Plugins documentation](/docs/directory-structure/app/plugins).
...
:read-more{to="/docs/directory-structure/app/plugins"}
...
For details about each type and examples, see the [Middleware documentation](/docs/directory-structure/app/middleware).
...
:read-more{to="/docs/directory-structure/app/middleware#when-middleware-runs"}

### Mount Vue app and hydrate

Calling `app.mount('#__nuxt')` mounts the Vue application to the DOM. If the application uses SSR or SSG mode, Vue performs a hydration step to make the client-side application interactive. During hydration, Vue recreates the application (excluding [Server Components](/docs/directory-structure/app/components#server-components)), matches each component to its corresponding DOM nodes, and attaches DOM event listeners.
