...
Plugins execute in a specific order and may have dependencies on one another. For more details, including execution order and parallelism, refer to the [Plugins documentation](/docs/directory-structure/plugins).
...
:read-more{to="/docs/directory-structure/plugins"}
...
For details about each type and examples, see the [Middleware documentation](/docs/directory-structure/middleware).
...
:read-more{to="/docs/directory-structure/middleware#when-middleware-runs"}

### Mount Vue app and hydrate

Calling `app.mount('#__nuxt')` mounts the Vue application to the DOM. If the application uses SSR or SSG mode, Vue performs a hydration step to make the client-side application interactive. During hydration, Vue recreates the application (excluding [Server Components](/docs/directory-structure/components#server-components)), matches each component to its corresponding DOM nodes, and attaches DOM event listeners.
