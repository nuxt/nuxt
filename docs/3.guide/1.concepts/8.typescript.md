...
Set `typeCheck` to `'build'` to type-check only when building, or `'dev'` to type-check only in the dev server (for example, so CI can run `nuxt typecheck` separately from `nuxt build`).

## Auto-generated Types

Nuxt projects rely on auto-generated types to work properly. These types are stored in the [`.nuxt`](/docs/directory-structure/nuxt) directory and are generated when you run the dev server or build your application. You can also generate these files manually by running `nuxt prepare`.

The generated `tsconfig.json` files inside the [`.nuxt`](/docs/directory-structure/nuxt) directory include **recommended basic TypeScript configuration** for your project, references to [auto-imports](/docs/guide/concepts/auto-imports), [API route types](/docs/guide/concepts/server-engine#typed-api-routes), path aliases like `#imports`, `~/file`, or `#build/file`, and more.

::warning
Nuxt relies on this configuration, and [Nuxt modules](/docs/guide/modules) can extend it as well. For this reason, it is not recommended to modify your `tsconfig.json` file directly, as doing so could overwrite important settings. Instead, extend it via `nuxt.config.ts`. [Learn more about extending the configuration here](/docs/directory-structure/tsconfig).
::
...
::read-more{to="/docs/guide/modules/recipes-advanced#extend-typescript-config"}
...
[Strict checks](https://www.typescriptlang.org/docs/handbook/migrating-from-javascript.html#getting-stricter-checks) are enabled by default in Nuxt when the [`typescript.typeCheck`](/docs/guide/concepts/typescript#type-checking) option is enabled to give you greater type safety.
