...
::note
Routes using `isr` or `swr` also generate `_payload.json` files alongside HTML. This enables client-side navigation to use cached payloads instead of re-fetching data.
::
