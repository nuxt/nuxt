...
::tip
For more examples about writing Vue code without hydration mismatch, see [the Vue docs](https://vuejs.org/guide/scaling-up/ssr#hydration-mismatch), the [Nuxt and Hydration](/docs/4.x/guide/best-practices/hydration) guide, and [Server and Client](/docs/4.x/guide/concepts/server-and-client).
::
...
::note
Routes using `isr` or `swr` also generate `_payload.json` files alongside HTML. Client-side navigation loads these cached payloads instead of re-fetching data. Read more about [payload extraction](/docs/4.x/getting-started/prerendering#payload-extraction).
::
