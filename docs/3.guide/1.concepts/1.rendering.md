...
::alert{type="warning"}
**Route Rules Precedence:** When multiple route rules could match a given URL, broad catch-all patterns like `'/*'` will override more specific patterns. To ensure specific routes use their intended rules, either avoid catch-all patterns or place your specific rules (like `/*/search`) before any catch-all rules in your configuration.
::
