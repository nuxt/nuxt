...
::alert{type="warning"}
**Route Rules Order:** When multiple route rules could match a given URL, broad catch-all patterns like `'/*'` will override more specific patterns if disclosed before. To ensure specific routes use their intended rules, ensure you place any catch-all patterns rules after specific rules in your configuration.
::
