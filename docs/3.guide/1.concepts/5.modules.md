...
Once you have installed the modules you can add them to your [`nuxt.config.ts`](/docs/directory-structure/nuxt-config) file under the `modules` property. Module developers usually provide additional steps and details for usage.
...
## Disabling Modules :badge[v4.3]{color="info" size="xs" class="align-middle"}
...
:read-more{to="/docs/guide/going-further/layers#disabling-modules-from-layers"}

## Create a Nuxt Module

Everyone has the opportunity to develop modules and we cannot wait to see what you will build.

:read-more{to="/docs/guide/modules" title="Module Author Guide"}
