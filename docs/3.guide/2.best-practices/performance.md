...
:read-more{title="Lazy loading components" to="/docs/directory-structure/app/components#dynamic-imports"}
...
:read-more{title="Lazy hydration" to="/docs/directory-structure/app/components#delayed-or-lazy-hydration"}

For content and marketing sites that need little or no client-side interactivity, you can go further and combine prerendering, the `noScripts` route rule, [server components](/docs/guide/concepts/server-components) and lazy hydration to ship near-zero JavaScript.

:read-more{title="Mostly-static sites" to="/docs/guide/recipes/mostly-static-sites"}
