...
making fetch requests in server routes. You can use `$fetch` with headers forwarded from `getHeaders(event)` to forward the request context and headers when making fetch requests in server routes.

```ts [server/api/forward.ts]
export default defineEventHandler(async (event) => {
  return $fetch('/api/forwarded', {
