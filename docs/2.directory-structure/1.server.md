...
::important
Do not import Vue app code (e.g. from `app/`, components, composables, or app-only utilities) in your server routes or server utilities. The server runs in a different runtime and has no Nuxt/Vue app context. Similarly, do not import server-only code (e.g. from `server/` or Nitro utilities) in your app—it can break the client build or leak server logic into the client bundle. For the full reasons and where to put shared code and types, see [Why you cannot mix Vue and Nitro code](/docs/4.x/directory-structure/shared#why-you-cannot-mix-vue-and-nitro-code) in the `shared/` directory docs.
::
