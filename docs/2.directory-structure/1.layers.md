...
Auto-registration is recursive: a `layers/` directory inside a layer you extend (a local directory, an npm package or a remote layer), or inside an auto-registered layer, is registered as well.
...
- [`nuxt.config.ts`](/docs/directory-structure/nuxt-config) - Layer-specific configuration that will be merged with the main config
- [`app.config.ts`](/docs/directory-structure/app/app-config) - Reactive application configuration
- [`app/components/`](/docs/directory-structure/app/components) - Vue components (auto-imported)
- [`app/composables/`](/docs/directory-structure/app/composables) - Vue composables (auto-imported)
- [`app/utils/`](/docs/directory-structure/app/utils) - Utility functions (auto-imported)
- [`app/pages/`](/docs/directory-structure/app/pages) - Application pages
- [`app/layouts/`](/docs/directory-structure/app/layouts) - Application layouts
- [`app/middleware/`](/docs/directory-structure/app/middleware) - Route middleware
- [`app/plugins/`](/docs/directory-structure/app/plugins) - Nuxt plugins
- [`server/`](/docs/directory-structure/server) - Server routes, middleware, and utilities
- [`shared/`](/docs/directory-structure/shared) - Shared code between app and server
...
Alternatively, reference the directories in [`extends`](/docs/api/nuxt-config#extends) (e.g. `extends: ['~~/layers/admin', '~~/layers/base']`) to order them from `nuxt.config` without renaming, with the first entry taking the highest priority.

:read-more{to="/docs/getting-started/layers#layer-priority"}
