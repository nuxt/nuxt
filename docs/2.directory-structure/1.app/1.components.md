...
There are a number of components that Nuxt provides, including `<ClientOnly>`, `<ServerOnly>` (implemented as a server component), and `<DevOnly>`. You can read more about them in the API documentation.
