...
... then the component's name will skip the grouping directory, resulting in:
...
If you want to auto-import components from an npm package, you can use [`addComponent`](/docs/api/kit/components#addcomponent) in a [local module](/docs/directory-structure/modules) to register them.
...
Server components can either be used on their own or paired with a [client component](/docs/directory-structure/app/components#paired-with-a-client-component).
