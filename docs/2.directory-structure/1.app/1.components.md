...
You can partially hydrate a component by setting a `v-load-client` attribute on the component you wish to be loaded client-side.
...
When rendering a server-only or island component, `<NuxtIsland>` makes a fetch request which comes back with a `NuxtIslandResponse` containing the island's rendered HTML and head entries (or, with `experimental.componentIslands: 'vue-onigiri'`, its serialized component tree). (This is an internal request if rendered on the server, or a request that you can see in the network tab if it's rendering on client-side navigation.)
