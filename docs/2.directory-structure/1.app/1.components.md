...
Sometimes you want to group components by concern on disk — for example `(auth)`, `(admin)`, `_internal` — without those folder names leaking into generated component names. This mirrors the [route groups](/docs/4.x/guide/directory-structure/pages#route-groups) convention in the `pages/` directory, but for components.

Use the `ignoreDirPatterns` option to opt in. Folder names matching one of the recognized patterns are stripped from the component name; other path segments are still used as prefixes.

```ts twoslash [nuxt.config.ts]
export default defineNuxtConfig({
  components: {
    dirs: ['~/components'],
    ignoreDirPatterns: true, // enables both recognized patterns
  },
})
```
...
| File on disk | Component name |
| --- | --- |
| `(auth)/forms/LoginForm.vue` | `<FormsLoginForm />` |
| `(apps)/(admin)/AdminDashboard.vue` | `<AdminDashboard />` |
| `_internal/widgets/CounterWidget.vue` | `<WidgetsCounterWidget />` |
| `regular/PlainBlock.vue` | `<RegularPlainBlock />` |

The matched segments (`(auth)`, `(apps)`, `(admin)`, `_internal`) are removed; unmatched segments (`forms`, `widgets`, `regular`) are preserved.
...
| --- | --- | --- |
| **Wrap** — parentheses | `(auth)`, `(admin)` | Mirrors Nuxt's pages route-group syntax. |
| **Prefix** — underscore | `_internal`, `_shared` | The de-facto "private folder" convention across the JS and Python ecosystems. |

`ignoreDirPatterns: true` enables both. You can also opt in per pattern by passing an object:
...
export default defineNuxtConfig({
  components: {
    dirs: ['~/components'],
    ignoreDirPatterns: {
      wrap: true, // only strip (parenthesized) folders
      prefix: false, // leave _underscore-prefixed folders in the component name
    },
  },
})
```
...
::note
The recognized patterns are fixed for this release. Additional patterns may be added in future releases — anything opened up in the future will default to off so existing apps remain unaffected.
::

::tip
Use `wrap: ['()']` when you want the same style as Nuxt's `pages/` route groups. Use `prefix: ['_']` if you prefer Unix-style "private" folders. You can combine both.
::
