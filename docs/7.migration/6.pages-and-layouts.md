...
In Nuxt 2, you could define any nested routes (with parent and child components) using `<Nuxt>` and `<NuxtChild>`. In Nuxt 3, they are replaced with a single `<NuxtPage>` component.
