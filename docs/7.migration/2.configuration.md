...
Nuxt 3 is an [ESM native framework](/docs/guide/concepts/esm). Although [`unjs/jiti`](https://github.com/unjs/jiti) provides semi-compatibility when loading the `nuxt.config` file, avoid any usage of `require` and `module.exports` in this file.
...
To make Nuxt loading behavior more predictable the async config syntax is now deprecated. Consider using Nuxt hooks for async operations.
...
The `static/` directory (for static assets) is now `public/`. You can either rename your `static` directory to `public` or keep the name by setting `dir.public` in your `nuxt.config`.
...
It will be much easier to migrate your application using Nuxt's TypeScript integration. This does not mean you need to write your application in TypeScript, just that Nuxt will provide automatic type hints for your editor.
...
There are several changes to what is recommended in Vue's best practice, along with several breaking changes between Vue 2 and 3.

Please read the [Vue 3 migration guide](https://v3-migration.vuejs.org) and particularly the [breaking changes list](https://v3-migration.vuejs.org/breaking-changes).
...
Enable the module in your Nuxt configuration file:
