...
1. In your `nuxt.config`, rename `head` to `meta` and consider moving this shared meta configuration into your `app.vue` instead. (Note that objects no longer have a `hid` key for deduplication.)
1. If you need to access the component state with `head`, you should migrate to `useHead`. You might also consider using the built-in meta-components.
1. If you need to use the Options API, there is a `head()` method you can use when you use `defineNuxtComponent`.

### Example: The `useHead` Function
...
// This will be reactive even if you change the title/description above
useHead({
  title,
  meta: [{
    name: 'description',
...
### Example: The built-in Meta-components
...
### Example: The Options API

```vue [Nuxt 3 (Options API)]
<script>
//If you use the options API `head` method, you must use `defineNuxtComponent`
