...
use with the computed getter syntax, the same as `useHead`.
...
The composable itself is a ~2kB, so you may consider only using it on the server and having it be treeshaken from the client bundle.
