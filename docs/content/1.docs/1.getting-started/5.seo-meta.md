...
You can use `useHead` in your `setup` function to manage your head tags.
...
The `useSeoMeta` composable is a wrapper around `useHead`, allowing you to define fully-typed meta tags in a flat object. 

This composable is removed from your production client bundle, so you can use it without worrying about bundle size. This
also means that the data will not be reactive between pages when navigating client side.
