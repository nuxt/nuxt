...
You can get a file in the `public/` directory from your application's code or from a browser by the root URL `/`. The `static/` directory is replaced by the `public/` directory for storing static assets. 
