...
When using server-side rendering, calling `$fetch` in an isomorphic code (which is executed on both server and client side) will fetch the data twice (initially on the server and again on the client during the hydration phase). `$fetch` will *not serialize* and *not transfer* the data to the client side (via the payload) and the data will be fetched again on client side.
...
If fetching twice isn't your intended behavior in an isomorphic code, to fetch only on the server side and transfer it to the client, wrap `$fetch` with `useAsyncData()` or use `useFetch()`.
