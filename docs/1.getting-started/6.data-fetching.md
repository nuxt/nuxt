...
::Alert
This section is not released yet. You can beta test using [Edge Channel](https://nuxt.com/docs/guide/going-further/edge-channel).
::
...
```vue [app.vue]
<script setup lang="ts">
// Type of `data` is infered as string even though we returned a Date object
...
Unfortunately, Nuxt does not currently support an alternative serializer to `JSON.stringify`. However, you can return your payload as a normal string and utilize the `toJSON` method to maintain type safety.
...
// `date` is infered as { createdAt: Date } and you can safely use the Date object methods
