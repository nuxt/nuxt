...
Nuxt is a framework which can run isomorphic, or universal code, in both server and client environments. If a normal `fetch` call is used with javascript, data will be fetched twice, once on the server and once on the client. This is why Nuxt offers specific data fetching composables so data is fetched once on the server, and not a second time on the client.
