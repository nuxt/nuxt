...
If you are dealing with a complex data structure and you are only interested in a few properties, simply make use of the `pick` option to only pick specific properties you are interested in. By using the `pick` option you will also stick to [Best Practices](#best-practices) as described in the [Minimize Payload](#minimize-payload) section, reducing the amount of properties being added to the page payload.
