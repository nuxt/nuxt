...
When using server-side rendering, calling `$fetch` in code that is executed on both server and client side will fetch the data twice (initially on the server and again on the client during the hydration phase). `$fetch` will not serialize or transfer the data to the client side, so the data will be fetched again on client side.
