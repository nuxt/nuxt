...
When using server-side rendering, calling `$fetch` in an isomorphic code (which is executed both on the server side and on the client side) will fetch the data twice (initially on the server, then again on the client). `$fetch` will *not serialize* and *not transfer* the data to the client side and the data will be fetched again on the client side.
