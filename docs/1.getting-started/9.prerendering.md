...
export default defineNuxtConfig({
  routeRules: {
    ...
    "/this-DOES-NOT-get-prerendered": { prerender: true },
    // Everything under /blog gets prerendered
    "/blog/**": { prerender: true },
  },
});
```
...
As a shorthand, can also configure this in a page file using [`defineRouteRules`](/docs/api/utils/define-route-rules).
