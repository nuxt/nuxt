...
When supported computed options are provided, they'll be automatically watched and trigger refetches. In some cases, it can be useful to opt-out of this behavior by specifying `watch: false`.
...
const { data, execute } = await useFetch('/api/users', {
  query: { id }, // This is reactive but won't trigger refetch
