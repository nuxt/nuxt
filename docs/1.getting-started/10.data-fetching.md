...
```vue
<script setup lang="ts">
const { data: discounts, status } = await useAsyncData('cart-discount', async (_, { signal }) => {
...
const { data: users1 } = useAsyncData('users-1', (_, { signal }) => $fetch('/api/users', { signal }))
const { data: users2 } = useAsyncData('users-2', (_, { signal }) => $fetch('/api/users', { signal }))
```
...
```ts
const { data } = await useAsyncData((_, { signal }) => {
  return Promise.all([
