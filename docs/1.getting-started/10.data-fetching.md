...
The policy controls *whether* the cache is consulted and updated. It composes with [`getCachedData`](/docs/4.x/api/composables/use-async-data#parameters), which controls *how* cached data is read.

::note
During first-load hydration, the server-rendered payload is always adopted, whatever the policy — it is how server data is transferred to the client, not a cache. With `cache-and-network`, the payload is additionally revalidated in the background once hydration completes.
::
