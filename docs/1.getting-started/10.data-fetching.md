...
### A note on using await

Examples in this documentation usually `await` calls to `useFetch` and `useAsyncData`. However, this is not generally required; what you need depends on the UX you want. Awaiting will lead to blocking renders and longer loading times. The advantage is that the user sees a full page without any loading spinners once it finally renders.

Not awaiting will lead to immediate navigation. Data collection will still happen, and Nuxt ensures the promise is awaited internally. The result is instant navigation, but you will need to handle loading states yourself.
