...
```vue
<script setup lang="ts">
const { data: discounts, status } = await useAsyncData('cart-discount', async ({ signal }) => {
...
const { data: users1 } = useAsyncData('users-1', ({ signal }) => $fetch('/api/users', { signal }))
const { data: users2 } = useAsyncData('users-2', ({ signal }) => $fetch('/api/users', { signal }))
```
