...
`$fetch` and [`useFetch`](/docs/4.x/api/composables/use-fetch) are typed from the routes known to your server, so a
...
[`routeRules`](/docs/4.x/guide/concepts/rendering#hybrid-rendering) proxy or a catch-all handler can answer it.
Enable [`experimental.strictRouteTypes`](/docs/4.x/guide/going-further/experimental-features#strictroutetypes)
...
::note
Modules that register handlers with [`addServerHandler`](/docs/4.x/api/kit/nitro#addserverhandler)
...
[`createUseFetch`](/docs/4.x/guide/recipes/custom-usefetch#custom-usefetch-with-createusefetch),
