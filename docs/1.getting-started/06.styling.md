...
If you're writing local stylesheets, the natural place to put them is the [`app/assets/` directory](/docs/directory-structure/app/assets).
...
The natural place for your stylesheets is the [`app/assets/` directory](/docs/directory-structure/app/assets). You can then reference its path and Nuxt will include it to all the pages of your application.
...
With the default Vite builder, no PostCSS plugins are configured out of the box. Vite resolves `@import` and `url()` itself, and minifies CSS as part of the build.

[autoprefixer](https://github.com/postcss/autoprefixer) and [cssnano](https://cssnano.github.io/cssnano/) are optional peer dependencies. To use either, install it and name it in `postcss.plugins`:
...
With the webpack builder, [postcss-import](https://github.com/postcss/postcss-import), [postcss-url](https://github.com/postcss/postcss-url), `autoprefixer` and `cssnano` are still configured for you.
...
:read-more{to="/docs/directory-structure/app/layouts"}
...
Nuxt modules provide you with a good developer experience out of the box, but remember that if your favorite tool doesn't have a module, it doesn't mean that you can't use it with Nuxt! You can configure it yourself for your own project. Depending on the tool, you might need to use a [Nuxt plugin](/docs/directory-structure/app/plugins) and/or [make your own module](/docs/guide/modules). Share them with the [community](/modules) if you do!
