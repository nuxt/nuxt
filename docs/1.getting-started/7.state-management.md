...
::callout
Make sure to install the Pinia module with `npx nuxi@latest module add pinia` or follow the module's installation steps.
::

::code-group
```ts [stores/website.ts]
export const useWebsiteStore = defineStore('counter', {
  state: () => ({
    name: '',
    description: ''
  }),
