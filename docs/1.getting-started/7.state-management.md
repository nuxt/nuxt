...
Nuxt provides the `useState` composable to create a shared, reactive, and SSR-friendly state across components.
...
::alert{icon=👉}
`useState` only works within component's `setup` function or [`lifecycle hooks`](https://vuejs.org/api/composition-api-lifecycle.html#composition-api-lifecycle-hooks).
::
::alert{type=warning}
...
The state will be shared across all users visiting your website and can lead to memory leaks!
::
...
We can define global type-safe states and import them across the app using [auto-imported composables](/docs/guide/directory-structure/composables).
