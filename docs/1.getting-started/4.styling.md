...
::alert{type=warning}
Due to the way browser works, importing in components will lead to better perfomances when it comes to [LCP](https://web.dev/lcp/).
::

You should use the CSS property if you explicitly want to include your style as an external stylesheet.
...
Then you can reference it direclty in your pages, layouts and components:
...
### LCP Avanced optimizations

If you're using the `css` property or external CSS files, you can do the following to speed-up the download of your global CSS files.
...
If you're not using the `css` property, and your modules are not using it, you can completely remove the `entry.css` from the build process.
