...
Alternatively, you can control the order of local layers from your `nuxt.config` using `app.layerOrdering`, without renaming your directories. Layers are listed from **lowest to highest priority** (the last entry wins), matching the numbered-directory convention:
...
export default defineNuxtConfig({
  app: {
    layerOrdering: [
      ...
      'auth',
      'orders',
      'shopping-cart', // highest priority (overrides the others)
    ],
  },
})
```

`app.layerOrdering` takes precedence over directory numbering. Any local layer not listed keeps its default alphabetical order at the lowest priority.
