...
The Nuxt integration can be enabled with the `experimental.viewTransition` option in your configuration file. This is the **master switch**: Nuxt only registers the View Transitions router plugin when this option is enabled.
...
After you enable `experimental.viewTransition`, view transitions are on for all [pages](/docs/4.x/directory-structure/app/pages) by default. Use `app.viewTransition` or [`definePageMeta`](/docs/4.x/api/utils/define-page-meta) only to change those defaults or overrides.
...
::warning
Setting `app.viewTransition` (or `definePageMeta({ viewTransition })`) without `experimental.viewTransition` does not load Nuxt's View Transitions plugin, so the integration will not run (including shared-element transitions that use `view-transition-name`).
::
...
### View Transition Types :badge[v4.4]{color="info" size="xs" class="align-middle"}
...
You can set default types globally in your `nuxt.config.ts` (still requires `experimental.viewTransition`):
...
- Enable `experimental.viewTransition` first. `app.viewTransition` alone does not activate the integration.
- For shared-element transitions with matching `view-transition-name` values, the browser captures the old element before navigation updates the DOM, then captures the new element after the destination page has rendered. The two do not need to exist in the DOM at the same time; the destination element must be present by that second capture. Avoid [`useLazyAsyncData`](/docs/4.x/api/composables/use-lazy-async-data) (or `lazy: true`) on the destination page when you need that match: navigation can finish before the named element exists. Blocking with `await` [`useAsyncData`](/docs/4.x/api/composables/use-async-data) or [`useFetch`](/docs/4.x/api/composables/use-fetch) is the right starting point, but it does not guarantee a visible shared-element transition with the current experimental integration.
- View Transitions freeze DOM updates while a transition runs. Long-running setup or data fetching during that window can make the UI feel stuck. Treat this carefully if your pages fetch a lot of data in `<script setup>`.
