...
Reactivity supports all properties, such as `computed`, `getters`, and `reactive`.
...
The `titleTemplate` can be a string with `%s` (which will later get replaced with the title) or a function.
