...
As with all composables, `useHead` must be used within a component's `setup` and lifecycle hooks.
...
That helps you avoid typos and common mistakes, such as using `name` instead of `property`.
...
Reactivity is supported on all properties via `computed`, `getters`, and `reactive`.
...
You can use the `titleTemplate` option to provide a dynamic template for customizing the title of your site, for example, adding the name of your site to the title of every page.

The `titleTemplate` can be a string with `%s` (which will get replaced with the title) or a function.

If you want to use a function (for full control), then this cannot be set in your `nuxt.config`, and we recommend setting it within your `app.vue` file, where it will apply to all pages on your site:
...
For example, you can first set the current page title (extracted at build time via a macro, and therefore dynamic setting will not work):
...
`nuxt.config` is also used as an alternative way of setting the page title. However, `nuxt.config` does not allow the page title to be dynamic. Therefore, we recommend using `titleTemplate` in the `app.vue` file to add a dynamic title, which then applies to all routes of your Nuxt app.
