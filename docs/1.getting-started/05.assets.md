...
When an `src` is a static string literal in your template, Nuxt rewrites it at build time. This is how a public asset picks up your [`app.baseURL`](/docs/4.x/api/nuxt-config#baseurl): the compiler sees the literal path and prefixes it, and a bundled asset is turned into an import and replaced with its hashed output path.

```vue
<template>
  <!-- Rewritten at build time: gets app.baseURL, and the bundled file is hashed. -->
  <img src="/img/nuxt.png">
  <img src="~/assets/img/nuxt.png">
</template>
```

A bound `:src` whose value is assembled at runtime is opaque to the compiler, so none of that rewriting happens. The string is used exactly as written:
