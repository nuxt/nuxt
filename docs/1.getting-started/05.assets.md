...
Bundlers need to discover asset paths at build time. A dynamic template URL is not an import, so Vite leaves this path unchanged because `name` is only known at runtime:

```vue
<template>
  <!-- This does not work because the path is created at runtime. -->
