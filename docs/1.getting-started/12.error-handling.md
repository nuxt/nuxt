...
Nuxt assigns a unique error code to every error it throws, both at build time and at runtime. Error codes follow the format `NUXT_<CODE>`, where the code indicates the category:

- **`B` prefix** (e.g., `NUXT_B1001`): Build-time errors (module issues, template compilation, config problems)
- **`E` prefix** (e.g., `NUXT_E2001`): Runtime errors (composable misuse, navigation errors, rendering issues)

Each error code links to a documentation page at `https://nuxt.com/docs/e/<CODE>` with more context.

### Structured Error Properties

In development mode, Nuxt errors include structured properties that provide actionable context:

| Property | Description |
|----------|-------------|
| `code` | The error code (e.g., `NUXT_E2001`) |
| `fix` | A concrete suggestion for how to fix the issue |
| `why` | An explanation of why the error occurred |
| `docsUrl` | A link to the relevant documentation page |

These properties are available on the error object in your custom `error.vue`:
...
<template>
  <div>
    ...
    <!-- Structured error context (available in dev) -->
    <div v-if="error?.fix || error?.docsUrl">
      <p v-if="error?.fix"><strong>Fix:</strong> {{ error.fix }}</p>
      <p v-if="error?.docsUrl">
        <a :href="error.docsUrl">Read more</a>
      </p>
    </div>
...
::note
The `fix`, `why`, and `docsUrl` properties are only available in development mode. They are tree-shaken from production builds.
::

### Build-Time Errors

Build-time errors display a framed message in the terminal with the error code, a fix suggestion, and a docs link:

```
[NUXT_B8001] Nuxt instance is unavailable!
├▶ see: https://nuxt.com/docs/e/B8001
╰▶ fix: Ensure this is called within a Nuxt module setup() function.
```

Module authors can create their own scoped error utilities using `createBuildErrorUtils` from `@nuxt/kit`:
