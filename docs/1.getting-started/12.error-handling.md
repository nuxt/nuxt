...
| `hint` | A hint to the user about the error |
...
<template>
  <div>
    ...
    <div v-if="error?.fix || error?.hint || error?.docsUrl">
      <p v-if="error?.fix">
...
::note
The `fix`, `why`, `hint`, and `docsUrl` properties are only available in development mode. They are tree-shaken from production builds.
::
...
Module authors can create their own scoped error utilities using `createErrorUtils` from `@nuxt/kit`:
