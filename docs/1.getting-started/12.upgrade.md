...
<summary> Codemod for migrating to Nuxt 4 : </summary>


```npx codemod@latest XXX/X/migration-recipe```

To facilitate the upgrade process, we have collaborated with the [Codemod](https://github.com/codemod-com/codemod) team to automate most of the migration steps. All codemods are open-source and available in the nuxt repository. For a complete list of nuxt 4 codemods, detailed information on each, their source, and various ways to run them, visit the [Codemod Registry](https://go.codemod.com/codemod-registry).

### **Running All Codemods**
...
This command will execute the following codemods in sequence, with the option to deselect any that you do not wish to run:
...
Each individual codemod is also listed alongside its respective change and can be executed independently.
...
**Note**: 
Automate this step by running this **codemod**:
...
```
npx codemod@latest nuxt/4/shallow-data-reactivity
```
...
```
npx codemod@latest nuxt/4/absolute-watch-paths
```
