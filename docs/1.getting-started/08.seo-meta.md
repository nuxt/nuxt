...
Providing an [`app.head`](/docs/api/nuxt-config#head) property in your [`nuxt.config.ts`](/docs/directory-structure/nuxt-config) allows you to statically customize the head for your entire app.
...
::note
When you set [`app.cdnURL`](/docs/api/nuxt-config#cdnurl), assets in your [`public/`](/docs/directory-structure/public) directory (including `favicon.ico`) are served from that CDN. Nuxt resolves public assets against `cdnURL`, falling back to `app.baseURL`. However, a static `app.head` link such as `href: '/favicon.ico'` above is a literal path and is **not** resolved against `cdnURL`. To point the favicon at the resolved location, build the `href` from runtime config with [`useHead()`](/docs/api/composables/use-head) in `app.vue`:
...
Within your [`app/pages/` directory](/docs/directory-structure/app/pages), you can use `definePageMeta` along with [`useHead`](/docs/api/composables/use-head) to set metadata based on the current route.
