...
You can enable view transitions to apply an automatic transition for all your [pages](/docs/guide/directory-structure/pages).
...
```
Possible values are: `false, true, 'always'`.
...
To override the default viewTransition value for a page, set the `viewTransition` key in [`definePageMeta`](/docs/api/utils/define-page-meta) of the page:
