...
A complete Nuxt testing strategy implementing both unit and end-to-end tests involves executing your application code across two different testing environments.

A testing environment is the environmental context that has been prepared by the time your test runner executes your spec files. It also determines what is available within any global setup files that will be executed. Among many other things, it determines if `happy-dom` or `jsdom` is loaded by default.

Test runners either implicitly or explicitly define what their environments are.

End-to-end test runners such as Playwright or Cucumber work by launching a browser and controlling your application over a remote connection and they will execute in Node by default. Imports from `@nuxt/test-utils/runtime` are unavailable in these test runners.

Vitest, Jest, and other more flexible test runners are designed to run any kind of test, and therefor have support for multiple environments. Depending on the environment and kind of test, you will be able to execute either `@nuxt/test-utils/runtime` _or_ `@nuxt/test-utils/e2e`.

::callout
**Note**: Misconfiguration and use of the "Nuxt" environment within tests where the `'@nuxt/test-utils/e2e'` module is used will result in flaky or unintended behavior.
::

### Nuxt Environment

The "Nuxt" environment provides access to the Nuxt context. This is useful for instances where you need to mock parts of your Nuxt application - such as server responses, components, or composables. You can use this environment for Unit Testing that requires the Nuxt environment to be accessible.

It is recommended to be explicit about what Unit Tests are executing in the Nuxt environment, therefor we recommend you place these tests in the `test/nuxt` directory.

### Node Environment

The "Node" environment is a pure Node environment and does NOT provide access to the Nuxt context.

This means that your test runtime (the specs or test files) is not able to access Nuxt's internals. Nuxt-specific bundling options such as custom paths via `~~` or `~` are also unavailable. This environment should be used for purely functional Unit tests and it MUST be used for end-to-end tests.

### Vitest Projects for Environments
...
- **E2E tests**: Place e2e tests in `test/e2e` - these must run in a Node environment to avoid unintended behavior
...
## Unit Testing with the Nuxt Environment
...
`@nuxt/test-utils` provides some built-in mocks for the DOM environment. These are designed to be run in context where the Nuxt Environment is available. This means that these utilities are not available within your end-to-end test files.
...
Your Nuxt and your Node environments need to be run in separate suites.

- Exports from the `@nuxt/test-utils/runtime` package must be run within the Nuxt environment.
- Exports from the `@nuxt/test-utils/e2e` package must be run in the Node environment. need to run in different testing environments and so can't be used in the same file.

If you would like to use both the end-to-end and unit testing functionality of `@nuxt/test-utils`, you must split your tests into separate files. It is recommended that you use file naming conventions to separate the two types of tests.

::callout
While you _can_ specify a test environment per-file with the special `// @vitest-environment nuxt` comment, failures where end-to-end tests are executed in the Nuxt environment are intermittent, flaky, and hard to isolate. We suggest that you name your runtime unit test files with the `.nuxt.spec.ts` extension or a similar folder structure or naming convention.
::
...
A common pattern is to use Vitest or Jest to execute Playwright via `@nuxt/test-utils`'s end-to-end helpers. This allows you to reduce the number of test runners and amount of test tooling included in your project; however, you may miss out on extra features included by the full test runner. `@nuxt/test-utils` is flexible and provides support for both workflows.

### Purpose

The `@nuxt/test-utils/e2e` module provider helpers that are crucial for orchestrating your Nuxt application in a testable, convenient, and stable way. Read more in the [End-to-End APIs](#end-to-end-apis) section to understand why these methods should be used when testing your Nuxt application.
...
The `createPage` method allows Nuxt to wrap methods like Playwright's `goto` with lifecycle hooks such as waiting for the page to be hydrated before continuing with the test.

::callout
This helper function is crucial to solving the flake that you will encounter if you try to interact with your pages and hydration has not yet occured.
::
