...
Test files in `test/` and `tests/` directories can now use TypeScript path aliases like `~/`, `@/`, and `#imports`.

::important
This provides **TypeScript type support only**. Unit tests should not depend on Nuxt runtime features like auto-imports or composables. For tests that need Nuxt runtime, use the `nuxt` environment as described above.
::

**Example - Unit test with path aliases:**

```ts
// test/unit/utils.test.ts - runs in Node environment
import { describe, expect, it } from 'vitest'
import { formatDate } from '~/utils/date' // ✅ TypeScript resolves this
...
**Example - Nuxt test with runtime support:**
...
To include additional test directories in the TypeScript configuration:
