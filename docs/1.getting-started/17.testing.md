...
By default, test files in `test/nuxt/` and `tests/nuxt/` directories have TypeScript path alias support (`~/`, `@/`, `#imports`). This matches the recommended structure where only tests that need the Nuxt runtime environment are placed in these directories.

::tip
Tests using the Nuxt environment (`test/nuxt/` or `tests/nuxt/`) automatically get TypeScript path alias support. Unit tests in other directories like `test/unit/` can be added manually if needed.
::

**Example - Nuxt test with path aliases:**

```ts
// test/nuxt/components.test.ts - runs in Nuxt environment
import { mountSuspended } from '@nuxt/test-utils/runtime'
import MyComponent from '~/components/MyComponent.vue' // ✅ Works automatically
...
If you have unit tests or other test directories that need TypeScript path alias support, add them to your configuration:
