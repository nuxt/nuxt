...
1. Install Vitest browser packages and a browser provider:
   ...
   ::code-group{sync="pm"}
   ```bash [npm]
   npm i --save-dev @vitest/browser @vitest/browser-playwright
   ```
   ```bash [yarn]
   yarn add --dev @vitest/browser @vitest/browser-playwright
   ```
   ```bash [pnpm]
   pnpm add -D @vitest/browser @vitest/browser-playwright
   ```
   ```bash [bun]
   bun add --dev @vitest/browser @vitest/browser-playwright
   ```
   ::
