...
Static and prerender builds also emit `200.html` and `404.html` SPA fallbacks. See [What are 200.html and 404.html?](/docs/guide/concepts/rendering#what-are-200html-and-404html).
...
You can manually specify routes that Nuxt will fetch and pre-render during the build or ignore routes that you don't want to pre-render like `/dynamic` in the `nuxt.config` file:
...
The top-level `prerender` option applies whichever `server.builder` you use. Builder-specific options, such as Nitro's `concurrency` or `failOnError`, can be set in `nitro.prerender`.
...
When Nuxt renders a page on the server, it serializes the results of your data fetching ([`useAsyncData`](/docs/api/composables/use-async-data) and [`useFetch`](/docs/api/composables/use-fetch)) and app state ([`useState`](/docs/api/composables/use-state)) into a payload so the client can hydrate without re-fetching. With payload extraction enabled, Nuxt also writes this payload to a `_payload.json` file alongside the route's HTML:

- Prerendered routes generate their payload file at build time.
- Routes using [ISR or SWR caching](/docs/guide/concepts/rendering#hybrid-rendering) generate their payload file when the route is first rendered, even on a hybrid (non-static) site.
...
You can control this behavior with the [`experimental.payloadExtraction`](/docs/api/nuxt-config#payloadextraction) option:
...
:read-more{title="Payload reducers and revivers" to="/docs/api/composables/use-nuxt-app#payload"}
