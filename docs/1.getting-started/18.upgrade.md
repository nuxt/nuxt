...
While Nuxt 4 is the current stable version, you can prepare for Nuxt 5 by testing your application with Nuxt 5 compatibility mode enabled. This allows you to identify and fix potential issues before Nuxt 5 is released.

#### What is compatibilityVersion?

The `compatibilityVersion` option allows you to opt-in to Nuxt 5 behavior while running Nuxt 4. Setting `compatibilityVersion: 5` changes defaults throughout your Nuxt configuration to match what will become the standard in Nuxt 5.

According to the [Nuxt roadmap](/docs/4.x/community/roadmap), Nuxt 5 is scheduled for Q4 2025 and will include an upgrade to Nitro v3 and additional changes. Testing with `compatibilityVersion: 5` now helps ensure a smooth transition when Nuxt 5 is released.

#### How to Enable Nuxt 5 Compatibility Mode

To test your application with Nuxt 5 compatibility mode, add the following to your `nuxt.config.ts`:
...
After enabling `compatibilityVersion: 5`, follow these steps to ensure your application works correctly:

1. **Run your development server** and check for any warnings or errors:
...
2. **Run your test suite** to identify any breaking changes:
...
3. **Build your application** to check for any build-time issues:
...
When you enable `compatibilityVersion: 5`, some defaults in your Nuxt configuration will change to match Nuxt 5 behavior. You may encounter:

- Changes in default configuration values
- Different behavior in certain APIs or features
- Deprecation warnings for features that will be removed in Nuxt 5

::note
Keep in mind that `compatibilityVersion: 5` enables opt-in behavior for upcoming Nuxt 5 features. The exact changes may evolve as Nuxt 5 development progresses. We recommend testing periodically with the latest Nuxt 4 version to stay up to date with the planned Nuxt 5 changes.
::

#### Best Practices

- **Test in a separate branch**: Create a dedicated branch for testing Nuxt 5 compatibility to avoid disrupting your main development workflow.
- **Monitor the changelog**: Keep an eye on the [Nuxt releases](https://github.com/nuxt/nuxt/releases) for updates about Nuxt 5 compatibility changes.
- **Report issues**: If you encounter unexpected behavior or issues while testing with `compatibilityVersion: 5`, please [report them on GitHub](https://github.com/nuxt/nuxt/issues/new/choose).
- **Gradual adoption**: You can test with `compatibilityVersion: 5` in development while keeping it disabled in production until you're ready to fully commit.

#### Disabling Nuxt 5 Compatibility Mode

If you need to revert back to Nuxt 4 defaults, simply remove the `compatibilityVersion` setting or set it to `4`:

```ts [nuxt.config.ts]
export default defineNuxtConfig({
  future: {
    compatibilityVersion: 4, // or remove this configuration entirely
  },
})
```

::tip
Testing your application with `compatibilityVersion: 5` now will help you prepare for a smoother migration when Nuxt 5 is officially released. Early testing allows you to identify and resolve compatibility issues at your own pace.
::
