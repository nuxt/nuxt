...
To type your custom page metadata, you should now augment `CustomPageMeta` interface instead of `PageMeta`.

#### Reasons for Change

This makes it possible to provide accurate autocompletion when setting page metadata defaults via `app.customPageMeta` in your `nuxt.config`.

#### Migration Steps

Previously, you might have extended the `PageMeta` interface to type custom metadata:
...
Now, you should extend `CustomPageMeta` instead:
