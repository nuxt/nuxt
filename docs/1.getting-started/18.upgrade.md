...
- **Deprecated `process.*` build flags removed**: Nuxt no longer injects [deprecated `process.*` flags](/docs/4.x/getting-started/upgrade#deprecated-process-build-flags-removed) when `compatibilityVersion` is `5`
...
### Deprecated `process.*` Build Flags Removed

🚦 **Impact Level**: Medium

#### What Changed

With `compatibilityVersion: 5`, Nuxt no longer injects build-time defines for deprecated `process.browser`, `process.client`, `process.dev`, `process.server`, and `process.test`.

Use [`import.meta.*`](/docs/4.x/api/advanced/import-meta) instead. These flags are replaced at build time, so they stay tree-shakeable.
...
To keep the legacy defines while you migrate, stay on `future.compatibilityVersion: 4`.
