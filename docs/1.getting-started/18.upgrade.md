...
- **`ServerOnly` as a server component**: [`<ServerOnly>` uses NuxtIsland](/docs/4.x/getting-started/upgrade#serveronly-component-migration-to-server-components); slot content may run on the client during hydration or SPA navigation
...
1. Replace `nitro/h3` and `@nuxt/nitro-server/h3` imports with `nuxt/server`, for the utilities in [the surface](/docs/5.x/guide/going-further/server-imports#the-surface).
...
Add explicit imports to your server code. Prefer [`nuxt/server`](/docs/5.x/guide/going-further/server-imports), which is what the auto-imports resolve to:
...
### ServerOnly Component Migration to Server Components

::warning
**Breaking change**: Slot content may now run on the client during hydration or SPA navigation. Values like `{{ new Date() }}` can differ from initial SSR output.
::
...
`<ServerOnly>` has been refactored from a custom implementation to a [server component](/docs/4.x/directory-structure/app/components#server-components) (NuxtIsland). Previously, the slot content was rendered only on the server and replaced with static HTML on the client—the slot never ran on the client. Now, as a server component, the slot can also execute on the client during hydration or client-side navigation.

#### Reasons for Change

This change aligns `<ServerOnly>` with Nuxt's server component architecture and the islands model, providing consistency with other built-in components like `<NuxtClientFallback>`.

#### Migration Steps

If you rely on the slot content being strictly fixed at SSR time (e.g. `{{ new Date() }}`, server-only computed values), be aware that the slot may run on the client during hydration or when navigating to the page via SPA. The displayed value can differ from the initial SSR output.

- For content that must remain exactly as rendered on the server, consider moving it into a dedicated `.server.vue` component.
- Do not use `<ServerOnly>` for secrets or PII—rendered content is visible in the page HTML.

::read-more{to="/docs/4.x/api/components/server-only"}
See the ServerOnly component documentation for details.
::
