...
- **Vite Environment API**: Automatically enables the new [Vite Environment API](/docs/getting-started/upgrade#migration-to-vite-environment-api) for improved build configuration
...
::tip
You can test this feature early by setting `future.compatibilityVersion: 5` (see [Testing Nuxt 5](/docs/getting-started/upgrade#testing-nuxt-5)) or by enabling it explicitly with `experimental.viteEnvironmentApi: true`.
::
...
With `compatibilityVersion: 5`, `$fetch` and `useFetch` are typed from the routes Nuxt's server
builder reports, rather than from nitro's `InternalApi` interface. As well as response types, a
`body`, `query` or `headers` the matching handler validates are now enforced on the call, a method
the route does not handle is rejected, and a response is typed as it arrives over the wire, so a
`Date` a handler returns is a `string` at the call site.
...
Nitro v3 drops typed fetch, and the old implementation did not scale past about 300 routes. The new
generated route types cost the same to check whatever the route count, and carry enough information
to type the request as well as the response. `params` is dropped so the name is free for named route
parameters later.
...
::tip
Either typing can be selected on its own with
[`experimental.routeTypedFetch`](/docs/guide/going-further/experimental-features#routetypedfetch),
so you can adopt this before moving to Nuxt 5, or move to Nuxt 5 with `routeTypedFetch: false` and
adopt it separately. The option is temporary: from Nuxt 5 the generated route set is the only
source.
::

Set [`experimental.strictRouteTypes`](/docs/guide/going-further/experimental-features#strictroutetypes)
to reject a path no route answers.
