...
- **Decorator support is not yet available.** The `experimental.decorators` option relies on esbuild's decorator transform, which is not available in Rolldown/Oxc. This is being tracked in [oxc-project/oxc#9170](https://github.com/oxc-project/oxc/issues/9170). See the [Vite 8 migration guide](https://main.vite.dev/guide/migration#javascript-transforms-by-oxc) for workarounds using Babel or SWC.
