...
- **Normalized page names**: Page component names will [match their route names](/docs/getting-started/upgrade#normalized-page-component-names) for consistent `<KeepAlive>` behavior
...
- **Client-side payload extraction**: [`experimental.payloadExtraction`](/docs/getting-started/upgrade#payload-extraction-defaults-to-client) becomes `'client'`, inlining the payload in the HTML of a prerendered page
- **`clearNuxtState` resets to defaults**: `clearNuxtState` will [reset state to its initial value](/docs/getting-started/upgrade#respect-defaults-when-clearing-usestate) instead of setting it to `undefined`
...
Other changes are part of Nuxt 5 itself and are not controlled by the flag, including the [Vite Environment API](/docs/getting-started/upgrade#migration-to-vite-environment-api) migration, the removal of the [`process.*` type augmentation](/docs/getting-started/upgrade#process-type-augmentation-removed), [typed `$fetch`](/docs/getting-started/upgrade#typed-fetch-rebuilt-on-generated-route-types) and the upgrade to [Nitro v3](/docs/getting-started/upgrade#migration-to-nitro-v3). They are documented in the sections below.
...
With `compatibilityVersion: 5`, `experimental.normalizePageNames` is enabled, so page components are named after their route rather than after their filename. `pages/foo/index.vue` and `pages/bar/index.vue` are named `foo` and `bar` instead of both being named `index`.
...
With `compatibilityVersion: 5`, `experimental.extractSerializablePageMeta` is enabled. Every JSON-serializable `definePageMeta()` property is written into the generated route record, rather than only a fixed list of keys (plus `experimental.extraPageMetaExtractionKeys`) with the rest resolved at runtime from a per-page meta module.
...
With `compatibilityVersion: 5`, `experimental.payloadExtraction` defaults to `'client'` rather than `true`. The payload of a prerendered or cached page is inlined in the HTML for the initial render, and the `_payload.json` file is only fetched for client-side navigation. It is still forced to `false` when `ssr` is disabled.
...
With `compatibilityVersion: 5`, `experimental.defaults.useState.resetOnClear` is enabled, so [`clearNuxtState`](/docs/api/utils/clear-nuxt-state) resets state to the value produced by the `init` function of [`useState`](/docs/api/composables/use-state) instead of setting it to `undefined`.
...
With `compatibilityVersion: 5`, `experimental.navigateToEarlyReturn` is enabled. A top-level `await navigateTo()` in `<script setup>` is compiled into an early return from `setup()` when the navigation succeeds, so the rest of the setup code does not run and the component renders a placeholder comment while the navigation proceeds.
...
With `compatibilityVersion: 5`, `experimental.watcher` defaults to `'builder'`, so Nuxt reuses the active builder's file watcher (Vite's `server.watcher`, for example) instead of starting a second one. If the builder provides no watcher, Nuxt falls back to its previous selection.
...
::note
This change cannot be opted into with `future.compatibilityVersion: 5`. If you want to test Vite 8 compatibility ahead of time, you can add a `"vite": "^8.3.0"` resolution override in your `package.json`.
::
...
With `compatibilityVersion: 5`, `useServerHead`, `useServerHeadSafe` and `useServerSeoMeta` are removed from Nuxt's auto-imports. They are still exported from `#app`, so an explicit import keeps working.
...
`autoprefixer` and `cssnano` are no longer configured by default, and are no longer dependencies of `@nuxt/vite-builder`. They are optional peer dependencies, applied only when you name them in `postcss.plugins`. CSS is minified by the bundler rather than by `cssnano`.
...
::tip
You can test this feature early by setting `future.compatibilityVersion: 5` (see [Testing Nuxt 5](/docs/getting-started/upgrade#testing-nuxt-5)) or by enabling it explicitly with `experimental.asyncCallHook: false`.
::
...
::tip
You can test this feature early by setting `future.compatibilityVersion: 5` (see [Testing Nuxt 5](/docs/getting-started/upgrade#testing-nuxt-5)) or by enabling it explicitly with `experimental.clientNodePlaceholder: true`.
::
...
::tip
You can test this feature early by setting `future.compatibilityVersion: 5` (see [Testing Nuxt 5](/docs/getting-started/upgrade#testing-nuxt-5)) or by enabling it explicitly with `experimental.typedPages: true`.
::
