...
1. **Multiple `tsconfig.json` files**: Nuxt now generates three separate TypeScript configurations:
   - `.nuxt/tsconfig.json` - For your app code (Vue components, composables, etc.)
...
2. **Project references**: Your project's root `tsconfig.json` should now use TypeScript's project references feature to reference these individual configurations.

3. **Context-specific type checking**: Each context now has appropriate compiler options and includes/excludes for its specific environment.

4. **New `typescript.nodeTsConfig` option**: You can now customize the TypeScript configuration for Node.js build-time code.
...
For most projects, the migration should be automatic when you run `nuxt prepare`. The new configuration provides better type safety and IntelliSense out of the box.
