...
Nuxt 5 is **currently in development**. From Nuxt 4.2+ you can test many of its breaking changes by setting `future.compatibilityVersion` to `5`:
...
Defaults throughout your Nuxt configuration then change to opt in to Nuxt 5 behaviour. On Nuxt 4 this covers:

- **Vite Environment API**: [`experimental.viteEnvironmentApi`](/docs/guide/going-further/experimental-features#viteenvironmentapi) is enabled, changing how Vite plugins are registered and configured
- **Case-sensitive routing**: page routes match URL casing exactly, consistent with Nitro, so `/About` no longer matches `pages/about.vue`
- **Normalized page names**: page component names [match their route names](/docs/guide/going-further/experimental-features#normalizepagenames) for consistent `<KeepAlive>` behaviour
- **Serializable page meta**: [`experimental.extractSerializablePageMeta`](/docs/guide/going-further/experimental-features#extractserializablepagemeta) extracts statically analysable `definePageMeta` values at build time
- **Client-side payload extraction**: [`experimental.payloadExtraction`](/docs/guide/going-further/experimental-features#payloadextraction) becomes `'client'`, inlining the payload in the HTML of a prerendered page
- **`clearNuxtState` resets to defaults**: `clearNuxtState` resets state to the initial value of [`useState`](/docs/api/composables/use-state) instead of setting it to `undefined`
- **Non-async `callHook`**: `callHook` may return `void` instead of always returning a `Promise`
- **Early return from `navigateTo`**: [`experimental.navigateToEarlyReturn`](/docs/guide/going-further/experimental-features#navigatetoearlyreturn) stops a middleware continuing after it redirects
- **Server-only head composables**: `useServerHead`, `useServerHeadSafe` and `useServerSeoMeta` are no longer auto-imported, `unhead.legacy` is ignored, Unhead's legacy plugins are no longer registered and Capo sorting is always on
- **PostCSS defaults off**: `autoprefixer` and `cssnano` are no longer configured unless you enable them in `postcss.plugins`
- **Keyed composables require `source`**: a [`optimization.keyedComposables`](/docs/api/nuxt-config#keyedcomposables) entry without a string `source` is no longer matched through its auto-import
- **Comment node placeholders**: client-only components use [comment nodes instead of `<div>`](/docs/guide/going-further/experimental-features#clientnodeplaceholder) as SSR placeholders
- **Typed pages**: [`experimental.typedPages`](/docs/guide/going-further/experimental-features#typedpages) is enabled for type-checked routing
- **Typed `$fetch`**: [`experimental.routeTypedFetch`](/docs/guide/going-further/experimental-features#routetypedfetch) types `$fetch` and `useFetch` from the generated route types
- **Nitro auto-imports off**: server code imports Nitro's helpers explicitly rather than relying on the auto-import compatibility layer
- **Vue Options API disabled**: the Options API is compiled out of the client bundle to reduce its size
- **Stricter side-effect imports**: the generated `tsconfig.json` enables `noUncheckedSideEffectImports`
- **TypeScript `baseUrl` ignored**: generated TypeScript configurations no longer use `compilerOptions.baseUrl` to resolve Nuxt aliases

::note
This list describes what the flag changes on Nuxt 4. Nuxt 5 itself carries further changes that no flag enables here, such as the upgrade to Nitro v3.
::

The migration steps for each change, and everything else the upgrade brings, are documented in the [Nuxt 5 upgrade guide](https://nuxt.com/docs/5.x/getting-started/upgrade).

## Moving to `nuxt/server`

From Nuxt 4.6, server code can import its handler and helpers from [`nuxt/server`](/docs/guide/going-further/server-imports) rather than from h3, so the same file runs on Nuxt 4 (`nitropack` v2 and h3 v1) and on [Nuxt 5](https://nuxt.com/docs/5.x/getting-started/upgrade) (Nitro v3 and h3 v2).

On Nuxt 4.6, import `defineEventHandler` and the helpers from `nuxt/server` explicitly. The auto-imported names of the same spelling are h3 v1's, and h3's `defineEventHandler` passes your handler a different event from the one the `nuxt/server` helpers read.
...
On Nuxt 5 the same names are auto-imported from `nuxt/server`, so explicit imports written on Nuxt 4.6 keep working unchanged.

### Differences From h3 v1

| h3 v1 | `nuxt/server` |
|---|---|
| `sendRedirect(event, location)` sends the response. | `sendRedirect(event, location)` returns the response body: `return sendRedirect(event, '/login')`. |
| `getRouterParams(event)` returns decoded values. | `getRouterParams(event)` returns the values as they appear in the URL. Pass `{ decode: true }` to decode them. |
| `handleCors(event, options)` returns `true` when it answered a preflight request. | `handleCors(event, options)` returns the preflight `Response` to return from the handler, or `false`. |
| `readValidatedBody` and `getValidatedQuery` take a validation function. | They take a [Standard Schema](https://standardschema.dev) or a function. |
| `createError({ statusCode, statusMessage })` | `createError({ status, statusText })` |
| `useSession` and friends seal an `h3` cookie with the `password` you pass. | They seal a `nuxt-session` cookie, by default with a secret derived from `appSecret`. The two are not interchangeable: a session one issues does not unseal with the other. |
...
### Helpers Outside `nuxt/server`

For h3 or Nitro helpers that `nuxt/server` does not provide, such as `readMultipartFormData`, `proxyRequest`, `useStorage` or `defineCachedEventHandler`, pass them `toNuxtRequestEvent(event)`, which is the h3 v1 event on Nuxt 4. Code that does this is tied to the server runtime and needs revisiting on Nuxt 5.
...
A request fails with a `500` whose message starts with `[NUXT_E8012]` when a `nuxt/server` helper such as `readValidatedBody`, `getValidatedQuery`, `handleCors` or `useSession` receives an h3 event. This means the handler was defined with h3's `defineEventHandler` (usually the auto-imported one). Import `defineEventHandler` from `nuxt/server` in the same file as the helpers.

### Application Secret

The `nuxt/server` session helpers and `deriveSecret` read `runtimeConfig.appSecret`, which you set with `NUXT_APP_SECRET`. On Nuxt 4, `nitropack` v2 parses environment overrides, so a secret made only of digits arrives as a number; see [the application secret](/docs/guide/going-further/runtime-config#application-secret) for how to avoid this.
...
- a new [`shared/`](/docs/directory-structure/shared) directory is available for code shared between the Vue app and the Nitro server, with auto-imports for `shared/utils/` and `shared/types/`
...
If you encounter issues with module order dependencies due to needing to register a hook, consider using the [`modules:done` hook](/docs/guide/modules/recipes-advanced) for modules that need to call a hook. This is run after all other modules have been loaded, which means it is safe to use.
