...
- **Decorator support now uses Babel** (Vite builder only). The `experimental.decorators` option now uses `@rollup/plugin-babel` with `@babel/plugin-proposal-decorators` instead of esbuild's decorator transform. Nuxt will prompt you to install these packages if they are not already present. The webpack and rspack builders continue to use esbuild for decorator lowering.
