...
Nuxt 5 includes significant improvements and changes. This guide covers what changed and how to migrate an existing Nuxt 4 application.

First, upgrade Nuxt using the [instructions above](#upgrading-nuxt).

Most Nuxt 5 behaviour is the new default. Where a change can still be configured, the section describing it names the option that restores the Nuxt 4 behaviour; where it cannot, the section says so.

The changes are:

- **Nitro v3**: The server engine is [rewritten on h3 v2 and srvx](/docs/getting-started/upgrade#migration-to-nitro-v3), with a [compatibility layer](/docs/getting-started/upgrade#nitro-v2-compatibility) for server code you have not migrated yet
- **Server imports**: Server utilities come from [`nuxt/server`](/docs/getting-started/upgrade#server-imports-move-to-nuxtserver), including [session helpers](/docs/getting-started/upgrade#sessions-come-from-nuxtserver)
- **Vite Environment API**: Vite plugins are [registered and configured per environment](/docs/getting-started/upgrade#migration-to-vite-environment-api)
- **Vite 8**: The bundler is [Rolldown rather than esbuild and Rollup](/docs/getting-started/upgrade#migration-to-vite-8)
- **Nitro as a Vite environment**: With `@nuxt/vite-builder`, server and client are [built by one Vite build](/docs/getting-started/upgrade#nitro-builds-as-a-vite-environment)
- **Typed `$fetch`**: `$fetch` and `useFetch` are typed from [generated route types](/docs/getting-started/upgrade#typed-fetch-rebuilt-on-generated-route-types) rather than nitro's `InternalApi`
- **`jiti` is no longer bundled**: `nuxt.config`, `modules/` and layer configs are [loaded by the runtime](/docs/getting-started/upgrade#jiti-is-no-longer-bundled)
- **`process.*` type augmentation removed**: TypeScript no longer exposes [deprecated `process.*` flags](/docs/getting-started/upgrade#process-type-augmentation-removed) on `NodeJS.Process`
- **Case-sensitive routing**: Page routes [match URL casing exactly](/docs/getting-started/upgrade#case-sensitive-routing), consistent with Nitro
- **Normalized page names**: Page component names [match their route names](/docs/getting-started/upgrade#normalized-page-component-names) for consistent `<KeepAlive>` behavior
- **Serializable page meta**: Statically analysable `definePageMeta` values are [written into the generated route record](/docs/getting-started/upgrade#serializable-page-meta-extracted-at-build-time)
- **Client-side payload extraction**: [`experimental.payloadExtraction`](/docs/getting-started/upgrade#payload-extraction-defaults-to-client) is `'client'`, inlining the payload in the HTML of a prerendered page
- **`clearNuxtState` resets to defaults**: `clearNuxtState` [resets state to its initial value](/docs/getting-started/upgrade#respect-defaults-when-clearing-usestate) instead of setting it to `undefined`
- **Early return from `navigateTo`**: A successful top-level `await navigateTo()` [stops the rest of your `setup()` running](/docs/getting-started/upgrade#early-return-from-navigateto)
- **Non-async `callHook`**: [`callHook` may return `void`](/docs/getting-started/upgrade#non-async-callhook) instead of always returning a `Promise`
- **Comment node placeholders**: Client-only components use [comment nodes instead of `<div>`](/docs/getting-started/upgrade#client-only-comment-placeholders) as SSR placeholders, fixing a scoped styles hydration issue
- **Server auto-imports off**: Server code [imports Nitro's helpers explicitly](/docs/getting-started/upgrade#server-auto-imports-are-now-opt-in) rather than relying on the auto-import layer
- **Parsed `error.data`**: [`experimental.parseErrorData`](/docs/getting-started/upgrade#experimentalparseerrordata-is-no-longer-configurable) is forced on, so `error.data` reaches the error page unstringified
- **Builder file watcher**: Nuxt [reuses the builder's own watcher](/docs/getting-started/upgrade#builder-file-watcher-in-development) in development instead of starting a second one
- **Server-only head composables**: `useServerHead`, `useServerHeadSafe` and `useServerSeoMeta` are [no longer auto-imported](/docs/getting-started/upgrade#server-only-head-composables-are-no-longer-auto-imported), and `unhead.legacy` is ignored
- **Vue Options API disabled**: The [Options API is compiled out of the client bundle](/docs/getting-started/upgrade#vue-options-api-disabled-by-default) to reduce its size
- **Typed pages**: [`experimental.typedPages`](/docs/getting-started/upgrade#typed-pages-enabled-by-default) is enabled by default for type-checked routing
- **Node-resolution config types**: The generated `node` tsconfig describes [how the runtime loads `nuxt.config` and modules](/docs/getting-started/upgrade#jiti-is-no-longer-bundled)
- **Stricter side-effect imports**: The generated `tsconfig.json` enables [`noUncheckedSideEffectImports`](/docs/getting-started/upgrade#stricter-side-effect-imports) to match the TypeScript 7 default
- **TypeScript `baseUrl` ignored**: Generated TypeScript configurations [no longer use `compilerOptions.baseUrl`](/docs/getting-started/upgrade#typescript-baseurl-is-ignored) to resolve Nuxt aliases
- **PostCSS plugins**: `autoprefixer` and `cssnano` are [no longer installed or configured by default](/docs/getting-started/upgrade#postcss-plugins-are-no-longer-installed-by-default)
- **Optional dependencies**: [`@vitejs/plugin-vue-jsx`](/docs/getting-started/upgrade#vitejsplugin-vue-jsx-is-now-optional) and [`giget`](/docs/getting-started/upgrade#giget-is-now-optional-remote-layers) are installed on demand
- **Removed options**: [Several experimental options](/docs/getting-started/upgrade#removed-configuration-options) are gone, because the behaviour they controlled is now unconditional

::note
Nuxt 5 is still in development, so this page changes as further work lands. Check back here regularly while you are migrating.
::

Breaking or significant changes are documented below along with migration steps and the configuration available for each.
...
Prefer [`import.meta.*`](/docs/api/advanced/import-meta). Those flags are replaced at build time and stay tree-shakeable.
...
`router.options.sensitive` now defaults to `true`, so page routes match URLs case-sensitively, consistent with Nitro. For example, `/About` no longer matches `pages/about.vue`.
...
`experimental.normalizePageNames` is now enabled by default, so page components are named after their route rather than after their filename. `pages/foo/index.vue` and `pages/bar/index.vue` are named `foo` and `bar` instead of both being named `index`.

#### Migration Steps

If you filter `<KeepAlive>` by component name, update `include`/`exclude` to the route names. To keep the filename-based names:
...
### Serializable Page Meta Extracted at Build Time

🚦 **Impact Level**: Minimal

#### What Changed

`experimental.extractSerializablePageMeta` is now enabled by default. Every JSON-serializable `definePageMeta()` property is written into the generated route record, rather than only a fixed list of keys (plus `experimental.extraPageMetaExtractionKeys`) with the rest resolved at runtime from a per-page meta module.

Properties whose values cannot be serialized, such as functions, variable references and spreads, are still resolved at runtime.

#### Migration Steps

If you read page meta from the route record at build time (in a `pages:extend` hook, for example), expect to see the extracted values there. To keep the previous behaviour:
...
### Payload Extraction Defaults to `'client'`

🚦 **Impact Level**: Minimal

#### What Changed

`experimental.payloadExtraction` now defaults to `'client'` rather than `true`. The payload of a prerendered or cached page is inlined in the HTML for the initial render, and the `_payload.json` file is only fetched for client-side navigation. It is still forced to `false` when `ssr` is disabled.

#### Migration Steps

No change is needed for most applications. If you fetch `_payload.json` for the initial render yourself, or measure HTML size in a test, set the option explicitly:
...
`experimental.defaults.useState.resetOnClear` is now enabled by default, so [`clearNuxtState`](/docs/api/utils/clear-nuxt-state) resets state to the value produced by the `init` function of [`useState`](/docs/api/composables/use-state) instead of setting it to `undefined`.

#### Migration Steps

If your code checks for `undefined` after clearing state, check for the initial value instead. To keep the previous behaviour:
...
`experimental.navigateToEarlyReturn` is now enabled by default. A top-level `await navigateTo()` in `<script setup>` is compiled into an early return from `setup()` when the navigation succeeds, so the rest of the setup code does not run and the component renders a placeholder comment while the navigation proceeds.

#### Migration Steps

If you relied on code after a successful `await navigateTo()` running, move it before the navigation. To keep the previous behaviour:
...
### Builder File Watcher in Development

🚦 **Impact Level**: Minimal

#### What Changed

`experimental.watcher` now defaults to `'builder'`, so Nuxt reuses the active builder's file watcher (Vite's `server.watcher`, for example) instead of starting a second one. If the builder provides no watcher, Nuxt falls back to its previous selection.

#### Migration Steps

If you depend on watching files the builder ignores, name a watcher explicitly:
...
**You can still write your config in TypeScript.** Nuxt 5 requires Node `22.19`, `24.11` or later, where type stripping is on by default, so `nuxt.config.ts` and TypeScript modules load natively. Two things the runtime does not do, which `jiti` used to paper over, are guess file extensions and compile TypeScript syntax that emits code.
...
::note
There is no switch for this: Nuxt 5 requires Vite 8. If you want to test Vite 8 compatibility on Nuxt 4 first, you can add a `"vite": "^8.3.0"` resolution override in your `package.json`.
::
...
### Nitro Builds as a Vite Environment

🚦 **Impact Level**: Medium

#### What Changed

With `@nuxt/vite-builder`, `experimental.nitroViteEnvironment` now defaults to `true`, so Nitro is built as a [Vite environment](https://vite.dev/guide/api-environment) through the `nitro/vite` plugin rather than through Nitro's own Rolldown pipeline. Server and client are built by one Vite build, and server code goes through the Vite plugins and resolution your app code already uses.

The option has no effect with other app builders, which keep Nitro's own pipeline. Setting it to `true` there logs a warning.

#### Migration Steps

Modules that configure Nitro's bundler directly (through `nitro.rollupConfig`, for example) reach a pipeline that no longer runs the server build. Register a Vite plugin instead, scoped with `applyToEnvironment`.

To keep Nitro's own build pass:
...
Which server runtime is under your application depends on the configured [`server.builder`](/docs/guide/going-further/builders), and importing from `nitro/h3` pins your code to one of them and to its major version. `nuxt/server` does not, so one file can serve Nuxt 4.6 (running either `nitropack` v2 or Nitro v3) and Nuxt 5.
...
1. Replace `nitro/h3` and `@nuxt/nitro-server/h3` imports with `nuxt/server`, for the utilities in [the surface](/docs/guide/going-further/server-imports#types-and-utilities).
...
`useSession`, `getSession`, `updateSession` and `clearSession` are now part of [`nuxt/server`](/docs/guide/going-further/server-imports#sessions), and are what the auto-imported names resolve to. They seal the session with `iron-webcrypto` on web-standard `Request` and `Response`, so they run under any [`server.builder`](/docs/guide/going-further/builders) rather than only under Nitro.
...
A `password` is no longer required. Without one, the session is sealed with a secret derived from [`appSecret`](/docs/guide/going-further/runtime-config#application-secret), which `NUXT_APP_SECRET` sets:

```diff [server/api/me.ts]
...
::read-more{to="/docs/guide/going-further/server-imports#sessions"}
...
The sections below highlight changes that are most relevant to Nuxt application developers and module authors. You don't have to make all of them before you upgrade: see [Nitro v2 Compatibility](#nitro-v2-compatibility) for the layer that keeps older server code running in the meantime.
...
Add explicit imports to your server code. Prefer [`nuxt/server`](/docs/guide/going-further/server-imports), which is what the auto-imports resolve to:
...
### Server-Only Head Composables Are No Longer Auto-Imported

🚦 **Impact Level**: Minimal

#### What Changed

`useServerHead`, `useServerHeadSafe` and `useServerSeoMeta` are removed from Nuxt's auto-imports. They are still exported from `#app`, so an explicit import keeps working.

The `unhead.legacy` option is ignored on Nuxt 5: setting it logs a warning. Unhead's legacy plugins (`DeprecationsPlugin`, `PromisesPlugin`, `AliasSortingPlugin`) are no longer registered, and Capo sorting is always on. `TemplateParamsPlugin` is still registered, so `%s`, `%siteName` and `%separator` title interpolation is unaffected.

Tree-shaking of server-only head composables out of the client bundle is handled by the `@unhead/vue/vite` plugin, which Nuxt registers when using `@nuxt/vite-builder`.

#### Migration Steps

Wrap a `useHead` call in `import.meta.server` instead, or import the composable explicitly:

```diff
+ import { useServerHead } from '#app'
+
  useServerHead({ meta: [{ name: 'generator', content: 'Nuxt' }] })
```

Remove `unhead.legacy` from your configuration:
...
The `experimental.parseErrorData` option is deprecated and forced on. Setting it to `false` logs a warning and is otherwise ignored, so `error.data` on the error page is always the value you passed to `createError`. There is no switch for this.
...
Three experimental options are removed, because the behaviour they controlled is now unconditional in Nuxt 5:

- `experimental.viteEnvironmentApi`. See [Migration to Vite Environment API](#migration-to-vite-environment-api).
- `experimental.routeTypedFetch`. See [Typed `$fetch` Rebuilt on Generated Route Types](#typed-fetch-rebuilt-on-generated-route-types).
- `experimental.renderJsonPayloads`. Payloads are always rendered and parsed as JSON, with support for revivers.

`experimental.externalVue` is removed too, which is covered in [its own section](#removal-of-experimentalexternalvue).

#### Migration Steps

Remove the options from your configuration:
...
### PostCSS Plugins Are No Longer Installed by Default

🚦 **Impact Level**: Medium

#### What Changed

`autoprefixer` and `cssnano` are no longer configured by default, and are no longer dependencies of `@nuxt/vite-builder`. They are optional peer dependencies, applied only when you name them in `postcss.plugins`. Vite minifies CSS as part of the build, so `cssnano` is not needed for that.

The webpack and rspack builders still configure both, along with `postcss-import` and `postcss-url`.

#### Reasons for Change

Both packages were installed for every project to run a pass most projects do not configure, and browser support for the properties `autoprefixer` prefixes has moved on since the default was set.

#### Migration Steps

If you need either plugin, install it and enable it explicitly:
...
```bash [pnpm]
pnpm add -D autoprefixer cssnano
```

```bash [bun]
bun add -D autoprefixer cssnano
```
...
If a configured plugin is not installed, Nuxt prompts you to install it and reports the command to run.
...
Nuxt 4 wrapped `callHook` with `Promise.resolve()` so that existing `.then()` and `.catch()` chaining continued to work. That wrapper is removed, and `experimental.asyncCallHook` now defaults to `false`.
...
To keep `callHook` always returning a `Promise`:
...
Client-only components (`.client.vue` files and `createClientOnly()` wrappers) now render an HTML comment (`<!--placeholder-->`) on the server instead of an empty `<div>` element.
...
To revert to the previous `<div>` placeholder behavior:
...
Nuxt's generated `tsconfig.json` enables [`noUncheckedSideEffectImports`](https://www.typescriptlang.org/tsconfig/#noUncheckedSideEffectImports). This is a default in TypeScript 7, so adopting it early keeps your project aligned ahead of that upgrade.
...
Nuxt sets Vue's [`__VUE_OPTIONS_API__`](https://vuejs.org/api/compile-time-flags#VUE_OPTIONS_API) feature flag to `false`, which compiles Vue's Options API runtime out of the client bundle.
...
`experimental.typedPages` is now enabled by default. Nuxt generates typed route names and paths from your `pages/` directory, so composables like `useRoute`, `navigateTo`, `<NuxtLink>` and `router.push` are type-checked against your actual routes.
...
To opt out and revert to the previous behavior:
...
- Routes you augmented by hand on nitro's `InternalApi` are no longer picked up. Augment
  `ServerRoutes` from `@nuxt/schema` instead.
- The `params` option is no longer accepted by `$fetch` or `useFetch`. It was an alias for `query`.

A path the generated route types do not describe still resolves, as `unknown`, unless you opt in to [`experimental.strictRouteTypes`](/docs/guide/going-further/experimental-features#strictroutetypes).
...
```diff [shared/server-routes.d.ts]
...
Nuxt removes `compilerOptions.baseUrl` from its generated TypeScript configurations. Relative Nuxt and Nitro aliases are resolved from Nuxt's build directory instead of a custom `baseUrl`.
...
- **Types.** If you augment `nitropack/types`, update it to the new module paths. See [Route Rule Types Move to `h3/rules`](#route-rule-types-move-to-h3rules).
...
2. Move your server imports to [`nuxt/server`](#server-imports-move-to-nuxtserver), which is available from Nuxt 4.6, and move anything it doesn't cover to `nitro/h3` or `nitro/*`. Leave `nitroLegacy` off while you do this.
