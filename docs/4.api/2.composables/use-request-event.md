...
`NuxtRequestEvent` is the event in the shape your configured [server builder](/docs/guide/going-further/builders) provides. With the default `@nuxt/nitro-server`, that is h3 v1's `H3Event`, which is why the example above reads `event.path` rather than `event.url`.
