...
`<NuxtPage>` is a built-in component that comes with Nuxt. It lets you display top-level or nested pages located in the [`app/pages/`](/docs/directory-structure/app/pages) directory.
...
- `name`: tells `<RouterView>` to render the component with the corresponding name in the matched route record's components option. See [Named Views](/docs/directory-structure/app/pages#named-views) for the `name@view.vue` filename convention.
  - type: `string`
