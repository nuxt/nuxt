...
- `name`: Specify a layout name to be rendered, can be a string, reactive reference or a computed property. It **must** match the name of the corresponding layout file in the [`layouts/`](/docs/directory-structure/layouts) directory, or `false` to disable the layout.
  - **type**: `string | false`
  - **default**: `default`
...
- `fallback`: If an invalid layout is passed to the `name` prop, no layout will be rendered. Specify a `fallback` layout to be rendered in this scenario. It **must** match the name of the corresponding layout file in the [`layouts/`](/docs/directory-structure/layouts) directory.
  - **type**: `string`
  - **default**: `null`
