...
::read-more{to="/docs/4.x/guide/best-practices/hydration#dev-only-debugging-aids"}
Learn when to use `<DevOnly>` vs client-only patterns.
::

::read-more{to="/docs/4.x/guide/concepts/server-and-client"}
See what runs on the server, during hydration, and in the browser.
::
