...
- `discover`: With [`experimental.lazyRouteDiscovery`](/docs/4.x/guide/going-further/experimental-features#lazyroutediscovery), controls whether the linked route is discovered when the link renders. `"render"` (default) discovers it so it becomes navigable without a round-trip on click; `"none"` leaves it undiscovered until navigation (useful on link-heavy pages). No effect when the feature is disabled.
