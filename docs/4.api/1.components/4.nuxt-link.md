...
The same `prefetch`, `prefetched` and `shouldPrefetch` values are returned by `NuxtLink.useLink()` :badge[v4.6]{color="info" size="xs" class="align-middle"}, which is useful if you wrap `<NuxtLink>` in your own component:

```vue
<script setup lang="ts">
const NuxtLink = resolveComponent('NuxtLink')
const { href, navigate, prefetch, prefetched, shouldPrefetch } = NuxtLink.useLink({ to: '/about' })
</script>
```

Prefetch state is shared per destination: `prefetched` becomes `true` once any link (rendered or created with `useLink()`) has prefetched that URL, and calling `prefetch()` for an already-prefetched destination is a no-op.
