...
Add `<NuxtRouteAnnouncer/>` in your [`app.vue`](/docs/directory-structure/app) or [`layouts/`](/docs/directory-structure/layouts) to enhance accessibility by informing assistive technologies about page title changes. This ensures that navigational changes are announced to users relying on screen readers.
