...
Nuxt supports multiple builders for the client-side application. By default, Vite is used, but you can switch to webpack, Rspack, Rsbuild, or even provide a custom builder implementation.

- **Type**: `'vite' | 'webpack' | 'rspack' | 'rsbuild' | string | { bundle: (nuxt: Nuxt) => Promise<void> }`
...
export default defineNuxtConfig({
  ...
  // uses @nuxt/rsbuild-builder
  builder: 'rsbuild',
})
```

If you are using `webpack`, `rspack` or `rsbuild` you will need to make sure `@nuxt/webpack-builder`, `@nuxt/rspack-builder` or `@nuxt/rsbuild-builder` is explicitly installed in your project.
