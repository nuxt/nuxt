...
When the app runs from a path inside `node_modules` (for example, with component libraries or similar tooling), Nuxt automatically adds ancestor `node_modules` directories to the resolution list so that dependencies from the host or workspace can be resolved. You can still set `modulesDir` explicitly if needed.
