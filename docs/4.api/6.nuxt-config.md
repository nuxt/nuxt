...
To create a custom builder package as a separate package, it should export a `bundle` function. You can then specify it in your `nuxt.config`:
...
**See**: [`c12` docs on extending config layers](https://github.com/unjs/c12#extending-config-layer-from-remote-git-repository)
...
Any file in `app/pages/`, `app/layouts/`, `app/middleware/`, and `public/` directories will be ignored during the build process if its filename starts with the prefix specified by the `ignorePrefix`. This is intended to prevent certain files from being processed or served in the built application. By default, the `ignorePrefix` is set to '-', ignoring any files starting with '-'.
...
An array of custom directories that will be auto-imported. Note that this option will not override the default directories (~/composables and ~/utils).
...
Whether to scan your `app/composables/` and `app/utils/` directories. Auto-imports registered by Nuxt or other modules, such as `vue`, will not be affected by this option.
...
As long as the number of arguments passed to the function is less than `argumentLength`, an additional magic string will be injected as the last argument. This key is stable across SSR and client-side hydration. You will need to take steps to handle this additional key.
...
export default defineNuxtConfig({
  optimization: {
    treeShake: {
      composables: {
        client: { vue: ['onServerPrefetch'] },
        server: { vue: ['onMounted'] },
      },
    },
  },
})
...
**See**: [`cssnano` configuration options](https://cssnano.github.io/cssnano/docs/config-file/)
...
Whether to enable hash history in SPA mode. In this mode, router uses a hash character (#) before the actual URL that is internally passed. When enabled, it will not support SSR.
...
Customize the options of `scrollBehavior`.
...
::callout
**Note**: Files from `server/api`, `server/middleware` and `server/plugins` will be automatically registered by Nuxt.
::
...
You can extend the generated `.nuxt/tsconfig.app.json` (and legacy `.nuxt/tsconfig.json`) TypeScript configuration using this option. Options set here take precedence over `tsConfig`.
...
You can extend the generated `.nuxt/tsconfig.server.json` TypeScript configuration using this option. Options set here take precedence over `tsConfig`.
...
If set to `true`, this will type check in development and at build time. Set to `build` to type check only when building, or `dev` to type check only in the dev server. Requires installing `typescript` and `vue-tsc` as dev dependencies.
...
Enable the legacy compatibility mode for the `unhead` nuxt module. This applies the following changes: - Disables Capo.js sorting - Adds the `DeprecationsPlugin`: supports `hid`, `vmid`, `children`, `body: true` - Adds the `PromisesPlugin`: supports promises as input
...
Enabling this allows components to compile at runtime (for example, string `template` options or templates supplied via data).
