...
If you extend [layers](/docs/4.x/getting-started/layers) and keep most of your Nuxt config in a shared layer, set `compatibilityDate` in that layer's `nuxt.config` rather than only in the project root. In interactive development, Nuxt can prompt you to choose among local layer configs when the date is missing.
