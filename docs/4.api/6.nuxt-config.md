...
Default values for view transitions (enabled flag and optional types).

This only has an effect when **experimental** support for View Transitions is [enabled in your nuxt.config file](/docs/4.x/getting-started/transitions#view-transitions-api-experimental). Setting `app.viewTransition` without `experimental.viewTransition` does not load the View Transitions plugin. You can override these defaults with `definePageMeta` on an individual page.

- **Type**: `boolean | 'always' | { enabled: boolean | 'always', types?: string[] | ((to, from) => string[]) }`
- **Default:** `false`
...
`"nitro"` and `"vite"` are shorthands for `@nuxt/nitro-server` and `@nuxt/vite-server`. The latter is experimental and implemented with Vite alone: it builds the client and ships no server of its own, so with `ssr: false` it produces a static SPA. With SSR enabled the server environment is still built, but running it is left to a Vite plugin that brings its own target or to a custom server. Server routes, route rules and prerendering are unavailable, and modules that need a server will not work.
