...
::note
`process.browser`, `process.client`, `process.dev`, `process.server`, and `process.test` were deprecated in Nuxt 3.12 and removed when `future.compatibilityVersion` is `5`. Use the matching `import.meta.*` properties instead.
::
