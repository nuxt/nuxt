...
Nuxt have builders based on [Vite](https://github.com/nuxt/nuxt/tree/main/packages/vite), [webpack](https://github.com/nuxt/nuxt/tree/main/packages/webpack), [Rspack](https://github.com/nuxt/nuxt/tree/main/packages/rspack) and [Rsbuild](https://rsbuild.rs). You can extend the config passed to each one using `extendViteConfig`, `extendWebpackConfig`, `extendRspackConfig` and `extendRsbuildConfig` functions. You can also add additional plugins via `addVitePlugin`, `addWebpackPlugin`, `addRspackPlugin`, `addRsbuildPlugin` and `addBuildPlugin`.
