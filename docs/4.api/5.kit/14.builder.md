...
Nuxt have builders based on [Vite](https://github.com/nuxt/nuxt/tree/main/packages/vite), [webpack](https://github.com/nuxt/nuxt/tree/main/packages/webpack), [Rspack](https://github.com/nuxt/nuxt/tree/main/packages/rspack) and [Rsbuild](https://github.com/nuxt/nuxt/tree/main/packages/rsbuild). You can extend the config passed to each one using `extendViteConfig`, `extendWebpackConfig`, `extendRspackConfig` and `extendRsbuildConfig` functions. You can also add additional plugins via `addVitePlugin`, `addWebpackPlugin`, `addRspackPlugin`, `addRsbuildPlugin` and `addBuildPlugin`.
...
interface AddBuildPluginFactory {
  ...
  rsbuild?: () => RspackPluginInstance | RspackPluginInstance[]
}
// ---cut---
...
**`pluginFactory`**: A factory function that returns an object with `vite`, `webpack`, `rspack` and/or `rsbuild` properties. These properties must be functions that return a plugin instance or an array of plugin instances for the matching builder.
