...
Add imports to both the Nuxt application and the Nitro server. This makes your imports available in both client and server contexts, as well as in the [`shared/`](/docs/4.x/directory-structure/shared) directory.

::warning
The source file should not import anything context-specific (i.e., Nitro context, Nuxt app context) or else it might cause errors during type-checking.
::

### Usage

```ts twoslash
import { addSharedImports, createResolver, defineNuxtModule } from '@nuxt/kit'
...
export default defineNuxtModule({
  setup (options, nuxt) {
    ...
    addSharedImports({
      name: 'formatCurrency',
      from: resolver.resolve('./runtime/shared/format-currency'),
    })
  },
})
```
...
```ts
function addSharedImports (imports: Import | Import[]): void
```
