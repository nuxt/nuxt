...
::tip
This is particularly useful when you want to disable modules inherited from [Nuxt layers](/docs/guide/going-further/layers#disabling-modules-from-layers).
::
...
**Deprecated:** Use the [`moduleDependencies`](/docs/api/kit/modules#specifying-module-dependencies) option in `defineNuxtModule` instead. The `installModule` function will be removed (or may become non-blocking) in a future version.
::
