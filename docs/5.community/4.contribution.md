...
* When documenting API pages (composables, utils, components), add the **minimal Nuxt version** when a feature or utility was introduced, so readers know which version they need.
  ...
  * **Global (whole page):** Add `minimalVersion: "3.9"` in the frontmatter. The docs layout renders the version badge automatically from this field. Use the format `vX.Y` (e.g. `v3.9`, `v3.15`).
