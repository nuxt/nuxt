...
Before creating the new local branch for the PR, make sure to prepare your local environment by running the following commands:

* `pnpm install --frozen-lockfile` to install the dependencies

* `pnpm build:stub` to create stubs

Once you've included the changes in your local branch, make sure to run the tests and build the repository to ensure that everything is working as expected, run the following commands:

* `pnpm test` to run the tests

* `pnpm build` to build the repository
