...
* When documenting API pages (composables, utils, components), add the **minimal Nuxt version** when a feature or utility was introduced, so readers know which version they need.
  ...
  * **Global (whole page):** Add `minimalVersion: "3.9"` in the frontmatter and an inline badge at the top of the content: `:badge[v3.9]{color="info" class="align-middle"}`. Use the format `vX.Y` (e.g. `v3.9`, `v3.15`).
