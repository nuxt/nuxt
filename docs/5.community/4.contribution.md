...
* When documenting API pages (composables, utils, components), add the **minimal Nuxt version** when a feature or utility was introduced, so readers know which version they need.
  ...
  * **Local (specific option or feature):** Add a version badge next to the option or section: `:versionBadge{version="3.8"}` (e.g. for `getCachedData` in useFetch, or for the `navigation` mode in callOnce). Pass the version without the "v" prefix — the badge renders it as `vX.Y`.

  The badge renders nothing once the version it names is no longer relevant to the docs being read, so always place it **last** — at the end of a heading, at the end of a table cell, or _after_ any trailing punctuation. Write `in route rules: :versionBadge{version="4.3"}`, not `in route rules :versionBadge{version="4.3"}:`, otherwise the sentence is left with a stray space before the colon when the badge is hidden.

  **If the feature has not been released yet, write `unreleased` instead of a number** — `minimalVersion: unreleased` or `:versionBadge{version="unreleased"}`. You don't need to know which release will carry your pull request, or whether it will be backported: the release tooling replaces `unreleased` with the version of the release that ships it, on each branch separately. The same markdown therefore becomes `v4.6` on the 4.x docs and `v3.21` on the 3.x docs, and the badge disappears entirely when the change lands in the first release of a major, where it is the baseline. Until then, the badge reads `unreleased`, which tells readers the feature is not in any release of that line yet.

  To find the version of an already released feature, check `@since` in JSDoc in the source, the [release notes](https://github.com/nuxt/nuxt/releases), or the [Nuxt blog](https://nuxt.com/blog). An `@since unreleased` tag means the feature has not shipped in any release yet, so the badge should say `unreleased` too.
