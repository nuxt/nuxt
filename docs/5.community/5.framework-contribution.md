...
When you add a new public API — a composable, utility, component, or kit function — record the version it arrived in with `@since` in its JSDoc, next to the code:
...
Write `unreleased` rather than a version number. You don't need to know which release will carry your pull request, or whether it will be backported: the release that ships it replaces the placeholder with its own version, on each branch separately. The [version badges in the docs](/docs/4.x/community/contribution#documentation-style-guide) work the same way.
