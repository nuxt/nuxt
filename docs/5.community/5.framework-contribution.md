...
Once you've read the [general contribution guide](/docs/community/contribution), here are some specific points on contributions to the `nuxt/nuxt` repository.
...
While working on a PR, you will likely want to check your changes work correctly.

You can modify the example app in `playground/`, and run it with `yarn dev`. Please make sure not to commit it to your branch though it could be helpful if you add some example code to your PR description. This can help reviewers and other Nuxt users understand the feature you've built in-depth.
...
You might have already noticed that we use ESLint to enforce a coding standard. Please run `yarn lint` before committing your changes to verify the code style is correct. If not, you can use `yarn lint --fix` to fix most style changes. If there are still errors left, you must correct them manually.

### Documentation

If you are adding a new feature or refactoring/changing the behavior of Nuxt in any other manner, you'll likely want to document the changes. Please include any changes to the docs in the same PR. You don't have to write documentation up on the first commit (but please do so as soon as your pull request is mature enough). Please make changes according to the [Documentation Guide](/docs/community/contribution#writing-documentation).
...
If you spot an area where we can improve the documentation or error messages, please open a PR - even if it's only to fix a typo!
...
1. Run `pnpm install` to install the dependencies.
    ...
    > If you're adding a dependency, please use `pnpm add`. The `pnpm-lock.yaml` file is the source of truth for all Nuxt dependencies.
...
::alert
🚧 This repository will be open-sourced shortly. Until then, you'll need to open a pull request to see a preview of your changes.
::

::alert
We recommend installing the [MDC extension](https://marketplace.visualstudio.com/items?itemName=Nuxt.mdc) for VS Code.
::
