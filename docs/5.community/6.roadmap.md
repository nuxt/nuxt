...
Auth Utils                          | Planned             | 3.x          | `nuxt/auth-utils` to be announced | The temporary repository [nuxt-auth-utils](https://github.com/atinux/nuxt-auth-utils) is available while awaiting its official integration into Nuxt via RFC.
