...
`app:beforeMount`      | `vueApp`            | Client          | Called before mounting the app, called only on the client side.
`app:mounted`          | `vueApp`            | Client          | Called when the Vue app is initialized and mounted in the browser.
...
`app:templates`          | `app`                      | Called during `NuxtApp` generation to allow customizing, modifying, or adding new files to the build directory (either virtually or to `.nuxt`).
`app:templatesGenerated` | `app`                      | Called after templates compile into the [virtual file system](https://nuxt.com/docs/guide/directory-structure/nuxt#virtual-file-system) (vfs).
`build:before`           | -                          | Called before Nuxt bundle builder.
`build:done`             | -                          | Called after Nuxt bundle builder is complete.
`build:manifest`         | `manifest`                 | Called during the manifest build by Vite and webpack. That allows customizing the manifest Nitro will use to render `<script>` and `<link>` tags in the final HTML.
...
`pages:extend`           | `pages`                    | Called after page routes get resolved.
`server:devHandler`      | `handler`                  | Called when the dev middleware registers on the Nitro dev server.
...
`components:dirs`        | `dirs`                     | Called within `app:resolve`, allowing to extend the directories scanned for auto-importable components.
...
`nitro:init`             | `nitro`                    | Called after Nitro initialized, allowing registering Nitro hooks and interacting directly with Nitro.
`nitro:build:before`     | `nitro`                    | Called before building the Nitro instance.
`nitro:build:public-assets`     | `nitro`                    | Called after copying public assets. Allows modifying public assets before the Nitro server gets built.
...
`prepare:types`          | `options`                  | Called before Nuxi writes `.nuxt/tsconfig.json` and `.nuxt/nuxt.d.ts`, allowing the addition of custom references and declarations in `nuxt.d.ts`, or directly modifying the options in `tsconfig.json`
...
`schema:resolved`        | `schema`                   | Allows extending the resolved schema.
`schema:beforeWrite`     | `schema`                   | Called before writing the schema.
...
`vite:serverCreated`     | `viteServer, env`          | Called when the Vite server gets created.
`vite:compiled`          | -                          | Called after Vite server is compiled.
`webpack:config`         | `webpackConfigs`           | Called before configuring the webpack compiler.
`webpack:configResolved` | `webpackConfigs`           | Allows reading the resolved webpack config.
`webpack:compile`        | `options`                  | Called before compilation.
`webpack:compiled`       | `options`                  | Called after loading resources.
`webpack:change`         | `shortPath`                | Called on `change` on WebpackBar.
`webpack:error`          | -                          | Called on `done` if it has errors on WebpackBar.
