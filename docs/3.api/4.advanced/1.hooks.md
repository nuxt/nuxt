...
`page:transition:start`|                     | Client          | Called before View Transitions API page transition begins (**experimental**).
`page:transition:finish`| `pageComponent?`    | Client          | After page transition [onAfterLeave](https://vuejs.org/guide/built-ins/transition.html#javascript-hooks) event or View Transitions API page transition (**experimental**) is finished.
