...
- **trailingSlashBehavior**: An option to force trailing slash in all urls or remove by default. If option is `undefined` or different from `"append"`/`"remove"` it will be ignored.
