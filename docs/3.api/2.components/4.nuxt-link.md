...
Nuxt provides a `<NuxtLink>` component to handle any links within your application.
...
In this example, `<NuxtLink>` links to a website.
...
In this example, `<NuxtLink>` links to another page in your application.
...
<template>
  ...
  <NuxtLink to="/contact" target="_blank">
    The contact page opens in another tab
  </NuxtLink>
...
- **noRel**: If set to `true`, no `rel` attribute will get added to the link
- **activeClass**: A class to apply on active links. Like [Vue Router's `active-class` prop](https://router.vuejs.org/api/interfaces/RouterLinkProps.html#Properties-activeClass) on internal links. Defaults to Vue Router's default (`"router-link-active"`)
- **exactActiveClass**: A class to apply on exact active links. Like [Vue Router's `exact-active-class` prop](https://router.vuejs.org/api/interfaces/RouterLinkProps.html#Properties-exactActiveClass) on internal links. Defaults to Vue Router's default `"router-link-exact-active"`)
...
- **external**: Forces the link as external (`true`) or internal (`false`). This is helpful for edge-cases
- **prefetch** and **noPrefetch**: Whether to enable prefetching assets for links that enter the viewport.
- **prefetchedClass**: A class to apply to prefetched links.
- **custom**: Whether `<NuxtLink>` should wrap its content in an `<a>` element. It allows full control of how a link renders and how navigation works when clicked. Works the same as [Vue Router's `custom` prop](https://router.vuejs.org/api/interfaces/RouterLinkProps.html#Properties-custom)
...
You can overwrite `<NuxtLink>` defaults by creating a custom link component using `defineNuxtLink`.
...
You can then use the `<MyNuxtLink />` component as usual with your new defaults.
...
- **prefetchedClass**: A default class to apply to prefetched links.
- **trailingSlash**: An option to add or remove trailing slashes in the `href`. It will get ignored if unset or not matching the valid values `append` or `remove`.
