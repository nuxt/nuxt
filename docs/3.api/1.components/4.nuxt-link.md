...
::note
In addition to the `params` prop, `NuxtLink` accepts all props supported by Vue Router's `RouterLink`, such as `to`, `query`, `hash`, and `path`. See the [Vue Router RouterLink documentation](https://router.vuejs.org/api/#routerlink) for more details.
::
