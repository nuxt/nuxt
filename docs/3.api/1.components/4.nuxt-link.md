...
When linking to static files in the `/public` directory or to another application hosted on the same domain, it’s essential to bypass Vue Router's internal routing mechanism to avoid unexpected 404 errors. In such cases, you can use the `external` prop with `<NuxtLink>`.
...
We now support custom prefetch triggers for `<NuxtLink>` after `v3.13.0`
