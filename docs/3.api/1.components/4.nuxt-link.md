...
`<NuxtLink>` fully leverages Vue Router’s handling of query parameters and automatically URL-encodes all keys and values. You don’t need to call `encodeURI` or `encodeURIComponent` manually—just (doing so could potentially lead to errors) pass an object to the query field.
