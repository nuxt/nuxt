...
- `prefetchMode`: Determines when to prefetch links. Possible options are `on-hover` and `on-visible` (default).
  This prop is only used when `prefetch` is enabled (default) and `noPrefetch` is not set.
