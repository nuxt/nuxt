...
In this exemple, we pass email as a url parameter to the `password-reset.vue`.

::code-group
```vue [pages/index.vue]
<template>
  <NuxtLink :to="{ name: 'password-reset', query: { email: user-email@example.com } }">
    Reset my password
  </NuxtLink>
</template>
```
...
```html [(Renders as) index.html]
<a href="/password-reset?email=user-email@example.com">Reset my password</a>
```
::
