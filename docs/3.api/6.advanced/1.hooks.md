...
`compiler:ready`         | -                          | Called when the Nuxt compiler is ready to be used.
`compiler:dirs`          | `dirs`                     | Called when the compiler is resolving directories to scan. Allows extending the directories.
`compiler:files`         | `files`                    | Called after the compiler has scanned files in specified directories. Allows extending the files to be scanned.
