...
Additionally to the above options, `nuxi` can pass options through to `listhen`, e.g. `--no-qr` to turn off the dev server QR code. You can find the list of `listhen` options in the README of the [listhen repository](https://github.com/unjs/listhen).
