...
### Setting Global Defaults for Custom Page Metadata

You can set global default values for your custom page metadata keys using the `app.customPageMeta` property in your `nuxt.config`. These defaults will be merged with any local values defined via `definePageMeta` in your page components. Local values always take precedence over global defaults.

For example, to set a global default for a custom `pageType` key:
...
Now, unless you override `pageType` in a specific page, all pages will have `pageType: 'default'` in their `$route.meta`.
