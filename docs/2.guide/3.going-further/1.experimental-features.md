...
Caches Nuxt/Nitro build artifacts based on a hash of the configuration and source files.
...
In addition, any changes to files within `srcDir` will trigger a rebuild of the Vue client/server bundle, and any files changed within `serverDir` (as well as any changes to the Vue client/server bundle) will trigger a rebuild of the Nitro server.
...
::note
A maximum of 10 cache tarballs are kept (note that Vue + Nitro builds are cached separately, so a fresh build potentially produces two new cache tarballs).
::
