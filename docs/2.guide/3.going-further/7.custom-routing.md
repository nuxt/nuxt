...
In Nuxt 3, your routing is defined by the structure of your files inside the [pages directory](/docs/guide/directory-structure/pages). However, since it uses [vue-router](https://router.vuejs.org/) under the hood, Nuxt offers you several ways to add custom routes to your project.
...
If it returns `null` or `undefined`, Nuxt will fall back to the default routes, which is useful when modifying the input array.
...
If you plan to add a whole set of pages related to a specific functionality, you might want to use a [Nuxt module](/modules).
...
You can enable hash history in SPA mode. In this mode, the router uses a hash character (#) before the URL that is passed internally. When enabled, the **URL is never sent to the server** and **SSR is not supported**.
...
You can optionally override history mode using a function that accepts the base URL and returns the history mode. If it returns `null` or `undefined`, Nuxt will fall back to the default history.
