...
Nuxt Modules come with their own builder provided by [`@nuxt/module-builer`](https://github.com/nuxt/module-builder#readme). This builder doesn't require any configuration on your end, supports TypeScript, and makes sure your assets are properly bundled to be distributed to other Nuxt applications.
...
- Published modules, they are modules published on npm, see a list of them on [the modules page](/modules).
...
Outside of really short modules, **we do not recommend** using this low-level function definition. Instead, to define a module, **we recommend** using the higher-level `defineNuxtModule` helper provided by [Nuxt Kit](/docs/api/advanced/kit).
...
::alert{type=warning}
Published modules cannot leverage auto-imports for assets within their runtime directory. Instead, they have to import them explicitly from `#app` or alike.
...
Because modules aren't part of the application runtime, their options aren't too. However, in many cases, you might want to expose module options to the runtime application. We recommend you use Nuxt's [`runtimeConfig`](/docs/api/configuration/nuxt-config#runtimeconfig) for the purpose.
...
If your module should provide other kinds of assets, they can also be injected. Here's an example module injecting a stylesheet.
...
With great power comes great responsibility. While module are powerful, here are some best practices to keep in mind while authoring modules to keep applications performant and developer experience great.
...
**Other community modules** are modules prefixed with `nuxt-`. Anyone can make them, using this prefix allows these modules to be discoverable on npm. This is the best starting point to draft and try an idea!
