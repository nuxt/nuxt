...
Set the render mode of your page.

You can define the page as [server only](/docs/guide/directory-structure/components#server-components) by setting the value to `'server'`. This is an alternative to the `.server.vue` suffix in the filename.

::alert{type=warning}
Within a server page, you will not have access to any query parameters.
::
