...
By default, Nuxt will create a page for all files ending with `extensions` configured in `nuxt.config`, default `[".js",".jsx",".mjs",".ts",".tsx",".vue"]`.

If you wish to remove pages from the auto generation this can be done via the `pages:extend` nuxt hook. The example below removes `.ts` files from page generation.
```ts [nuxt.config.ts]
export default defineNuxtConfig({
  hooks: {
    'pages:extend' (pages) {
