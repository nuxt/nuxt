...
This means that when the route is server-rendered or statically generated, you will be able to see its contents correctly, but when you navigate to that route during client-side navigation, the transition between routes will fail and the route won't render.

Here are some examples to illustrate what a page with/without a single root element looks like:
...
If you place anything within square brackets, it will turn into a [dynamic route](https://router.vuejs.org/guide/essentials/dynamic-matching.html) parameter. You can mix and match multiple parameters and even non-dynamic text within a file/directory name.
...
If you want more control over when the `<NuxtPage>` component gets re-rendered (for example, to trigger a transition under specific conditions), you can pass a string variable or a function via the `<NuxtPage>`'s `pageKey` prop or a `key` value in `definePageMeta`. Whenever the string variable or the function's return value changes, re-render occurs:
...
You can define page aliases. They allow you to access the same page from different paths. It can be a string or an array of strings as defined in the [Vue Router docs](https://router.vuejs.org/guide/essentials/redirect-and-alias.html#alias).
...
If you want to preserve the state for parent routes, use this syntax: `<NuxtPage keepalive />`. You can also pass props to `<KeepAlive>` (see the entire list [here](https://vuejs.org/api/built-in-components.html#keepalive)).

You can set a default value for this property in the [`nuxt.config` file](/docs/api/configuration/nuxt-config#keepalive).
...
You can define middleware to apply before loading the page. It will merge with all the other middleware used in matching parent/child routes. It can be a string, a function (an anonymous/inlined middleware function following the [global before guard pattern](https://router.vuejs.org/guide/advanced/navigation-guards.html#global-before-guards)), or an array of strings/functions. [More about named middleware](/docs/guide/directory-structure/middleware).

#### `name`

You may define a name for the page's route.
...
Nuxt 3 allows programmatic navigation through the `navigateTo()` utility method. It is great for taking input from the user and navigating them dynamically throughout your application. In this example, we have a simple method called `navigate()` that gets called when the user submits a search form.
...
As your app gets bigger and more complex, your routing might require more flexibility. For this reason, Nuxt exposes the router options, `pages:extend` hook, and utility functions for module authors, allowing for customization in different ways.
