...
::alert
Any nested directories must get added first, as they are scanned in order.
::
...
If you need to restrict the file extensions that should get registered as components, you can use the extended form of the components directory declaration and its `extensions` key:
...
... then, the component's name will be based on its path directory and filename, with duplicate segments removed. Therefore, the component's name will be:
...
::alert
For clarity, we recommend that the component's filename matches its name. (In the example above, you could rename `Button.vue` to `BaseFooButton.vue`.)
::

If you want to auto-import components based only on their name, not the path, then you need to set the `pathPrefix` option to `false` using the extended form of the configuration object:
...
Alternatively, though not recommended, you can register all your components globally, creating async chunks for all your components and making them available throughout your application.
...
To dynamically import a component (also known as lazy-loading a component), you need to add the `Lazy` prefix to the component's name.
...
This is particularly useful if the component is not always needed. Using the `Lazy` prefix, you can delay loading the component code until the right moment, which can help optimize your JavaScript bundle size.
...
Nuxt provides the `<ClientOnly>` component for purposely rendering a component only on the client side. To import a component only on the client, register the component in a client-side only plugin.
...
Use a slot as a fallback until `<ClientOnly>` is mounted on the client side.
...
If a component is to be rendered only on the client side, you can add the `.client` suffix to it.
...
::alert{type=warning}
This feature only works with Nuxt auto-imports and `#components` imports. Explicitly importing these components from their paths does not make them client-only.
::
...
Standalone server components will always be rendered on the server. When their props update, this will result in a network request that updates the rendered HTML in place.
...
Server components are currently experimental and to use them you must enable the component islands feature in your `nuxt.config` file:
...
In this case, the `.server` + `.client` components are two 'halves' of a component and can be used in advanced use cases for separate implementations of a component on the server and client side.
...
::alert{type=warning}
It is essential that the client half of the component can 'hydrate' the server-rendered HTML. That is, it should render the same HTML on the initial load, or you will experience a hydration mismatch.
::
...
It will automatically import the components only if used and support HMR when updating your components in `node_modules/awesome-ui/components/`.
