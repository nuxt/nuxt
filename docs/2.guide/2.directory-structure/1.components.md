...
In order to optimize the page, you may want to delay the hydration of some components until they're visible, or until the browser is done with more important tasks. Delaying the hydration of components will ensure they are only loaded when necessary, which is great for user experience and app performance. Nuxt supports lazy (or delayed) hydration, allowing you to control when components become interactive.
...
You can customize the behavior of each hydration strategy by passing specific values to the props.
...
2.  **Conditional Rendering:** When using `v-if="false"` on a lazy component, you might not need delayed hydration. You can use a delayed hydration prop.
...
5.  **Avoid `LazyNever` on interactive components:** If a component requires user interaction, it should not be set to never hydrate.
