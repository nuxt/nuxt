...
::alert{type=warning}
Legacy support is possible using [unjs/h3](https://github.com/unjs/h3), though it's advised to avoid legacy handlers as much as possible.
::
...
::alert{type=warning}
Never combine the `next()` callback with a legacy middleware that is `async` or returning a `Promise`!
::
