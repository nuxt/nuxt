...
For adding server routes without the `/api` prefix, you can instead put them into the `~/server/routes` directory.
...
Nuxt will automatically read any file in the `~/server/middleware` directory to create server middleware for your project.

Middleware handlers run on every request before any server routes to add or check headers, log requests, or extend the event's request object.
...
Server routes are powered by [H3](https://github.com/unjs/h3), which comes with a handy set of helpers.

::ReadMore{link="https://www.jsdocs.io/package/h3#package-index-functions" title="Available H3 Request Helpers"}
::

You can add more helpers inside the `~/server/utils` directory.
...
If no errors get thrown, the HTTP `200 OK` success status response code is returned. Any uncaught errors return the HTTP `500 Internal Server Error` response code.

To return other error response codes, throw an exception with `createError`.
...
You can use the `nitro` key in `nuxt.config` to directly set [Nitro configuration](https://nitro.unjs.io/config).

::alert{type=warning}
This is an advanced option. Custom configuration can affect production deployments, as the configuration interface might change over time when Nitro gets upgraded in semver-minor versions of Nuxt.
::
...
**Note:** This is an experimental feature, only available within Node.js environments.
...
::alert{type=warning}
Legacy support is possible using [H3](https://github.com/unjs/h3), though it's advised to avoid legacy handlers as much as possible.
::
...
::alert{type=warning}
Never combine the `next()` callback with a legacy middleware that is async or returns a `Promise`!
::

### Server Storage

Nitro provides a cross-platform [storage layer](https://nitro.unjs.io/guide/storage). To configure additional storage mount points, you can use `nitro.storage`.
