...
Nuxt will automatically read any file in the `~/server/middleware` to create server middleware for your project.
...
If no errors get thrown, the HTTP `200 OK` success status response code will be returned. Any uncaught errors will return the HTTP `500 Internal Server Error` response code..
