...
## Server / Client Only Composables

Using environment-specific composables is a useful way to avoid leaking server only code on the client and vice versa,
allowing you to keep your bundle size small.

Similar to how you can specify [components](/docs/guide/directory-structure/components#client-components) and [plugins](/docs/guide/directory-structure/plugins#plugins-directory) as server or client only,
you can also specify composables in the same way.
...
Note: This will only work with composables when you aren't using their return value.

```vue [pages/index.vue]
<template lang="ts" setup>
// ✅ This works
useWindowResize((e) => { console.log(e) }) 
</template>
````
