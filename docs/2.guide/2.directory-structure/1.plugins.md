...
By default, Nuxt loads plugins sequentially. You can define a plugin as `parallel` so Nuxt don't wait the end of the plugin's execution before loading the next plugin.
