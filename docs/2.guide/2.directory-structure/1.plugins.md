...
If a plugin needs to await a parallel plugin before it runs, you can add the plugin's name to the `dependsOn` array or callback.

```ts [plugins/depending-on-my-plugin.ts]
export default defineNuxtPlugin({
  name: 'depends-on-my-plugin',
  dependsOn: ['my-plugin'], // or () => ['my-plugin']
  async setup (nuxtApp) {
