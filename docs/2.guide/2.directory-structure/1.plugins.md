...
If you want to use Vue plugins, like [vue-gtag](https://github.com/MatteoGabriele/vue-gtag) to add Google Analytics tags, you do not need to add them to the plugins directory or register them in nuxt.config.ts. Simply install them and then import them where you wish to use them:

```ts
import vue-gtag from "vue-gtag"
```
