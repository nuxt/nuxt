...
* Any time you access process.env in `nuxt.config.ts` you will receive the baked in values from build time. The only way to override runtime variables is by using the `NUXT_MY_VAR` syntax to override `runtimeConfig`.
