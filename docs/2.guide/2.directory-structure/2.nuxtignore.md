...
The `.nuxtignore` file lets Nuxt ignore `layouts`, `pages`, `components`, `composables`, and `middleware` files in your project’s root directory (`rootDir`) during the build phase. The `.nuxtignore` file is subject to the same specification as the `.gitignore` and `.eslintignore` files, in which each line is a glob pattern indicating which files to ignore.
...
# ignore route middleware files under the foo folder except for foo/bar.js
middleware/foo/*.js
