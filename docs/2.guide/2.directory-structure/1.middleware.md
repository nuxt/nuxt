...
### Running middleware from plugins

Instead of using `definePageMeta` on each page, it may be handy to apply a named route middleware on a group of routes. This can be easily done by calling a named route middleware within a plugin.

```js [plugin/applyMiddleware.js]
...
export default defineNuxtPlugin({
  hooks: {
    "app:created"() {
      ...
      const nuxtApp = useNuxtApp();
      const currentRoute = nuxtApp.payload.path;
