...
::alert{type=info}
Route middleware runs within the Vue part of your Nuxt app. Despite the similar name, it differs from server middleware which is ran in the Nitro server part of your app.
::
...
3. Global route middleware, placed in the `middleware/` directory (with a `.global` suffix) and ran automatically on every route change.
...
1. Global Middleware
2. Page-defined middleware order (if there are multiple middlewares declared with the array syntax)
...
Now, before the navigation to that page can complete, the `auth` route middleware will be ran.
