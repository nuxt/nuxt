...
However, there are some differences with the actual spec; here's a [helpful explainer](https://hacks.mozilla.org/2018/03/es-modules-a-cartoon-deep-dive).
...
When using modules you'd install into your package, things were a little different. A sample library might expose both CJS and ESM versions and let us pick the one we want:
...
However, in recent Node.js LTS releases, it is now possible to use native [ESM modules](https://nodejs.org/api/esm.html) within Node.js. That means Node.js can process JavaScript using ESM syntax, although it doesn't do it by default. The two most common ways to enable ESM syntax are:

- use the `.mjs` file extensions (recommended)
- set `type: 'module'` within your `package.json` and keep using the `.js` extension

This is what we do for Nuxt's Nitro; we output a `.output/server/index.mjs` file. That tells Node.js to treat this file as a native ES module.
...
1. files ending in `.mjs` - are expected to use ESM syntax
1. files ending in `.cjs` - are expected to use CJS syntax
1. files ending in `.js` - are expected to use CJS syntax unless their `package.json` has `type: 'module'`
...
However, if you try to import a package with a `.esm.js` file in a Node.js ESM context, it won't work, and you'll get an error similar to this:
...
You may find that you _also_ need to add other packages imported by these libraries.

### Aliasing Libraries

In some cases, you may also need to manually alias the library for the CJS version, for example:
...
- Use the new `exports` field with conditional exports. ([Read more](https://nodejs.org/api/packages.html#conditional-exports)).
