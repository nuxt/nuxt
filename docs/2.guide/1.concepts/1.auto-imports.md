...
Nuxt auto-imports its helper functions, composables, and Vue APIs, making them available across your application without explicit imports. Auto-imports also include directory-based contents of your Nuxt application, including components, composables, plugins, and utils.

Contrary to a classic global declaration, Nuxt preserves typings and IDE's completions and hints and only includes what is used in your production code.
...
Vue 3 exposes Reactivity APIs like `ref` and `computed` as well as lifecycle hooks and helpers that are auto-imported by Nuxt.
...
That will disable implicit auto-imports entirely but will keep the option of [explicit imports](#explicit-imports).
