...
The default files `index.html`, `200.html` and `404.html` are part of Nuxt and ofter helpfull, but if by any means you need to avoid having those files on your build you can need to overwrite its generation by using the `"prerender:generate"` hook from [Nitro](https://nuxt.com/docs/getting-started/prerendering#prerendergenerate-nitro-hook).
