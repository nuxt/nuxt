...
- `swr` - Server-side renders routes and add cache headers in the response for the server or the reverse proxy to cache it for a configurable TTL. The `node-server` preset of Nitro is able to cache the full response. For Netlify and Vercel, the response is also added to the CDN layer.
