...
- **Offline:** Because code entirely runs in the browser, it can keep working while the internet is unavailable.
...
::alert{type=warning}
When importing a library that relies on browser APIs and has side effects, make sure the component importing it is only called client-side. Bundlers do not tree-shake imports of modules containing side effects.
::
