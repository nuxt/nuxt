...
- `swr` - Server-side renders sections of your app and add cache headers in the response for the server or the reverse proxy to cache it for a configurable TTL. The node-server preset of Nitro is able to cache the response. For Netlify and Vercel, the response is also added to the CDN layer.
- `static` - The behavior is the same as `swr` except that there is no TTL: the response is cached until the next deployment. On Netlify, it enables full incremental static generation. Vercel is coming soon.
- `prerender` - Fetches your app sections and copies them as static assets during the build
