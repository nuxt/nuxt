...
Nuxt provides the ability to deploy your application into a [static hosting](getting-started/deployment#static-hosting). But when you use `ssr: false` and run `nuxi generate`, it will produce all the pages as static HTML files. In a common SPA we only need one single `index.html` file. In order to achieve this we can change how the routes are prerendered. Just add this to your hooks in your `nuxt.config.ts`:
