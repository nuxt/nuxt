...
Let's create a custom `$fetch` instance with a [Nuxt utility](/docs/guide/directory-structure/utils).
...
With this Nuxt utility, `$api` is auto-imported to make API calls directly from the Vue components:
