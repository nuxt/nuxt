...
describe('errors', () => {
  it('constructs an error the server runtime recognises as its own', () => {
    const error = createError({ status: 404, statusMessage: 'Not Found' })
  ...
  it('recognises an error thrown by the server runtime, which carries no Nuxt signature', () => {
    ...
    // h3 v1 marks its errors on the constructor and leaves `name` alone
    class H3Error extends Error {
      static __h3_error__ = true
      status = 418
    }
    expect(isNuxtError(new H3Error('teapot'))).toBe(true)
  })
