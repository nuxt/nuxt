...
function createWatcher () {
  ...
  watcher.on('all', (event, path) => nuxt.callHook('builder:watch', event, nuxt.options.future.compatibilityVersion === 4 ? normalize(path) : normalize(relative(nuxt.options.srcDir, path))))
...
function createGranularWatcher () {
  ...
  for (const dir of pathsToWatch) {
    ...
    watcher.on('all', (event, path) => {
      ...
      if (!pending) {
        // TODO: consider moving to emit absolute path in 3.8 or 4.0
        nuxt.callHook('builder:watch', event, nuxt.options.future.compatibilityVersion === 4 ? path : relative(nuxt.options.srcDir, path))
      }
      ...
      if (event === 'addDir' && path !== dir && !ignoredDirs.has(path) && !pathsToWatch.includes(path) && !(path in watchers) && !isIgnored(path)) {
        ...
        // TODO: consider moving to emit absolute path in 3.8 or 4.0
        watchers[path].on('all', (event, p) => nuxt.callHook('builder:watch', event, nuxt.options.future.compatibilityVersion === 4 ? normalize(p) : normalize(relative(nuxt.options.srcDir, p))))
  ...
  for (const layer of nuxt.options._layers) {
    ...
    const watcher = subscribe(layer.config.srcDir, (err, events) => {
      ...
      for (const event of events) {
        ...
        // TODO: consider moving to emit absolute path in 3.8 or 4.0
        nuxt.callHook('builder:watch', watchEvents[event.type], nuxt.options.future.compatibilityVersion === 4 ? normalize(event.path) : normalize(relative(nuxt.options.srcDir, event.path)))
      }
    }, {
      ignore: [
        ...nuxt.options.ignore,
