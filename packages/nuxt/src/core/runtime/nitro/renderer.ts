...
const getEntryIds: () => Promise<string[]> = () =>
...
const getServerEntry = () =>
  import("#build/dist/server/server.mjs").then((r) => r.default || r);

// @ts-expect-error file will be produced after app build
const getSSRStyles = lazyCachedFunction(
  (): Promise<Record<string, () => Promise<string[]>>> =>
    import("#build/dist/server/styles.mjs").then((r) => r.default || r)
);
...
const getSSRRenderer = lazyCachedFunction(async () => {
  ...
  async function renderToString(
    input: RenderToStringParams[0],
    context: RenderToStringParams[1]
  ) {
...
const getSPARenderer = lazyCachedFunction(async () => {
  ...
  // @ts-expect-error virtual file
  const spaTemplate = await import("#spa-template")
    .then((r) => r.template)
    .catch(() => "")
...
const payloadCache = import.meta.prerender
  ? useStorage("internal:nuxt:prerender:payload")
  : null;
const islandCache = import.meta.prerender
  ? useStorage("internal:nuxt:prerender:island")
  : null;
const islandPropCache = import.meta.prerender
  ? useStorage("internal:nuxt:prerender:island-props")
  : null;
const sharedPrerenderPromises =
  import.meta.prerender && process.env.NUXT_SHARED_DATA
    ...
    ? {
        get<T = unknown>(key: string): Promise<T> | undefined {
          if (sharedPrerenderKeys.has(key)) {
            return (
              sharedPrerenderPromises!.get(key) ??
              (useStorage("internal:nuxt:prerender:shared").getItem(
                key
              ) as Promise<T>)
            );
          }
        ...
        async set<T>(key: string, value: Promise<T>): Promise<void> {
          ...
          useStorage("internal:nuxt:prerender:shared")
            .setItem(key, (await value) as any)
...
async function getIslandContext(event: H3Event): Promise<NuxtIslandContext> {
  ...
  if (
    ...
    event.path &&
    (await islandPropCache!.hasItem(event.path))
  ) {
  ...
  const componentParts = url
    .substring("/__nuxt_island".length + 1)
...
const PAYLOAD_URL_RE = process.env.NUXT_JSON_PAYLOADS
  ? /\/_payload.json(\?.*)?$/
  : /\/_payload.js(\?.*)?$/;
const ROOT_NODE_REGEX = new RegExp(
  `^<${appRootTag}[^>]*>([\\s\\S]*)<\\/${appRootTag}>$`
);
...
export default defineRenderHandler(
  async (event): Promise<Partial<RenderResponse>> => {
    ...
    if (
      ssrError &&
      !("__unenv__" in event.node.req) /* allow internal fetch */
    ) {
      throw createError({
        status: 404,
    ...
    const isRenderingIsland =
      (componentIslands as unknown as boolean) &&
      event.path.startsWith("/__nuxt_island");
    const islandContext = isRenderingIsland
    ...
    if (
      ...
      islandContext &&
      event.path &&
      (await islandCache!.hasItem(event.path))
    ) {
      return islandCache!.getItem(event.path) as Promise<
        Partial<RenderResponse>
      >;
    }
    ...
    const _PAYLOAD_EXTRACTION =
      import.meta.prerender &&
      process.env.NUXT_PAYLOAD_EXTRACTION &&
      !ssrContext.noSSR &&
      !isRenderingIsland;
    const payloadURL = _PAYLOAD_EXTRACTION
      ? joinURL(
          ssrContext.runtimeConfig.app.cdnURL ||
            ssrContext.runtimeConfig.app.baseURL,
          url,
          process.env.NUXT_JSON_PAYLOADS ? "_payload.json" : "_payload.js"
        ) +
        "?" +
    ...
    // Render app
    const renderer =
      process.env.NUXT_NO_SSR || ssrContext.noSSR
    ...
    const _rendered = await renderer
      ...
      .catch(async (error) => {
        ...
        if (ssrContext._renderResponse && error.message === "skipping render") {
          return {} as ReturnType<(typeof renderer)["renderToString"]>;
        }
    ...
    const inlinedStyles =
      process.env.NUXT_INLINE_STYLES || isRenderingIsland
        ? await renderInlineStyles(ssrContext.modules ?? [])
        : [];

    const NO_SCRIPTS =
      process.env.NUXT_NO_SCRIPTS || routeOptions.experimentalNoScripts;
    ...
    if (!isRenderingIsland || import.meta.dev) {
      ...
      for (const style in styles) {
        ...
        if (
          !import.meta.dev ||
          !isRenderingIsland ||
          (resource.file.includes("scoped") &&
            !resource.file.includes("pages/"))
        ) {
          link.push({
            rel: "stylesheet",
    ...
    if (!routeOptions.experimentalNoScripts && !isRenderingIsland) {
      head.push(
        {
          script: Object.values(scripts).map(
            (resource) =>
              <Script>{
    ...
    if (isRenderingIsland && islandContext) {
      ...
      for (const tag of await head.resolveTags()) {
        if (tag.tag === "link") {
          islandHead.link.push({
            key: "island-link-" + hash(tag.props),
            ...tag.props,
          });
        } else if (tag.tag === "style" && tag.innerHTML) {
          islandHead.style.push({
      ...
      if (import.meta.prerender) {
        await islandCache!.setItem(
          `/__nuxt_island/${islandContext!.name}_${islandContext!.id}.json`,
          response
        );
        await islandPropCache!.setItem(
          `/__nuxt_island/${islandContext!.name}_${islandContext!.id}.json`,
          event.path
        );
      }
...
async function renderInlineStyles(
  usedModules: Set<string> | string[]
): Promise<Style[]> {
...
function renderPayloadResponse(ssrContext: NuxtSSRContext) {
  return {
    ...
    headers: {
      "content-type": process.env.NUXT_JSON_PAYLOADS
...
function renderPayloadScript(opts: {
  ...
  const _PAYLOAD_EXTRACTION =
    import.meta.prerender &&
    process.env.NUXT_PAYLOAD_EXTRACTION &&
    !opts.ssrContext.noSSR;
  if (_PAYLOAD_EXTRACTION) {
    return [
      {
        type: "module",
        innerHTML: `import p from "${
          opts.src
        }";window.__NUXT__={...p,...(${devalue(opts.data)})}`,
      },
    ];
  }
  return [
...
function getSlotIslandResponse(
  ssrContext: NuxtSSRContext
): NuxtIslandResponse["slots"] {
  if (
    !ssrContext.islandContext ||
    !Object.keys(ssrContext.islandContext.slots).length
  ) {
    return undefined;
  }
...
function getClientIslandResponse(
  ssrContext: NuxtSSRContext
): NuxtIslandResponse["components"] {
  if (
    !ssrContext.islandContext ||
    !Object.keys(ssrContext.islandContext.components).length
  ) {
    return undefined;
  }
...
function replaceIslandTeleports(ssrContext: NuxtSSRContext, html: string) {
  ...
  for (const key in teleports) {
    ...
    if (matchClientComp) {
      ...
      html = html.replace(
        new RegExp(` data-island-component="${clientId}"[^>]*>`),
        (full) => {
    ...
    if (matchSlot) {
      ...
      html = html.replace(
        new RegExp(
          ` data-island-uid="${uid}" data-island-slot="${slot}"[^>]*>`
        ),
        (full) => {
          return full + teleports[key];
        }
