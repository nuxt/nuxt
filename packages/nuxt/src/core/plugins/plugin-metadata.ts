  ...
  parseAndWalk(code, `file.${loader}`, (node) => {
    ...
    if (metaArg) {
      if (metaArg.type !== 'ObjectExpression') {
        buildErrorUtils.throw({ message: `Invalid plugin metadata: the second argument to \`${name}\` must be an object literal, but got \`${metaArg.type}\`.`, code: ErrorCodes.B2001, fix: 'Pass an object literal as the second argument, e.g. `defineNuxtPlugin(() => {}, { name: \'my-plugin\' })`.' })
      }
...
function extractMetaFromObject (properties: Array<ObjectPropertyKind>) {
  ...
  for (const property of properties) {
    if (property.type === 'SpreadElement' || !('name' in property.key)) {
      buildErrorUtils.throw({ message: 'Invalid plugin metadata: spread elements and computed keys are not supported in plugin options.', code: ErrorCodes.B2002, fix: 'Use static properties instead.' })
    }
