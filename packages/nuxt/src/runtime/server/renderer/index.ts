...
import { traceAsync } from '../../../app/internal/tracing.ts'

import { APP_ROOT_CLOSE_TAG, APP_ROOT_OPEN_TAG } from './build-files'

import { renderPayloadJsonScript, renderPayloadResponse, renderPayloadScript, splitPayload } from './payload'
import { createSSRContext, returnRenderResponse, setSSRError } from './app'
import { renderInlineStyles } from './inline-styles'
import { createInlinedCSSFilter } from './inlined-css'
import { throwIfUnmatchedPagePath } from './early-404'
...
function fetch (instance: NuxtRendererInstance, event: RendererEvent): Promise<Response> {
  ...
  if (isErrorRoute && !getRequestState(event)?.['~rendering-error']) {
    if (inlineErrors) {
      return renderErrorResponse(instance, event, runtime.createError({
        status: 404,
...
async function renderErrorResponse (instance: NuxtRendererInstance, event: RendererEvent, error: unknown): Promise<Response> {
  ...
  if (import.meta.dev && devError && !import.meta.test) {
    const html = typeof body === 'string' ? body : await new Response(body).text()
  ...
  const status = rendered.statusCode ?? event.res.status
  return runtime.createResponse(body, {
    status: status && status !== 200 ? status : described.status,
    statusText: rendered.statusMessage || described.statusText,
    headers,
  })
}
...
async function toSSRError (described: DescribedError, error: unknown, event: RendererEvent): Promise<NuxtPayload['error'] & { url: string }> {
  const candidate = (error || {}) as { fatal?: boolean, unhandled?: boolean, stack?: string, cause?: unknown }
  const ssrError = {
    status: described.status,
    statusText: described.statusText,
    statusMessage: described.statusText,
    message: described.message,
    data: PARSE_ERROR_DATA ? described.data : stringifyErrorData(described.data),
    fatal: candidate.fatal ?? false,
    unhandled: candidate.unhandled,
    url: event.url.pathname + event.url.search + urlHash(event.url),
 ...
 * an empty SPA shell.
 */
function getPrerenderedErrorPage (event: RendererEvent): (NuxtPayload['error'] & { url: string }) | null {
...
async function renderRoute (instance: NuxtRendererInstance, event: RendererEvent, ssrError: (NuxtPayload['error'] & { url: string }) | null): Promise<RenderedResponse> {
  const runtime = instance.options
  const hooks = runtime.hooks()
  const hookEvent = appEvent(event)
  const payloadCache = runtime.prerender?.payloadCache
  ...
  if (routeOptions.ssr === false && !(NUXT_INLINE_ERROR_RENDERING && ssrError)) {
  ...
  if (isRenderingPayload) {
    ...
    if (import.meta.prerender && payloadCache && await payloadCache.hasItem(cacheKey)) {
      return (await payloadCache.getItem(cacheKey)) as RenderedResponse
    }
  }
  ...
  if (NUXT_SSR_STREAMING && canStream && renderRouteContext.prefersStream) {
    const streamArgs = { instance, event, ssrContext, renderer, routeOptions, ssrError, _PAYLOAD_EXTRACTION, _PAYLOAD_INLINE, payloadURL }
  ...
  // TODO: remove _renderResponse in nuxt v5
  const inlinedStyles = (NUXT_INLINE_STYLES || ((NUXT_NO_SCRIPTS || NUXT_HAS_NO_SCRIPTS_ROUTES) && NO_SCRIPTS)) && !ssrContext['~renderResponse'] && !ssrContext._renderResponse && !isRenderingPayload
    ? await renderInlineStyles(ssrContext.modules ?? [])
    : []

  if (import.meta.prerender && ssrError && isPrerenderedErrorPage(event.url.pathname)) {
  ...
  if (_PAYLOAD_EXTRACTION && import.meta.prerender) {
    ...
    if (payloadCache) {
      await payloadCache.setItem(getPayloadCacheKey(ssrContext.url), renderPayloadResponse(ssrContext, event))
    }
  }
...
async function renderStreamedResponse (ctx: {
  ...
  if (!NO_SCRIPTS && !import.meta.dev && iifeChunkFileName) {
    ssrContext.head.push({
      link: [{ rel: 'preload', as: 'script', href: runtime.buildAssetsURL(iifeChunkFileName) }],
    })
  }
  ...
  const createSSRApp = await instance.getServerApp()
  let vueApp
  try {
  ...
  if (import.meta.dev) {
    ...
    await hooks.callHook('render:html', shellContext, { event: hookEvent, streaming: true })
  ...
  const enqueueChunk = async (controller: ReadableStreamDefaultController<Uint8Array>, chunk: Uint8Array): Promise<void> => {
    ...
    await hooks.callHook('render:html:chunk', chunkContext, { event: hookEvent })
    ...
    async start (controller) {
      try {
        ...
        if (committedSnapshot) {
          ...
          const currentStatusText = event.res.statusText
          ...
          if (lateMutations.length) {
            rendererDiagnostics.NUXT_E8002({ mutations: lateMutations.join('\n  - '), path: event.url.pathname })
          }
        }
      } catch (error) {
  ...
  return {
    body: outputStream as unknown as RenderedResponse['body'],
  }
}
...
function snapshotResponseHeaders (event: RendererEvent): string {
  return Array.from(event.res.headers.entries())
    .map(([k, v]) => `${k}: ${v}`)
    .sort()
    .join('\n')
}
...
function entryImportMapPath (instance: NuxtRendererInstance, event: RendererEvent, ssrContext: NuxtSSRContext): string {
