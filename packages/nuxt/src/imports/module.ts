...
function addDeclarationTemplates (ctx: Unimport, options: Partial<ImportsOptions>) {
  ...
  // Helper function to filter shared imports
  ...
  // Add shared imports declaration file
  addTypeTemplate({
    ...
    getContents: async () => {
      ...
      // Filter imports that come from shared directories
      const sharedImports = allImports.filter(isSharedImport)

      if (sharedImports.length === 0) {
        return '// No shared imports found'
      }

      // Create a temporary context with only shared imports
      const sharedCtx = createUnimport({
        imports: sharedImports,
      })

      return '// Generated by auto imports for shared utilities\n' + (
        options.autoImport
          ? await sharedCtx.generateTypeDeclarations({ resolvePath: r })
          : '// Implicit auto importing is disabled for shared imports.'
      )
    },
  }, { shared: true })
}
