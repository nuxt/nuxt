 ...
 *
 * Returns an `unfreeze` function that restores the original `push`. The same
 * head instance is kept throughout, so registered head plugins/transformers
 * stay attached. This intentionally avoids mutating any head that could be
 * shared with a concurrent route render (the regression behind #32100, where
 * Vue's module-global `currentInstance` could resolve `injectHead()` to a
 * different request's head and `clear()` would wipe its entries).
 */
