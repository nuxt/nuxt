  ...
  async setup (props, { slots, expose }) {
    ...
    async function fetchComponent (force = false) {
      ...
      try {
        ...
        ssrHTML.value = res.html.replaceAll(DATA_ISLAND_UID_RE, (full, match) => {
          if (match) {
            // avoid replacing uid that are already set. This can happen with nested islands
            return full
          }
          return `data-island-uid="${uid.value}"`
        })
