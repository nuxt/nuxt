...
export const addRouteMiddleware: AddRouteMiddleware = (
...
export const navigateTo = (
  ...
  const toPath =
    typeof to === "string"
      ? to
      : withQuery((to as RouteLocationPathRaw).path || "/", to.query || {}) +
        (to.hash || "");
    ...
    if (nuxtApp.ssrContext) {
      ...
      if (!isExternal && inMiddleware) {
        router.afterEach((final) =>
          final.fullPath === fullPath ? redirect(false) : undefined
        );
        return to;
      }
      return redirect(
        !inMiddleware ? undefined : /* abort route navigation */ false
      );
    }
  }
...
export const setPageLayout = (
  layout: unknown extends PageMeta["layout"] ? string : PageMeta["layout"]
) => {
  const nuxtApp = useNuxtApp();
  if (import.meta.server) {
    if (
