...
/**
 * Shows the development error overlay on a page that is already open. The bundler
 * announces a report over its HMR channel and the dev server renders the overlay.
 */
export default defineNuxtPlugin({
  ...
  setup (nuxtApp) {
    ...
    hot.on('nuxt:dev:error', ({ url, reloadOnClear: reload }: { url: string, reloadOnClear?: boolean }) => {
      ...
      // an overlay already on the page follows the report channel itself
      if (document.querySelector('[data-nuxt-dev-error]')) { return }
      mount(url).catch(() => {})
    })
    ...
    async function mount (url: string) {
      ...
      if (!html || document.querySelector('[data-nuxt-dev-error]')) { return }
