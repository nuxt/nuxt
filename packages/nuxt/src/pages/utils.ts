 ...
 */
export function normalizeRoutesForLazyDiscovery (routes: NuxtPage[], options: NormalizeRoutesOptions & { groupSize: number }): { imports: Set<string>, routes: string, groups: LazyRouteGroup[] } {
  ...
  for (let page of routes) {
    if (options.overrideMeta && isLazyDiscoverable(page) && subtreeHasFile(page)) {
      ...
      serialized.push(stubRoute(page, [groups.length - 1, group.routes.length]))
      group.routes.push(genObjectFromRaw(normalizeRoute(page, group.imports, options) as Record<string, any>))
      group.pages.push(page)
    } else {
      serialized.push(normalizeRoute(page, imports, options))
    }
  }

  return { imports, routes: genArrayFromRaw(serialized), groups }
}
