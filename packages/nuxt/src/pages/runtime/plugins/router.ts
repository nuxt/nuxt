        ...
        for (const entry of middlewareEntries) {
          ...
          try {
            ...
            if (import.meta.server && start) {
              ...
              const name = typeof entry === 'string' ? entry : (middleware as any)._name || (middleware as any).name || 'anonymous'
              nuxtApp.ssrContext!.event.context._nuxt_server_timings ||= []
              nuxtApp.ssrContext!.event.context._nuxt_server_timings.push({ name: `middleware:${name}`, duration })
            }
