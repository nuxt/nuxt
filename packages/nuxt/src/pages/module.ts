...
export default defineNuxtModule({
  ...
  async setup (_options, nuxt) {
    ...
    if (useExperimentalTypedPages) {
      const addedPagePaths = new Map<string, { file: NuxtPage['file'], name: EditableTreeNode['name'] }>()

      const typedRouterDtsPathRelative = 'types/typed-router.d.ts'
      const typedRouterDtsPathAbsolute = resolve(nuxt.options.buildDir, typedRouterDtsPathRelative)
      const readTypedRouterDts = async () => await readFile(typedRouterDtsPathAbsolute, 'utf-8')

      const fileRouteNameMapDtsPathRelative = 'types/typed-pages.d.ts'
      const fileRouteNameMapDtsPathAbsolute = resolve(nuxt.options.buildDir, fileRouteNameMapDtsPathRelative)

      const typedRouterOptions: TypedRouterOptions = {
        routesFolder: [],
        dts: typedRouterDtsPathAbsolute,
        ...
        async beforeWriteFiles (rootPage) {
          ...
          function addPage (parent: EditableTreeNode, page: NuxtPage) {
            ...
            addedPagePaths.set(absolutePagePath, {
              file: page.file,
              name: route.name,
            })
          }
      ...
      nuxt.hook('prepare:types', ({ references, tsConfig }) => {
        ...
        references.push({ path: typedRouterDtsPathRelative })
        // This file will be generated by us
        references.push({ path: fileRouteNameMapDtsPathRelative })
        references.push({ types: 'unplugin-vue-router/client' })

        // TODO: right now we add the VLS plugin whenever `typedPages: true` and `languageFeatures: true`,
        // but we probably want to add the plugin only when `languageFeatures: true` and then set some flags
        // to `true` when `typedPages: true`.
        if (nuxt.options.experimental.languageFeatures) {
          const pluginPath = import.meta.resolve('@nuxt/language-features').replace(/^file:\/\//, '')
          const pluginPathRelative = relative(nuxt.options.buildDir, pluginPath)

          // TODO: tried a bunch of these, but can't resolve to a subdep with `resolveModulePath` (nuxt -> @nuxt/language-features)
          // const nuxtModulePath = dirname(resolveModulePath('nuxt/package.json'))
          // console.log(nuxtModulePath)
          // console.log(resolveModulePath('@nuxt/language-features', { from: nuxtModulePath }))
          // const p = relative(nuxt.options.buildDir, resolveModulePath('@nuxt/language-features'))

          tsConfig.vueCompilerOptions ??= {}
          tsConfig.vueCompilerOptions.plugins ??= []
          tsConfig.vueCompilerOptions.plugins.push(pluginPathRelative)
        }
      })

      const context = createRoutesContext(resolveOptions(typedRouterOptions))

      // TODO: ensure this function doesn't run multiple times in parallel. if it does, stop the previous run.
      async function generateTypedRouterFiles (startWatchers: boolean) {
        // ensure types directory exists
        await mkdir(dirname(typedRouterDtsPathAbsolute), { recursive: true })

        await context.scanPages(startWatchers)

        const fileRouteNameMap = Array.from(addedPagePaths.values())
          .filter(({ file }) => file !== undefined)
          .map(({ file, name }) => `'${file}': '${name}'`)
          .join('\n    ')

        const typedPagesDtsContents = `declare module '#app' {
  export interface FileRouteNameMap {
    ${fileRouteNameMap}
  }

  export type GetRouteNameByPath<T extends string> = T extends keyof FileRouteNameMap
    ? FileRouteNameMap[T]
...
`
        ...
        await writeFile(fileRouteNameMapDtsPathAbsolute, typedPagesDtsContents, 'utf-8')
      }

      await generateTypedRouterFiles(false)

      if (nuxt.options._prepare || !nuxt.options.dev) {
        // TODO: could we generate this from context instead?
        const typedRouterDtsContents = await readTypedRouterDts()
        addTemplate({
          filename: 'types/typed-router.d.ts',
          getContents: () => typedRouterDtsContents,
        })
      }
      ...
      nuxt.hook('app:templatesGenerated', async (_app, _templates, options) => {
        if (!options?.filter || options.filter({ filename: 'routes.mjs' } as any)) {
          await generateTypedRouterFiles(true)
        }
      })
    }
