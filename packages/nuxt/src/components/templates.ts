...
export const componentsTemplate: NuxtTemplate<ComponentsTemplateContext> = {
  ...
  getContents ({ options }) {
    ...
    const components = options.getComponents(options.mode).filter(c => !c.island).flatMap((c) => {
      ...
      definitions.push(`export const Lazy${c.pascalName} = /*#__PURE__*/ defineAsyncComponent(${genDynamicImport(c.filePath, { comment })}.then(c => ${isClient ? `/*#__PURE__*/ createClientOnly(${exp})` : exp}))`)
      return definitions
    })
    return [
      ...imports,
