...
vi.mock('nitropack/runtime', () => ({ useNitroApp: () => nitroApp, useRuntimeConfig: () => ({ app: { baseURL: '/' } }) }))
...
describe('forwarding channel', () => {
  ...
  it('posts reports, paired with the request, to the owning dev server', async () => {
    ...
    const event = {
      method: 'GET',
      path: '/ok?a=1',
      headers: new Headers({ 'x-nuxt-dev-request-id': '42' }),
      node: { req: { headers: { host: 'localhost' } } },
    } as any
