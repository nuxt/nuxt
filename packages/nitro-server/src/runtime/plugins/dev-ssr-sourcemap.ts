...
interface RunnerModules {
  ...
  /**
   * Lines the evaluator prepends to a module before running it. The module's
   * sourcemap is shifted by them, so its generated positions sit ahead of the
   * code the module was served as.
   */
  startOffset: number
}
...
function getCompiledPosition (file: string, line: number, column?: number): { file: string, line: number, column: number } | undefined {
  for (const { modules, startOffset } of getRunnerModules()) {
    for (const id of candidateIds(modules, file)) {
      ...
      // positions are reported against the code `getCode` returns, so the
      // evaluator's own lines come back off
...
export default definePlugin((nitroApp) => {
  nitroApp.ssrSourceMaps = {
    getCode,
    getCompiledPosition,
    // the `ModuleRunner` installs a sourcemap interceptor, so `error.stack` is
    // mapped as it is generated
