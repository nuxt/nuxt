...
export default <NitroErrorHandler> async function errorhandler (_error, event, { defaultHandler }) {
  ...
  if (!res) {
    ...
    if (import.meta.dev && isRenderingError) {
      // the request that failed holds the report worth rendering, so it renders the page
  ...
  if (import.meta.dev && !import.meta.test && report && typeof html === 'string') {
    const { publishErrorReport, renderErrorPage, withErrorOverlay } = await import('../utils/error-channel')
    try {
      ...
      html = res.headers.has(ERROR_PAGE_HEADER)
        ...
        ? await renderErrorPage(report)
        // the page behind is the app's own error page, so the report sits a click away
        : await withErrorOverlay(html, report, { startMinimized: true })
    } catch {
...
/** Set when the app's own error page could not render, so the request that failed renders the report. */
