...
export async function bundle (nuxt: Nuxt & { _nitro?: Nitro }): Promise<void> {
  ...
  if (nitro.options.static && nuxt.options.experimental.payloadExtraction === false) {
    warnBuild('Payload extraction is recommended for full-static output.', { code: ErrorCodes.B7015, fix: 'Enable it by setting `experimental.payloadExtraction` to `true` or `\'client\'`.' })
  }
