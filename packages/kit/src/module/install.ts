...
export async function loadNuxtModuleInstance (nuxtModule: string | NuxtModule, nuxt: Nuxt = useNuxt()): Promise<{ nuxtModule: NuxtModule<any>, buildTimeModuleMeta: ModuleMeta, resolvedModulePath?: string }> {
  ...
  try {
    ...
    if (typeof resolvedNuxtModule !== 'function') {
      buildErrorUtils.throw({ message: `Nuxt module should be a function: \`${nuxtModule}\`.`, code: ErrorCodes.B8016, fix: 'Ensure the module has a default export that is a function. Use `defineNuxtModule()` to create a valid module.' })
    }
    ...
    if (code === 'ERR_PACKAGE_PATH_NOT_EXPORTED' || code === 'ERR_UNSUPPORTED_DIR_IMPORT' || code === 'ENOTDIR') {
      buildErrorUtils.throw({ message: `Could not load \`${nuxtModule}\`. Is it installed?`, code: ErrorCodes.B8017, fix: `Run \`npm install ${nuxtModule}\` to install it.` })
    }
    if (code === 'MODULE_NOT_FOUND' || code === 'ERR_MODULE_NOT_FOUND') {
      ...
      if (module && !module.includes(nuxtModule as string)) {
        buildErrorUtils.throw({ message: `Error while importing module \`${nuxtModule}\`: ${error}`, code: ErrorCodes.B8018, fix: 'A sub-dependency of this module is missing. Install it, or check that the module is compatible with the current environment.' })
      }
    }
  }
