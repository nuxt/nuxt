...
export const configDiagnostics = /* #__PURE__ */ defineDiagnostics({
  ...
  codes: {
    ...
    NUXT_B5002: {
      ...
      fix: (p: { install: string }) => `Install it manually with \`${p.install}\`, or change the \`builder\` option to \`vite\` in \`nuxt.config\`.`,
      docs: false,
    },
    NUXT_B5003: {
    ...
    NUXT_B5009: {
      ...
      fix: (p: { install: string }) => `Install \`@parcel/watcher\` for better file watching: \`${p.install}\`.`,
      docs: false,
    },
    NUXT_B5010: {
      why: (p: { names: string }) => `Required packages are not installed: ${p.names}.`,
      fix: (p: { install: string }) => `Run \`${p.install}\` to install them.`,
      docs: false,
    },
    NUXT_B5011: {
