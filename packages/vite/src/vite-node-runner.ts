...
/**
 * Rewrite the positions in a stack string from SSR-transformed positions to source
 * positions.
 *
 * Columns are emitted 1-based, as in a V8 stack, rather than the 0-based columns a
 * sourcemap stores. `ssrFixStacktrace` only recognises parenthesised positions, so
 * frames without a function name are parenthesised for the mapping and restored
 * afterwards, and it is given a throwaway carrier because it can rewrite a stack
 * asynchronously.
 *
