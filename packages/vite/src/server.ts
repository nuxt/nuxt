...
export async function buildServer (nuxt: Nuxt, ctx: ViteBuildContext) {
  ...
  const serverConfig: ViteConfig = vite.mergeConfig(
    ctx.config,
    vite.mergeConfig(
      {
        configFile: false,
        ...
        cacheDir: resolve(
          ...
          ctx.config.cacheDir ?? 'node_modules/.cache/vite',
          'server',
        ),
        build: {
          ...
          sourcemap: nuxt.options.sourcemap.server
            ? ctx.config.build?.sourcemap ?? nuxt.options.sourcemap.server
            : false,
            ...
            external: [
              ...
              new RegExp(
                '^' +
                  escapeStringRegexp(
                    withTrailingSlash(
                      resolve(nuxt.options.rootDir, nuxt.options.dir.shared),
                    ),
                  ),
              ),
            ],
  ...
  if (
    ...
    !Array.isArray(serverConfig.build.rollupOptions.output)
  ) {
