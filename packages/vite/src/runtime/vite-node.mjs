...
export default async (ssrContext) => {
  ...
  if (updates.size) {
    ...
    consola.success(
      `Vite server hmr ${updates.size} files`,
      time ? `in ${time}ms` : ""
    );
  }
    ...
    async fetchModule(id) {
      ...
      return await viteNodeFetch("/module/" + encodeURI(id)).catch((err) => {
        ...
        try {
          ...
          consola.warn(
            "Internal nuxt error while formatting vite-node error. Please report this!",
            formatError
          );
          const message = `[vite-node] [TransformError] ${
            errorData?.message || "-"
          }`;
...
function formatViteError(errorData, id) {
  ...
  const getLocId = (locObj = {}) =>
    locObj.file || locObj.id || locObj.url || id || "";
  /** @param locObj {{ line?: string, column?: string }} */
  const getLocPos = (locObj = {}) =>
    locObj.line ? `${locObj.line}:${locObj.column || 0}` : "";
  const locId =
  ...
  const stack = [message, `at ${loc}`, errorData.stack]
    .filter(Boolean)
    .join("\n");
