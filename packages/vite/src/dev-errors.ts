...
/**
 * Turns the compile errors Vite raises while transforming into reports on the
 * dev error channel. Whoever owns the channel (the Nitro worker, or a dev
 * server in front) receives them over a `BroadcastChannel`, so error pages and
 * overlays update before anyone requests a page.
 */
export function createDevErrorReporter (nuxt: Nuxt, options: { print: (rendered: string) => void }): DevErrorReporter {
  ...
  // pages already open are shown the overlay directly rather than reloading;
  // once mounted it follows the channel like any other error page
  let server: ViteDevServer | undefined
  // an error raised while the page loads predates its hmr connection, so the
  // overlay is kept and replayed to each client that connects
  ...
  async function showOverlay (report: ErrorReport, options: { reloadOnClear?: boolean } = {}) {
    ...
    // the page underneath is the one the developer was looking at, so the
    // overlay arrives minimised
  ...
  broadcast.onmessage = (event) => {
    ...
    switch (message?.type) {
      ...
      case 'nuxt:dev:error:sync':
        ...
        return
      // an error the server hit while rendering, which the bundler never saw;
      // a compile error is remembered by file so that fixing the file clears
      // it, and anything else is cleared by the next update
  ...
  /**
   * Build and publish a report for an error the browser raised. Its stack
   * points at the modules Vite served, so the URLs are resolved back to files
   * before the report maps them through the module graph.
   */
  ...
  function resolveStackUrls (devServer: ViteDevServer, stack: string): string {
    ...
    // module graph urls are relative to the bundler's base, which the app's
    // own base URL sits in front of in the browser
...
/**
 * Clears the report once the file that failed compiles again, and clears a
 * browser runtime error on any update.
 *
 * Vite only transforms on demand, and the page showing the error has no HMR
 * client to ask for it, so the failed file is transformed here when it
 * changes: once, in the first environment to see the change, and after Vite's
 * own update pass so it cannot race the invalidation of modules that compiled.
 */
