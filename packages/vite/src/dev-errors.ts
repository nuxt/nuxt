...
const reporters = new WeakMap<Nuxt, DevErrorReporter>()

/** Remember the reporter for a build, so each vite server can report to the same one. */
export function setDevErrorReporter (nuxt: Nuxt, reporter: DevErrorReporter): void {
  reporters.set(nuxt, reporter)
}

/** The reporter for a build, absent outside development. */
export function getDevErrorReporter (nuxt: Nuxt): DevErrorReporter | undefined {
  return reporters.get(nuxt)
}

/** An `onTransformError` handler for the vite logger, reporting what it recognises. */
export function reportTransformError (reporter: DevErrorReporter | undefined): ((error: unknown) => boolean) | undefined {
  return reporter
    ? (error) => {
