...
describe('pickSocketPath', () => {
  it('uses a filesystem socket on Linux, not an abstract namespace socket', () => {
    ...
    const { socketPath, parentDir } = trackedPick('linux', tmpdir)
    expect(socketPath.startsWith('\0')).toBe(false)
    expect(parentDir).toBeDefined()
    expect(
      fs.realpathSync(parentDir!).startsWith(fs.realpathSync(tmpdir)),
    ).toBe(true)
  })

  it('uses a filesystem socket on macOS', () => {
    const tmpdir = createSocketTmpdir()
    const { socketPath, parentDir } = trackedPick('darwin', tmpdir)
    expect(socketPath.startsWith('\0')).toBe(false)
    expect(parentDir).toBeDefined()
    expect(
      fs.realpathSync(parentDir!).startsWith(fs.realpathSync(tmpdir)),
  ...
  it(
    'places the socket in a 0700 parent directory on Unix',
  ...
  it(
    'falls back to /tmp and stays within the `sun_path` limit when $TMPDIR is too long',
    { skip: process.platform === 'win32' },
    () => {
...
describe('listenAndRestrict', () => {
  ...
  it.runIf(process.platform !== 'win32')(
    'binds with no group/other permission bits before the listening callback fires',
    () => {
      const { socketPath } = trackedPick()
      const server = net.createServer()
      listenAndRestrict(server, socketPath)
  ...
  it.runIf(process.platform === 'win32')(
    'binds on a Windows named pipe without attempting chmod',
    async () => {
      ...
      try {
        ...
        expect(chmodSpy).not.toHaveBeenCalledWith(
          socketPath,
          expect.anything(),
        )
      } finally {
