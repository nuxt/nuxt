...
const GENERATED_TS_HEADER = GENERATED_NOTICE.map(line => `// ${line}`).join('\n') + '\n\n'
const GENERATED_VUE_HEADER = `<!--\n${GENERATED_NOTICE.map(line => `  ${line}`).join('\n')}\n-->\n\n`
  ...
  return <Plugin> {
    ...
    async writeBundle () {
      ...
      for (const fileName of htmlFiles) {
        ...
        // Write new template
        writeFileSync(fileName.replace('/index.html', '.ts'), GENERATED_TS_HEADER + functionalCode)
        writeFileSync(fileName.replace('/index.html', '.vue'), GENERATED_VUE_HEADER + vueCode)
