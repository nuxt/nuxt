...
export default function webpackClientConfig() {
  ...
  let env = {}
  fillEnv(this.options.env, env, 'process.env.')
...
function fillEnv(src, dest, prefix) {
  each(src, (value, key) => {
    if (typeof value === 'object') {
      fillEnv(value, dest, prefix + key + '.')
    }
    dest[prefix + key] = (['boolean', 'number'].indexOf(typeof value) !== -1 ? value : JSON.stringify(value))
  })
}
