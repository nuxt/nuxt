...
export default class WebpackClientConfig extends WebpackBaseConfig {
  ...
  plugins() {
    ...
    // Compression plugin
    if (!this.options.dev && this.options.build.compress) {
      plugins.push(new CompressionPlugin(Object.assign({
        exclude: [/vue-ssr-client-manifest.json/, /index.(?:ssr|spa).html/]
      }, this.options.build.compress)))
    }
