...
export default class Builder {
  ...
  async checkAssetsExist (assets, callback) {
    await Promise.all(assets.map(async (a) => {
      const file = `${a.src}${path.extname(a.src) ? '' : '.js'}`
  ...
  async webpackBuild() {
    ...
    await this.checkAssetsExist(this.css)

    // Ensure plugins exist then set alias to their real path
    await this.checkAssetsExist(this.plugins, (asset) => {
