...
export default class Builder {
  ...
  async build() {
    ...
    // Whether pages/index.js is present or not
    this._nuxtPagesIndex = false
  ...
  async generateRoutesAndFiles() {
    ...
    let customRoutesIndex

    // Check for router.index
    if (typeof this.options.router.index === 'string') {
      // Let the user specify entirely custom routes index file
      customRoutesIndex = this.nuxt.requireModule(r(this.options.router.index))
      templateVars.router.routes = customRoutesIndex
      this._nuxtPagesIndex = true
    }

    customRoutesIndex = r(this.options.srcDir, this.options.dir.pages, 'index.js')
    if (fsExtra.existsSync(customRoutesIndex)) {
      // Check if pages/index.js is defined
      customRoutesIndex = this.nuxt.requireModule(customRoutesIndex)
      templateVars.router.routes = customRoutesIndex
      this._nuxtPagesIndex = true
    }

    // Resolve component paths
    if (customRoutesIndex !== undefined) {
      templateVars.router.routes.forEach((route, i) => {
        templateVars.router.routes.component = route.component
      })
      this._nuxtPagesIndex = true
    }

    if (!this._nuxtPagesIndex) {
      if (this._defaultPage) {
        ...
        await new Promise(async (resolve) => {
          const files = await glob(`${this.options.dir.pages}/**/*.{vue,js}`, {
            cwd: this.options.srcDir,
            ignore: this.options.ignore
          })
          files.forEach((f) => {
