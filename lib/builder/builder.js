...
export default class Builder {
  ...
  async generateRoutesAndFiles() {
    ...
    if (typeof this.options.router.index === 'string') {
      ...
      templateVars.router.routes = this.nuxt.requireModule(
        r(this.options.router.index)
      )
      this._nuxtPagesIndex = true
    }

    // Check if pages/index.js is defined
    let customRoutesIndex = r(
      path.join(
        this.options.srcDir,
    ...
    if (this._nuxtPagesIndex) {
      templateVars.router.routes.forEach((route, i) => {
        templateVars.router.routes[i].component = route.component
      })
    }
