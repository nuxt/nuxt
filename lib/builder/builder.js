...
export default class Builder {
  ...
  async generateRoutesAndFiles() {
    ...
    // Check for pages/index.js or router.index
    if (typeof this.options.router.index === 'string') {
      // let the user specify entirely custom routes index file
      customRoutesIndex = await import(r(this.options.router.index))
      templateVars.router.routes = customRoutesIndex.default
    } else if (fsExtra.existsSync(customRoutesIndex)) {
      // Check if pages/index.js is defined
      customRoutesIndex = await import(
        r(this.options.srcDir, this.options.dir.pages, 'index.js')
      )
      templateVars.router.routes = customRoutes.default
    }
    
    // Resolve component paths
    if (customRoutesIndex !== undefined) {
      customRoutesIndex = customRoutesIndex.map((route) => {
        return { ...route, component: r(route.component) }
      })
    }
