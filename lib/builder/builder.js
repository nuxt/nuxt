...
export default class Builder {
  ...
  async webpackBuild() {
    ...
    await Promise.all(this.options.css.map(async (css) => {
      ...
      const file = `${src}${path.extname(src) || ''}`
      const resolved = r(this.options.srcDir, file)
    ...
    await Promise.all(this.options.plugins.map(async (plugin) => {
      let src = plugin.src || plugin
      const file = `${src}${path.extname(src) ? '' : `.js`}`
      if (!await fsExtra.pathExists(r(file))) {
        throw new Error(`Plugin not found: ${file}`)
      }

      src = this.relativeToBuild(plugin.src)
