...
export default class Builder {
  ...
  async generateRoutesAndFiles() {
    ...
    let customRoutes
    // router.index property
    if (typeof this.options.router.index === 'string') {
      // let the user specify entirely custom routes index file
      customRoutes = await import(r(this.options.router.index))
      consola.debug('customRoutes', customRoutes)
      templateVars.router.routes = customRoutes
    } else if (fsExtra.existsSync(r(this.options.srcDir, this.options.dir.pages, 'index.js'))) {
