...
export default class Builder {
  ...
  async build() {
    ...
    if (this._nuxtPages) {
      if (!fsExtra.existsSync(path.join(this.options.srcDir, this.options.dir.pages))) {
        ...
        if (fsExtra.existsSync(path.join(this.options.srcDir, '..', this.options.lpages))) {
          throw new Error(
  ...
  async webpackBuild() {
    ...
    // Ensure listed CSS files exist
    await Promise.all(this.options.css.map(async (css) => {
      const src = css.src || css
      const file = `${src}${path.extname(src) ? '' : '.css'}`
      const resolved = this.nuxt.resolveAlias(this.options.srcDir, file)
      if (!await fsExtra.pathExists(resolved)) {
        throw new Error(`Stylesheet not found: ${resolved}`)
      }
    }))

    // Ensure plugins exist then set alias to their real path
    await Promise.all(this.options.plugins.map(async (plugin) => {
      const resolved = this.nuxt.resolveAlias(plugin.src)
      const file = `${resolved}${path.extname(plugin.src) ? '' : `.js`}`
      if (!await fsExtra.pathExists(file)) {
        throw new Error(`Plugin not found: ${file}`)
      }

      const src = this.relativeToBuild(plugin.src)

      // Client config
      if (!clientConfig.resolve.alias[plugin.name]) {
        clientConfig.resolve.alias[plugin.name] = src
      }

      // Server config
      if (serverConfig && !serverConfig.resolve.alias[plugin.name]) {
        // Alias to noop for ssr:false plugins
        serverConfig.resolve.alias[plugin.name] = plugin.ssr ? src : './empty.js'
      }
    }))
