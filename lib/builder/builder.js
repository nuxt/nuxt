...
import ModernWebpackConfig from './webpack/modern'
...
export default class Builder {
  ...
  async webpackBuild() {
    ...
    // Modern
    let modernConfig
    if (this.options.build.mode === 'modern') {
      modernConfig = new ModernWebpackConfig(this).config()
    ...
    await Promise.all(this.plugins.map(async (p) => {
      ...
      if (modernConfig && !modernConfig.resolve.alias[p.name]) {
        modernConfig.resolve.alias[p.name] = src
      }
