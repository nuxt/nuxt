...
module.exports = class Builder {
  ...
  async generateRoutesAndFiles() {
    ...
    if (existsSync(resolve(this.options.srcDir, this.options.dir.layouts))) {
      const extensions = this.options.pageExtensions
      const layoutsFiles = await glob(`${this.options.dir.layouts}/**/*.{${extensions.join(',')}}`, {
      ...
      layoutsFiles.forEach(file => {
        let name = file
          ...
          .slice(1)
          .join('/')
          .replace(RegExp(`.(${extensions.join('|')})$`), '')
        if (name === 'error') {
          hasErrorLayout = true
          return
        }
    ...
    if (this._nuxtPages) {
      ...
      const files = {}
      const extensions = this.options.pageExtensions
      ;(await glob(`${this.options.dir.pages}/**/*.{${extensions.join(',')}}`, {
        cwd: this.options.srcDir,
        ignore: this.options.ignore
      })).forEach(f => {
        const key = f.replace(RegExp(`.(${extensions.join('|')})$`), '')
  ...
  watchFiles() {
    const extensions = this.options.pageExtensions.join(',')
    const src = this.options.srcDir
    let patterns = [
      r(src, this.options.dir.layouts),
      r(src, this.options.dir.store),
      r(src, this.options.dir.middleware),
      r(src, `${this.options.dir.layouts}/*.{${extensions}}`),
      r(src, `${this.options.dir.layouts}/**/*.{${extensions}}`)
    ]
    if (this._nuxtPages) {
      patterns.push(
        r(src, this.options.dir.pages),
        r(src, `${this.options.dir.pages}/*.{${extensions}}`),
        r(src, `${this.options.dir.pages}/**/*.{${extensions}}`)
      )
    }
