...
export default class Builder {
  constructor(nuxt) {
    ...
    } else if (this.options.build.includeConfig) {
  ...
  async generateConfig() {
    const config = resolve(this.options.buildDir, 'dist', 'build.config.js')
    const options = _.omit(_.pick(this.options, Options.safeKeys), Options.unsafeKeys)
