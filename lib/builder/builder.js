...
export default class Builder {
  ...
  async webpackBuild() {
    ...
    await this.checkAssetsExist(this.plugins, 'js', (asset) => {
      const src = this.relativeToBuild(asset.src)

      // Client config
      if (!clientConfig.resolve.alias[asset.name]) {
        clientConfig.resolve.alias[asset.name] = src
      }

      // Server config
      if (serverConfig && !serverConfig.resolve.alias[asset.name]) {
        // Alias to noop for ssr:false plugins
        serverConfig.resolve.alias[asset.name] = p.ssr ? src : './empty.js'
      }
    })
