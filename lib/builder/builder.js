...
export default class Builder {
  ...
  async checkAssetsExist(assets, ext, callback) {
    await Promise.all(assets.map(async (a) => {
      const file = `${a.src}${path.extname(a.src) ? '' : `.${ext}`}`
  ...
  async webpackBuild() {
    ...
    await this.checkAssetsExist(this.options.css, 'css')

    // Ensure plugins exist then set alias to their real path
    await this.checkAssetsExist(this.plugins, 'js', (plugin) => {
