  ...
  async build() {
    ...
    if (this._nuxtPages) {
      if (!fsExtra.existsSync(path.join(this.options.srcDir, this.options.dir.pages))) {
        ...
        if (fsExtra.existsSync(path.join(this.options.srcDir, '..', this.options.dir.pages))) {
          throw new Error(
            `No \`${this.options.dir.pages}\` directory found in ${dir}. ` +
            'Did you mean to run `nuxt` in the parent (`../`) directory?'
          )
        } else {
    ...
    await Promise.all(this.options.css.map(async (css) => {
      ...
      const resolved = this.nuxt.resolveAlias(src)
      const file = `${resolved}${path.extname(src) ? '' : '.css'}`
