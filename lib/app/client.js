...
async function render (to, from, next) {
  ...
  try {
    ...
    await Promise.all(Components.map((Component, i) => {
      ...
      const hasManualLoading = Component.options.loading === 'manual'
      ...
      <% if (loading && !hasManualLoading) { %>const loadingIncrease = (hasAsyncData && hasFetch) ? 30 : 45<% } %>
    ...
    // If not redirected
    if (!nextCalled) {
      <% if (loading && !hasManualLoading) { %>if(this.$loading.finish) this.$loading.finish()<% } %>
      next()
    }
