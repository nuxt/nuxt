...
const NUXT = window.<%= globals.state %> || {}
...
function nuxtReady (_app) {
  window.<%= globals.callbacks.ready %>.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window.<%= globals.callbacks.loaded %> === 'function') {
    window.<%= globals.callbacks.loaded %>(_app)
  }
  // Add router hooks
...
async function mountApp(__app) {
  ...
  const mount = () => {
    ...
    Vue.nextTick(() => {
      // Call window.{{globals.callbacks.ready}} callbacks
      nuxtReady(_app)
      <% if (isDev) { %>
