...
const NUXT = window.__<%= globalName.toUpperCase() %>__ || {}
...
Vue.config.errorHandler = (err, vm, info, ...rest) => {
  ...
  if (vm && vm.$root && vm.$root.$<%= globalName %> && vm.$root.$<%= globalName %>.error && info !== 'render function') {
    vm.$root.$<%= globalName %>.error(nuxtError)
  }
  if (typeof defaultErrorHandler === 'function') {
    return handled
  }
