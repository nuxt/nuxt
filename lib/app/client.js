...
const NUXT = window.__<%= GLOBALNAME %>__ || {}

Object.assign(Vue.config, <%= serialize(vue.config) %>)

<% if (debug || mode === 'spa') { %>
// Setup global Vue error handler
if (!Vue.config.errorHandler.$nuxt) {
  ...
  Vue.config.errorHandler = (err, vm, info, ...rest) => {
    ...
    const nuxtApp = Object.keys(Vue.config.errorHandler.$nuxt)
      .find((globalName) => vm.$root[`$${globalName}`])
    
    // Show Nuxt Error Page
    if (vm && vm.$root && nuxtApp && nuxtApp.error && info !== 'render function') {
  ...
  Vue.config.errorHandler.$nuxt = {}
}
Vue.config.errorHandler.$nuxt.<%= globalName %> = true
...
async function loadAsyncComponents (to, from, next) {
  ...
  try {
    ...
    this.$<%= globalName %>.$emit('routeChanged', to, from, err)
    next(false)
  }
}
...
async function render (to, from, next) {
  ...
  try {
    ...
    if (!error) {
      ...
      return this.$<%= globalName %>.$emit('routeChanged', to, from, error)
    }
    _lastPaths = []
    ...
    this.error(error)
    this.$<%= globalName %>.$emit('routeChanged', to, from, error)
    next(false)
  }
}
...
function nuxtReady (_app) {
  window._<%= globalName %>ReadyCbs.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window._on<%= _.capitalize(globalName) %>Loaded === 'function') {
    window._on<%= _.capitalize(globalName) %>Loaded(_app)
  }
  // Add router hooks
  router.afterEach((to, from) => {
    // Wait for fixPrepatch + $data updates
    Vue.nextTick(() => _app.$<%= globalName %>.$emit('routeChanged', to, from))
  })
}
...
function hotReloadAPI (_app) {
  ...
  let $components = getNuxtChildComponents(_app.$<%= globalName %>, [])
...
async function mountApp(__app) {
  ...
  const mount = () => {
    _app.$mount('#__<%= globalName %>')

    // Listen for first Vue update
    Vue.nextTick(() => {
      // Call window.on{{globalName}}Ready callbacks
      nuxtReady(_app)
      <% if (isDev) { %>
