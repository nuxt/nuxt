...
// window.{{globals.callbacks.loaded}} hook
// Useful for jsdom testing or plugins (https://github.com/tmpvar/jsdom#dealing-with-asynchronous-script-loading)
if (process.client) {
  window.<%= globals.callbacks.ready %>Cbs = []
  window.<%= globals.callbacks.ready %> = (cb) => {
    window.<%= globals.callbacks.ready %>Cbs.push(cb)
  }
}
...
export async function setContext(app, context) {
  ...
  if (!app.context) {
    ...
    if (process.client) {
      app.context.nuxtState = window.<%= globals.state %>
    }
  }
  // Dynamic keys
