...
// window.on{{globalName}}Ready(() => console.log('Ready')) hook
// Useful for jsdom testing or plugins (https://github.com/tmpvar/jsdom#dealing-with-asynchronous-script-loading)
if (process.browser) {
  window._<%= globalName %>ReadyCbs = []
  window.on<%= _.capitalize(globalName) %>Ready = function (cb) {
    window._<%= globalName %>ReadyCbs.push(cb)
  }
}
...
export async function setContext(app, context) {
  ...
  if (!app.context) {
    ...
    if (process.client) app.context.nuxtState = window.__<%= globalName.toUpperCase() %>__
  }
  // Dynamic keys
