...
async function createApp (ssrContext) {
  ...
  const inject = function (key, value) {
    ...
    const installKey = '__<%= globalName %>_' + key + '_installed__'
  ...
  if (process.browser) {
    ...
    if (window.__<%= globalName.toUpperCase() %>__ && window.__<%= globalName.toUpperCase() %>__.state) {
      store.replaceState(window.__<%= globalName.toUpperCase() %>__.state)
    }
  }
  <% } %>
