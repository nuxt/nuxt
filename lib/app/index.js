...
/* Plugins */
<% plugins.forEach((plugin) => { %>import <%= plugin.name %> from '<%= plugin.name %>'<%= isTest ? '/* eslint-disable-line camelcase */' : '' %> // Source: <%= relativeToBuild(plugin.src) %><%= (plugin.ssr===false) ? ' (ssr: false)' : '' %>
<% }) %>
