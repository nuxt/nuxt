...
export default class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    const serializedSession = `window.__${this.options.GLOBALNAME}__=${devalue(context.nuxt)};`
  ...
  async renderAndGetWindow(url, opts = {}) {
    ...
    const nuxtExists = window.document.body.innerHTML.includes(
      this.options.render.ssr ? `window.__${this.options.GLOBALNAME}__` : `id="__${this.options.globalName}">`
    )
    /* istanbul ignore if */
    if (!nuxtExists) {
    ...
    const GlobalName = _.capitalize(this.options.globalName)
    await timeout(new Promise((resolve) => {
      window[`_on${GlobalName}Loaded`] = () => resolve(window)
