...
export default class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    const serializedSession = `window.${this.options.globals.context}=${devalue(context.nuxt)};`
  ...
  async renderAndGetWindow(url, opts = {}) {
    ...
    const nuxtExists = window.document.body.innerHTML.includes(
      this.options.render.ssr ? `window.${this.options.globals.context}` : `<div id="${this.options.globals.id}">`
    )
    /* istanbul ignore if */
    if (!nuxtExists) {
    ...
    const onNuxtLoaded = this.options.globals.callbacks.loaded
