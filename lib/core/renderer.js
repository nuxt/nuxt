...
export default class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    if (this.noSSR || spa) {
      ...
      const APP =
        `<div id="__${this.options.globalName}">${this.resources.loadingHTML}</div>` + BODY_SCRIPTS
    ...
    if (!context.nuxt.serverRendered) {
      APP = `<div id="__${this.options.globalName}"></div>`
    }
    ...
    const serializedSession = `window.__${this.options.globalName.toUpperCase()}__=${serialize(context.nuxt, {
      isJSON: true
    })};`
  ...
  async renderAndGetWindow(url, opts = {}) {
    ...
    const nuxtExists = window.document.body.innerHTML.includes(
      this.options.render.ssr ? `window.__${this.options.globalName.toUpperCase()}__` : `<div id="__${this.options.globalName}">`
    )
    /* istanbul ignore if */
    if (!nuxtExists) {
    ...
    await timeout(new Promise(resolve => {
      window[`_on${_.capitalize(this.options.globalName)}Loaded`] = () => resolve(window)
