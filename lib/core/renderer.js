...
export default class Renderer {
  ...
  async renderRoute(url, context = {}) {
    ...
    if (this.noSSR || spa) {
      ...
      const APP =
        `<div id="${this.options.globals.id}">${this.resources.loadingHTML}</div>` + BODY_SCRIPTS
    ...
    if (!context.nuxt.serverRendered) {
      APP = `<div id="${this.options.globals.id}"></div>`
    }
    ...
    const serializedSession = `window.${this.options.globals.state}=${devalue(context.nuxt)};`
  ...
  async renderAndGetWindow(url, opts = {}) {
    ...
    const nuxtExists = window.document.body.innerHTML.includes(
      this.options.render.ssr ? `window.${this.options.globals.state}` : `<div id="${this.options.globals.id}">`
    )
    /* istanbul ignore if */
    if (!nuxtExists) {
    ...
    const nuxtInstance = this.options.globals.nuxt
    await timeout(new Promise((resolve) => {
      window[nuxtInstance] = () => resolve(window)
