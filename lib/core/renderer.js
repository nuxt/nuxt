...
export default class Renderer {
  constructor(nuxt) {
    ...
    // Will be available once dist is built
    this.gzipStaticMiddleware = null
  ...
  async loadResources(_fs = fs) {
    ...
    // Load gzipStatic middleware
    if (!this.options.dev && fs.existsSync(distPath)) {
      this.gzipStaticMiddleware = gzipStatic(distPath, {
  ...
  async setupMiddleware() {
    ...
    const staticDir = path.resolve(this.options.srcDir, this.options.dir.static)
    if (fs.existsSync(staticDir)) {
      const staticMiddleware = gzipStatic(staticDir, this.options.render.static)
    ...
    if (!this.options.dev) {
      this.useMiddleware({
        ...
        handler: (req, res, next) => {
          if (this.gzipStaticMiddleware) {
            return this.gzipStaticMiddleware(req, res, next)
          } else {
            return next()
          }
        }
