...
Options.from = function (_options) {
  ...
  // Dummy proof buildDir and generate.dir as those folders
  //  will be completely removed when used
  if (options.rootDir.startsWith(options.buildDir)) {
    const errorMessage =
      'options.buildDir cannot be set lower than or equal to rootDir'
    consola.fatal(errorMessage)
    throw errorMessage
  }

  if (options.generate && options.generate.dir) {
    const distPath = path.resolve(options.rootDir, options.generate.dir)
    if (options.rootDir.startsWith(distPath)) {
      const errorMessage =
