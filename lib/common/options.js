...
Options.from = function (_options) {
  ...
  // Grouping patterns by preprocessors
  if (options.build.styleResources) {
    let patterns = options.build.styleResources.patterns
    if (!Array.isArray(patterns)) patterns = [patterns]

    options.build.styleResources.patternsNuxt = _
      .chain(patterns)
      .map((pattern) => glob.sync(pattern, { cwd: options.srcDir }))
      .flatten()
      .groupBy((path) => extname(path).slice(1))
      .value()
  }
