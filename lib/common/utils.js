...
function cleanChildrenRoutes(routes, hasDynamicParent = false, isChild = false) {
  let start = -1
  let routesIndex = []
  routes.forEach((route) => {
    if (hasDynamicParent || /-index$/.test(route.name) || route.name === 'index') {
  ...
  routes.forEach((route) => {
    ...
    if (route.children) {
      ...
      route.children = cleanChildrenRoutes(route.children, /^\/:/.test(route.path), true)
    }
  })
  return routes
}
