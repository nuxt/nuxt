...
// Guard dir1 from dir2 which is indiscriminately removed when used
export const guardDir = function guardDir(options, key1, key2) {
  const dir1 = _.get(options, key1)
  const dir2 = _.get(options, key2)

  if (dir1.startsWith(dir2)) {
    const errorMessage =
      `options.${key2} cannot be a parent of or same as ${key1}`
